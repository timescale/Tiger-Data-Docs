...
{C.TIMESCALE_DB} also creates sparse indexes per {C.COLUMNSTORE} {C.CHUNK} for optimization, including bloom indexes for equality filters and min/max indexes for range filters. You can manually configure these indexes when you call [`CREATE TABLE`](/reference/timescaledb/hypertables/create_table) or [`ALTER TABLE`](/reference/timescaledb/hypercore/alter_table).
