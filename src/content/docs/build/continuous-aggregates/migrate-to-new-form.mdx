...
<Callout variant="warning">
  `cagg_migrate()` was removed in {C.TIMESCALE_DB} 2.25. The migration planning tables
  (`_timescaledb_catalog.continuous_agg_migrate_plan` and
  `_timescaledb_catalog.continuous_agg_migrate_plan_step`) were removed in
  {C.TIMESCALE_DB} 2.26. If you are on {C.TIMESCALE_DB} 2.25 or later, this procedure is
  no longer available.
</Callout>
