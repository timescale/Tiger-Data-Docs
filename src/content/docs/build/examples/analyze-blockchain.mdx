...
import CreateHypertableBlockchain from "../../../../partials/_create-hypertable-blockchain.mdx";
import AddDataBlockchain from "../../../../partials/_add-data-blockchain.mdx";
import FinancialIndustry from "../../../../partials/_financial-industry-data-analysis.mdx";
...
import imgGrafanaTransactionsFees from "../../../../assets/images/build/examples/grafana-transactions-fees.webp";
import imgGrafanaVolumeRate from "../../../../assets/images/build/examples/grafana-volume-rate.webp";
import imgGrafanaTransactionsMiningfee from "../../../../assets/images/build/examples/grafana-transactions-miningfee.webp";
import imgGrafanaBlockweightMiningfee from "../../../../assets/images/build/examples/grafana-blockweight-miningfee.webp";
import imgGrafanaCoinbaseRevenue from "../../../../assets/images/build/examples/grafana-coinbase-revenue.webp";
import imgGrafanaBlockweightRewards from "../../../../assets/images/build/examples/grafana-blockweight-rewards.webp";
import imgGrafanaBlockweightRevenue from "../../../../assets/images/build/examples/grafana-blockweight-revenue.webp";

<FinancialIndustry />

In this tutorial, you use {C.CLOUD_LONG} to ingest, store, and analyze transactions
on the Bitcoin blockchain.

[Blockchains](https://www.pcmag.com/encyclopedia/term/blockchain) are, at their essence, a distributed database. The
[transactions](https://www.pcmag.com/encyclopedia/term/bitcoin-transaction) in a blockchain are an example of time-series data. You can use
{C.TIMESCALE_DB} to query transactions on a blockchain, in exactly the same way as you
might query time-series transactions in any other database.

This tutorial uses a sample Bitcoin dataset and covers:

1.  **Ingest data**: set up and connect to a {C.SERVICE_LONG}, create tables and {C.HYPERTABLE}s, and ingest data.
1.  **Query the data**: obtain information about recent transactions and blocks using basic SQL queries.
1.  **Analyze the data**: create {C.CAGG}s and use {C.TIMESCALE_DB} hyperfunctions to discover insights about transaction fees, mining revenue, and market correlations.
...
[Coinbase](https://www.pcmag.com/encyclopedia/term/coinbase-transaction) transactions are the first transaction in a block, and
...
<NumberedList>
<NumberedItem title={`Connect to the ${C.SERVICE_LONG} that contains the Bitcoin dataset`}>
</NumberedItem>
<NumberedItem title="Select the five most recent coinbase transactions">
...
<NumberedItem title="Check the results">
...
<NumberedItem title="Select the five most recent non-coinbase transactions">
...
<NumberedItem title="Select the five most recent blocks">
...
In this section, you use [{C.TIMESCALE_DB} hyperfunctions](/reference/timescaledb/hyperfunctions) to construct
analytical queries that are not possible in standard {C.PG}. You create {C.CAGG}s
...
You can use [{C.CAGG}s](/learn/continuous-aggregates/about-continuous-aggregates) to simplify and speed up your
queries. For this analysis, you need three {C.CAGG}s focusing on
three aspects of the dataset: Bitcoin transactions, blocks, and coinbase
transactions. In each {C.CAGG} definition, the `time_bucket()`
...
<NumberedItem title={`Create a ${C.CAGG} called one_hour_transactions`}>
...
<NumberedItem title={`Add a refresh policy to keep the ${C.CAGG} up-to-date`}>
...
<NumberedItem title={`Create a ${C.CAGG} called one_hour_blocks`}>
...
<NumberedItem title={`Create a ${C.CAGG} called one_hour_coinbase`}>

This view holds aggregated data about all the transactions that miners received as rewards
each hour:
...
<NumberedItem title={`Query average transaction volume and fees from the one_hour_transactions ${C.CAGG}`}>
...
<NumberedItem title="Visualize this in Grafana">

<ol type="a">
<li>From the `Dashboards` page, click `New` and select `New dashboard`.</li>
<li>Click `Add visualization`, then select the data source that connects to your {C.SERVICE_LONG}.</li>
<li>In the `Queries` section, change the `Format` to `Time series` and select `Code`.</li>
<li>

Type the query from the previous step and click `Run query`.

<Image src={imgGrafanaTransactionsFees} alt="Visualizing number of transactions and fees" />
...
<NumberedItem title="Query the trading volume and the BTC to US Dollar exchange rate">
...
<li>Click `Add visualization`, then select the data source that connects to your {C.SERVICE_LONG}.</li>
<li>In the `Queries` section, change the `Format` to `Time series` and select `Code`.</li>
<li>Type the query from the previous step and click `Run query`.</li>
<li>Under the panel options on the right, click `Add field override` > `Fields with name`, then choose `btc-usd rate` in the dropdown.</li>
<li>

Click `Add override property`, then select `Axis > Placement` and click `Right`.

<Image src={imgGrafanaVolumeRate} alt="Visualizing transaction volume and BTC-USD conversion rate" />
...
<NumberedItem title="Query the number of transactions in a block, compared to the mining fee">
...
<li>In the `Queries` section, change the `Format` to `Time series` and select `Code`.</li>
<li>Type the query from the previous step and click `Run query`.</li>
<li>Under the panel options on the right, click `Add field override` > `Fields with name`, then choose `mining fee` in the dropdown.</li>
<li>

Click `Add override property`, then select `Axis > Placement` and click `Right`.

<Image src={imgGrafanaTransactionsMiningfee} alt="Visualizing transactions in a block and the mining fee" />
...
<NumberedItem title="Query block weight compared to the mining fee">
...
<Image src={imgGrafanaBlockweightMiningfee} alt="Visualizing blockweight and the mining fee" />
...
<NumberedItem title="Query coinbase transactions with block fees and rewards">
...
In the options panel, in the `Graph styles` section, for `Stack series` select `100%`.

<Image src={imgGrafanaCoinbaseRevenue} alt="Visualizing coinbase revenue sources" />
...
<NumberedItem title="Query block weight with block fees and rewards">
...
<Image src={imgGrafanaBlockweightRewards} alt="Visualizing block weight and mining fees" />
...
<NumberedItem title="Query average miner revenue per block with a 12-hour moving average">
...
<li>In the `Queries` section, change the `Format` to `Time series` and select `Code`.</li>
<li>Type the query from the previous step and click `Run query`.</li>
<li>Under the panel options on the right, click `Add field override` > `Fields with name`, then choose `revenue in USD` in the dropdown.</li>
<li>

Click `Add override property`, then select `Axis > Placement` and click `Right`.

<Image src={imgGrafanaBlockweightRevenue} alt="Visualizing block revenue over time" />
