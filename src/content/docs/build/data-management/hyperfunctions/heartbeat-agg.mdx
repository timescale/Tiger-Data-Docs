...
description: Analyze intermittent or irregular time-series data to determine system health over a given interval
...
overall health of a system over a given interval. {C.PG} provides window
functions that you use to get a sense of where unhealthy gaps are, but they can
be somewhat awkward to use efficiently.

This is one of the many cases where {C.HYPERFUNC} provide an efficient, simple solution for
...
This example uses the [SustData public dataset](https://osf.io/2ac8q/). This dataset tracks
...
When you have loaded the data into {C.HYPERTABLE}s, you can create a materialized
...
[hyperfunction API documentation](/reference/toolkit/state-tracking/heartbeat_agg/heartbeat_agg).