...
description: Use common statistical aggregates in window functions and continuous aggregates
...
Window functions and {C.CAGG}s often need the same statistical moments (mean, stddev, kurtosis) in a composable aggregate. {C.TIMESCALE_DB} wraps those patterns in `stats_agg` so you can reuse one state object across rollups instead of recomputing from scratch in plain {C.PG}.
...
rolled up. Additionally, because this query syntax is used in all {C.TOOLKIT_LONG} queries, when you are used to it, you can use it to construct more and
...
method, see the [blog post on aggregates](https://www.tigerdata.com/blog/how-postgresql-aggregation-works-and-how-it-inspired-our-hyperfunctions-design) or the
[developer documentation](https://github.com/timescale/timescaledb-toolkit/blob/main/docs/two-step_aggregation.md).
...
[hyperfunction API documentation](/reference/toolkit/statistical-and-regression-analysis/stats_agg-one-variable/stats_agg).