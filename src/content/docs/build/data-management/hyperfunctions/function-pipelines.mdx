...
description: Use function pipelines to improve how you write data analysis queries in PostgreSQL
...
import imgTimevectorLight from "../../../../../assets/images/build/data-management/timevector.png";
import imgTimevectorDark from "../../../../../assets/images/build/data-management/timevector-dark.png";
import imgTimeseriesVectorLight from "../../../../../assets/images/build/data-management/timeseries_vector.png";
import imgTimeseriesVectorDark from "../../../../../assets/images/build/data-management/timeseries_vector-dark.png";

Experimental **function pipelines** let you express multi-step analysis in one readable chain, similar to functional APIs in Pandas or PromQL, without nesting many levels of SQL. They bring functional-programming ideas from those tools and from {C.PG} and SQL into a single pipeline style.
...
time, that could look something like this:

<ThemeImage light={imgTimevectorLight} dark={imgTimevectorDark} alt="An example timevector" />
...
<ThemeImage light={imgTimeseriesVectorLight} dark={imgTimeseriesVectorDark} alt="An example of a timevector within a larger dataset" />
...
The Lambda element functions use the {C.TOOLKIT_LONG}'s experimental Lambda syntax to transform
...
- **Interval literals** are expressed with a trailing `i`. For example,
  `'1 day'i`. Except for the trailing `i`, these follow the {C.PG}
  `INTERVAL` input format.
- **Time literals** such as `'2021-01-02 03:00:00't` expressed with a
  trailing `t`. Except for the trailing `t` these follow the {C.PG}
...
| `num_resets()` | Number of times the counter reset |
...
- [`to_text`](#to_text), which allows you to specify the template
- [`to_plotly`](#to_plotly), which outputs a format suitable for use with the
