...
integrationCardTitle: "Start coding"
...
Every Tiger Cloud service is a 100% {C.PG} database hosted in Tiger Cloud with Tiger Data extensions such as TimescaleDB. You connect to your Tiger Cloud service from a standard Rails app configured for {C.PG}.

1.  **Create a new Rails app configured for {C.PG}** Rails creates and bundles your app, then installs the standard {C.PG} Gems.
...
3.  **Connect your app to your Tiger Cloud service**
    ...
    2.  Set the environment variable for `DATABASE_URL` to the value of `Service URL` from your [connection details](/integrate/find-connection-details)
    ...
    5.  Verify the connection from your app to your Tiger Cloud service: The result shows the list of extensions in your Tiger Cloud service
        ...
        foreign-data wrapper for remote {C.PG} servers
...
Hypertables are {C.PG} tables designed to simplify and accelerate data analysis. Anything you can do with regular {C.PG} tables, you can do with hypertables - but much faster and more conveniently.In this section, you use the helpers in the TimescaleDB gem to create and manage a [hypertable](/open-source/timescaledb/hypertables).
...
You are not limited to these languages. Tiger Cloud is based on {C.PG}, you can interface with TimescaleDB and Tiger Cloud using any [{C.PG} client driver](https://wiki.postgresql.org/wiki/List_of_drivers).