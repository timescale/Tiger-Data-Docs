...
1. **Optimize time-series data in {C.HYPERTABLE}s**
   ...
   [Connect to your {C.SERVICE_LONG}][in-console-editors] and create a table that will point to a Supabase database:
...
3. **Create optimized analytical queries**
   ...
   faster. {C.CAGG_CAP}s in {C.CLOUD_LONG} use {C.PG} [materialized views][postgres-materialized-views] to
...
2. **Check the data in your {C.SERVICE_LONG}**
   ...
   [Connect to your {C.SERVICE_LONG}][in-console-editors] and run the following query:
