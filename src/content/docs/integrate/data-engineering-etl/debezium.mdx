...
<Tabs label="Integrate with Debezium" persistKey="source-database">
  <Tab title={C.SELF_LONG_CAP} label="self-hosted">
  ...
  <Tab title={C.CLOUD_LONG} label="tiger-cloud">
    Debezium requires logical replication to be enabled. Currently, this is not enabled by default on {C.SERVICE_LONG}s.
    We are working on enabling this feature as you read. As soon as it is live, these docs will be updated.
  </Tab>
</Tabs>

And that is it,  you have configured Debezium to interact with {C.COMPANY} products.
