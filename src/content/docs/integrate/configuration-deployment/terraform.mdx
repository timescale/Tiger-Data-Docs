...
<Tabs>
  <Tab title={C.CLOUD_LONG}>
    You use the [{C.COMPANY} Terraform provider][terraform-provider] to manage {C.SERVICE_LONG}s:
  ...
  <Tab title={C.SELF_LONG_CAP}>
    You use the [`cyrilgdn/postgresql`][pg-provider] {C.PG} provider to connect to your {C.SELF_LONG} instance.
