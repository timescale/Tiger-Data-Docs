...
This page provides a list of metrics you can export with all {C.CLOUD_LONG} exporters.
...
These are the metrics exported by default by every {C.CLOUD_LONG} exporter. You can filter them by `project_id`, `service_id`, `role`, and `region`.
...
These additional metrics are collected if you tick `PostgreSQL metrics` when creating {C.CLOUD_LONG} exporters.
