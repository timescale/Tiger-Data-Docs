...
description: Reference for system and PostgreSQL metrics exported by Tiger Cloud
products: [cloud]
integrationPlatforms: ["aws", "azure"]
keywords: [metrics, postgresql, monitoring]
integrationCardLogo: "exported-metrics.png"
integrationCardLogoDark: "exported-metrics-dark.png"
---
...
This page provides a list of metrics you can export with all {C.CLOUD_LONG} exporters. You can filter them by `project_id`, `service_id`, `role`, and `region`.
...
## {C.CLOUD_LONG} metrics

These are the metrics exported by default by every {C.CLOUD_LONG} exporter.
...
These additional metrics are collected if you enable **{C.PG} metrics** when creating {C.CLOUD_LONG} exporters.
...
|Metric|Description|
|-|-|
|`pg_exporter_last_scrape_duration_seconds`|Duration of the last scrape of metrics from {C.PG}.|
|`pg_exporter_last_scrape_error`|Whether the last scrape of metrics from {C.PG} resulted in an error (1 for error, 0 for success).|
|`pg_exporter_scrapes_total`|Total number of times {C.PG} was scraped for metrics.|
...
These metrics expose the current values of {C.PG} configuration parameters as numeric values, covering memory limits, planner costs, autovacuum tuning, WAL configuration, and more.
...
|`pg_settings_timescaledb_max_background_workers`|Maximum background worker processes allocated to {C.TIMESCALE_DB}|
...
These metrics come from {C.PG}'s internal statistics collector and include connection counts, transaction throughput, buffer usage, replication state, and database-level I/O.

<Callout variant="note">

Metrics in the `pg_stat_database_*` family, along with `pg_stat_activity_count` and `pg_locks_count`, are emitted per database and carry a `datname` label. Your service's application data lives in the `tsdb` database, so filter on `datname="tsdb"` to see your own workload.
...
|`pg_stat_database_blks_hit_total`|Number of times disk blocks were found already in the buffer cache, so that a read was not necessary (this only includes hits in the {C.PG} buffer cache, not the operating system's file system cache).|
...
|`pg_static`|Version string as reported by {C.PG}.|
|`pg_up`|Whether the last scrape of metrics from {C.PG} was able to connect to the server (1 for yes, 0 for no).|