...
1. **Set up your {C.SERVICE_SHORT} connection string**
...
2. **Generate a Telegraf configuration file**
   ...
   various metrics about CPU usage, and the {C.PG} output plugin. `telegraf.conf`
...
4. **Configure the {C.PG} output plugin**
   ...
   2. Use {C.HYPERTABLE}s when Telegraf creates a new table:
