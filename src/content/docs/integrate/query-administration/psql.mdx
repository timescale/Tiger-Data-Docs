...
[`psql`](https://www.postgresql.org/docs/current/app-psql.html) is a terminal-based frontend to {C.PG} that enables you to type in queries interactively, issue them to {C.PG}, and see the query results. This page shows you how to use the `psql` command line tool to interact with your Tiger Cloud service.
...
To use `psql` to connect to your service, you need the connection details. See [Find your connection details](/integrate/find-connection-details). Connect to your service with either:

-   The parameter flags:
-   The service URL: You are prompted to provide the password.
-   The service URL with the password already included and [a stricter SSL mode](/deploy/tiger-cloud/secure-access/connect-to-a-service-with-stricter-ssl) enabled:
