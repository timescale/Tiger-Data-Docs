...
[`psql`][psql-docs] is a terminal-based frontend to {C.PG} that enables you to type in queries interactively, issue them to Postgres, and see the query results.
...
If you already have the latest version of `psql` installed, proceed to the [Connect to your {C.SERVICE_SHORT}][connect-using-psql] section.
...
<Tabs>
  ...
  <Tab title="Windows">
    ...
    1. Download and run the {C.PG} installer from [www.enterprisedb.com][windows-installer].
...
To use `psql` to connect to your {C.SERVICE_SHORT}, you need the connection details. See [Find your connection details][connection-info].
...
- The {C.SERVICE_SHORT} URL with the password already included and [a stricter SSL mode][ssl-mode] enabled:
...
For more on `psql` commands, see the [{C.COMPANY} psql cheat sheet][psql-cheat-sheet] and [psql documentation][psql-docs].
...
[connect-using-psql]: #connect-to-your-service
