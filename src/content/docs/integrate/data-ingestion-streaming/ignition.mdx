...
<NumberedList>
  ...
  <NumberedItem title={`Tell ${C.TIMESCALE_DB} what the current time is`}>
    Ignition stores `t_stamp` as a Unix epoch timestamp in milliseconds, not as a {C.PG} `timestamp`. Automated jobs such as columnstore and retention policies work out the age of each {C.CHUNK} relative to the current time, so you must tell {C.TIMESCALE_DB} how to read "now" for this integer column with [`set_integer_now_func`](/reference/timescaledb/hypertables/set_integer_now_func). The function must return milliseconds to match `t_stamp`:
