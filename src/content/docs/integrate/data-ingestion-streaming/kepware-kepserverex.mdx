...
This page shows you how to configure Kepware DataLogger to stream OPC data into {C.COMPANY} and store it in a {C.HYPERTABLE}.
...
- Your Tiger Cloud service configured with remote ODBC access enabled.

</Prerequisites>

## Prepare your {C.SERVICE_SHORT_CAP}

Create the target table in {C.COMPANY} before DataLogger begins writing. Convert that table to a {C.HYPERTABLE} so your OPC time-series data is partitioned and chunked correctly.
...
Point DataLogger at your {C.COMPANY} {C.HYPERTABLE} and choose a table write mode that preserves data.
...
<NumberedList>
  ...
  <NumberedItem title={`Select the ${C.COMPANY} DSN`}>

    In the log group settings, choose the DSN you created for {C.COMPANY}.

    If required, enter the DSN `Username` and `Password` for the {C.COMPANY} database.
  </NumberedItem>
  ...
  <NumberedItem title="Enable the log group">
    ...
    DataLogger begins sending OPC updates to your {C.COMPANY} {C.HYPERTABLE} when the log group is enabled.
  </NumberedItem>
</NumberedList>
...
<NumberedList>
  ...
  <NumberedItem title={`Confirm the data landed in ${C.COMPANY}`}>
...
You have successfully integrated Kepware KEPServerEX with {C.COMPANY}.
