...
<NumberedList>
  ...
  <NumberedItem title={`Create a ${C.HYPERTABLE} for MQTT messages`}>
    Create a compact `hypertable` suitable for time-series tag readings. The default structure stores a timestamp, a `tag_id` identifier, a numeric `value`, and the message metadata fields that come from the payload:
    ...
    <Callout variant="note">
      This layout (`ts`, `tag_id`, `value`, `quality`, `units`) depends entirely on what data you intend to extract from the MQTT topic and payload.
    </Callout>
