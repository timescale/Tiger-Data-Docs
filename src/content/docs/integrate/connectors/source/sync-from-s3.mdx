...
- The {C.S3_CONNECTOR} offers an option to enable a [{C.HYPERTABLE}](/learn/hypertables/understand-hypertables) during the file-to-table schema mapping setup. You can enable [{C.COLUMNSTORE}](/learn/columnar-storage) and [{C.CAGG}s](/learn/continuous-aggregates/about-continuous-aggregates) through the {C.SQL_EDITOR} once the connector has started running.
