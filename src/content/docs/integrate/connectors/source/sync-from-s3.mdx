...
![{C.CLOUD_LONG} overview][cloud_long-overview]
...
- The {C.S3_CONNECTOR} offers an option to enable a [{C.HYPERTABLE}][hypertables-section] during the file-to-table schema mapping setup. You can enable [{C.COLUMNSTORE}][compression] and [{C.CAGG}s][caggs] through the {C.SQL_EDITOR} once the connector has started running.
...
If you have any questions or feedback about the {C.S3_CONNECTOR}, join us in the [{C.COMPANY} community][join-livesync-on-slack].
...
- Configure access credentials for the S3 bucket.
  ...
  - [IAM Role][credentials-iam].
    ...
    - Configure the trust policy. Set the:
      ...
      - `ExternalID`: set to the [{C.CLOUD_LONG} {C.PROJECT_SHORT} and {C.SERVICE_LONG} ID][get-project-id] of the {C.SERVICE_SHORT} you are syncing to in the format `<projectId>/<serviceId>`.
...
1. **Connect to your {C.SERVICE_LONG}**
   ...
   In [{C.CONSOLE}][services-portal], select the {C.SERVICE_SHORT} to sync live data to.

2. **Connect the source S3 bucket to the target {C.SERVICE_SHORT}**

   ![Connect CLOUD\_LONG to S3 bucket][connect-cloud_long-to-s3-bucket]
...
3. **Optimize the data to synchronize in hypertables**
   ...
   {C.CONSOLE} checks the file schema and, if possible, suggests the column to use as the time dimension in a [{C.HYPERTABLE}][hypertables-section].
   ...
   4. Choose the polling interval. This can be a minute, an hour, or a [cron expression][cron-expression].
...
4. **Monitor synchronization**
   ...
   1. To view the amount of data replicated, click `Connectors`.
      ...
      ![CLOUD\_LONG connectors overview][cloud_long-overview]
