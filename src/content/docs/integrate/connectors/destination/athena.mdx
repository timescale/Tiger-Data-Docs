---
title: Query TigerLake S3 Tables with AWS Glue and Athena
description: "Expose Iceberg tables written by TigerLake to the AWS Glue Data Catalog and query them with Amazon Athena, with no ETL or data duplication."
products: [cloud]
...
After {C.LAKE_LONG} syncs your {C.SERVICE_LONG} data to Iceberg tables in Amazon S3 Tables, this guide shows you how to make those tables discoverable through the AWS Glue Data Catalog and query them with Amazon Athena.

This integration does not change the {C.LAKE_SHORT}'s write path and does not use the {C.LAKE_SHORT} generic REST catalog configuration. The {C.LAKE_SHORT} continues to write directly to the Amazon S3 Tables Iceberg REST endpoint using AWS SigV4. AWS Glue federation makes those existing S3 Tables resources discoverable to Athena.

AWS documents each service separately. This guide provides the {C.LAKE_SHORT}-specific resource mapping and the minimum sequence needed to expose a namespace and tables created by the {C.LAKE_SHORT} through the federated catalog.
...
The S3 table bucket owner performs the steps in this guide in the AWS account and Region that contain the table bucket. Enabling Glue federation is an account-and-Region-level operation, not an {C.LAKE_SHORT} operation.
...
- [{C.LAKE_LONG}](/integrate/connectors/destination/tigerlake) set up and active in your {C.SERVICE_LONG}, writing to an AWS S3 Tables destination.
...
<NumberedList>
  <NumberedItem title={`Verify that the ${C.LAKE_SHORT} created the S3 Tables resources`}>
    ...
    The {C.LAKE_SHORT} normally creates the `timescaledb` namespace. List its tables:
    ...
    Do not continue until the expected tables created by the {C.LAKE_SHORT} appear here. Glue and Athena cannot expose a table that the {C.LAKE_SHORT} has not yet created.
  ...
  <NumberedItem title="Grant Lake Formation access">
    ...
    Grant visibility on the {C.LAKE_SHORT} namespace:
  ...
  <NumberedItem title="Query with the Athena console">
    ...
    Then query a table created by the {C.LAKE_SHORT}:
...
Run a count query, insert additional rows into the source TimescaleDB table, wait for the {C.LAKE_SHORT} to commit a new Iceberg snapshot, and run the count query again. An increased count confirms the complete path:
...
- It has `DESCRIBE` on the {C.LAKE_SHORT} database.
...
- Grant `SELECT` and `DESCRIBE`; avoid Athena write or DDL permissions on tables managed by the {C.LAKE_SHORT}.
