...
[Snowflake](https://www.snowflake.com/) is a cloud data platform for data warehousing, analytics, and data sharing. This page shows you how to configure Snowflake to query data from your {C.SERVICE_LONG} using {C.LAKE_LONG} — no ETL pipelines or data duplication required. Because Snowflake reads directly from Amazon S3 Tables in your AWS account, your data stays in one place and is always current.
...
- {C.LAKE_LONG} set up and active in your {C.SERVICE_LONG}. Follow the [Tiger Lake guide](/integrate/connectors/destination/tigerlake) to create one. Note your `S3TableBucketArn` from the CloudFormation stack outputs — you need it throughout this guide.
...
<NumberedList>
  <NumberedItem title="Find your namespace">
    ...
    | --- | --- | --- |
    | `<YOUR_AWS_REGION>` | AWS region where your S3 Table Bucket was created | Your `S3TableBucketArn` — the segment after `s3tables:`, for example `us-east-1` |
    | `<YOUR_AWS_ACCOUNT_ID>` | Your 12-digit AWS account number | Your `S3TableBucketArn`, or the `ownerAccountId` field above |
    | `<YOUR_BUCKET_NAME>` | Name of your S3 Table Bucket | Your `S3TableBucketArn` — the name after `bucket/` |
  ...
  <NumberedItem title="Get Snowflake's AWS identity">
    ...
    From the output, save these two values — you need them in the next section:
...
<NumberedList>
  ...
  <NumberedItem title="Query a registered table">
    ...
    The real value is joining time-series data from {C.CLOUD_LONG} with reference data that lives natively in Snowflake — no pipeline required. For example, given a `customers` table already in Snowflake and a `sensor_readings` {C.HYPERTABLE} synced from {C.CLOUD_LONG}:
