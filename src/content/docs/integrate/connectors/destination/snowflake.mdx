---
title: Query Tiger Lake S3 Tables from Snowflake
...
- [{C.ICEBERG_CONNECTOR_LONG}](/integrate/connectors/destination/tigerlake) set up and active in your {C.SERVICE_LONG}, writing to an AWS S3 Tables destination. Note your `S3TableBucketArn` from the CloudFormation stack outputs. You need it throughout this guide.
...
<NumberedList>
  <NumberedItem title="Find your namespace">
    ...
    Run the following command, replacing `<YOUR_S3_TABLE_BUCKET_ARN>` and `<YOUR_AWS_REGION>` with the values from your {C.ICEBERG_CONNECTOR_LONG} setup:
...
Create a dedicated IAM role so Snowflake can authenticate to your S3 Table Bucket. A separate role keeps the {C.ICEBERG_CONNECTOR_LONG} write path isolated. Snowflake configuration changes can never affect the sync, and access can be revoked independently.
...
Each table that {C.ICEBERG_CONNECTOR_LONG} has synced must be registered once in Snowflake before it can be queried.
...
<NumberedList>
  ...
  <NumberedItem title="List the synced tables">
    ...
    Run the following command to see which tables {C.ICEBERG_CONNECTOR_LONG} has written to your bucket:
  ...
  <NumberedItem title="Register each table">
    ...
    Each time {C.ICEBERG_CONNECTOR_LONG} syncs a new table, run this statement to make it queryable in Snowflake.
...
- **Table not visible after `list-tables`:** {C.ICEBERG_CONNECTOR_LONG} has not completed a full sync cycle yet. Check connector status in {C.CONSOLE}.
- **Stale data in Snowflake:** Run `ALTER CATALOG INTEGRATION tiger_s3tables_catalog REFRESH` to force a metadata refresh.
- **{C.ICEBERG_CONNECTOR_LONG} write path failing:** The {C.ICEBERG_CONNECTOR_LONG} IAM role and the `snowflake-s3tables-reader` role are completely separate. Check that you have not modified the {C.ICEBERG_CONNECTOR_LONG} IAM role.

## Limitations

- Snowflake has read-only access to your S3 Tables data. Writing back to S3 via Snowflake is not supported.
- Tables added by {C.ICEBERG_CONNECTOR_LONG} after initial setup are not automatically visible in Snowflake. Run `CREATE ICEBERG TABLE` for each new table.
