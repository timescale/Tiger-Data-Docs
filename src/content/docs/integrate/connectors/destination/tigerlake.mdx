...
running in {C.SERVICE_LONG}s to Iceberg tables running in [Amazon S3 Tables][s3-tables] in your AWS account.
...
<Tabs>
  <Tab title="AWS Management Console">
    ...
    2. **Create your CloudFormation stack**
       ...
       3. In `Specify stack details`, enter the following details, then click `Next`:
          ...
          - `ProjectID` and `ServiceID`: enter the [connection details][get-project-id] for your {C.LAKE_LONG} {C.SERVICE_SHORT}
    ...
    3. **Connect your {C.SERVICE_SHORT} to the data lake**
       ...
       1. In [{C.CONSOLE}][services-portal], select the {C.SERVICE_SHORT} you want to integrate with AWS S3 Tables, then click
          `Connectors`.
  ...
  <Tab title="AWS CloudFormation CLI">
    1. **Create your CloudFormation stack**
       ...
       - `ProjectID`: enter your {C.SERVICE_LONG} [connection details][get-project-id]
       - `ServiceID`: enter your {C.SERVICE_LONG} [connection details][get-project-id]
  ...
  <Tab title="Manual configuration">
    ...
    2. **Create an ARN role**
       ...
       3. Replace `<ProjectID>` and `<ServiceID>` with the the [connection details][get-project-id] for your {C.LAKE_LONG}
...
- `tigerlake.iceberg_partitionby`: optional property to define a partition specification in Iceberg. By default the
  ...
  to {C.HYPERTABLE}s. For more information, see [partitioning][partitioning].
...
[tigerlake.iceberg\_partitionby][samples]. The following partition intervals and specifications are supported:
...
- In order to collect deletes made to data in the columstore, certain columnstore optimizations are disabled for {C.HYPERTABLE}s, this includes [Direct Compress][direct-compress].
- The `TRUNCATE` statement is not supported, and does not truncate data in the corresponding Iceberg table.
- Data in a {C.HYPERTABLE} that has been moved to the [low-cost object storage tier][data-tiering] is not synced.
...
[get-project-id]: /integrate/find-connection-details#find-your-project-and-service-id
...
[partitioning]: /integrate/connectors/destination/tigerlake#partitioning-intervals

[s3-console]: https://console.aws.amazon.com/s3/

[s3-tables]: https://aws.amazon.com/s3/features/tables/

[samples]: /integrate/connectors/destination/tigerlake#sample-code
