...
- In order to collect deletes made to data in the columstore, certain columnstore optimizations are disabled for {C.HYPERTABLE}s, this includes [Direct Compress](/learn/hypertables/understand-hypertables/#speed-up-data-ingestion).
- The `TRUNCATE` statement is not supported, and does not truncate data in the corresponding Iceberg table.
- Data in a {C.HYPERTABLE} that has been moved to the [low-cost object storage tier](/learn/data-lifecycle/tiered-storage/) is not synced.
