...
1. Create a bastion host that can reach the source database, for example an EC2 instance with an Elastic IP.
2. Allow-list the Elastic IP on the source firewall or security group.
3. In the [connector wizard](/integrate/connectors/source/sync-from-postgres), toggle `Enable SSH tunneling` and enter the bastion host details.
