...
[{C.HYPERTABLE} documentation](/build/data-management/understand-hypertables).

You may also wish to consider taking advantage of some of {C.CLOUD_LONG}'s killer
features, such as:
- [Retention policies](/build/data-management/data-retention) to automatically drop unneeded data
- [Tiered storage](/build/data-management/storage) to automatically move data to {C.CLOUD_LONG}'s low-cost bottomless object storage tier
- [{C.HYPERCORE_CAP}](/build/columnar-storage) to reduce the size of your {C.HYPERTABLE}s by compressing data in the {C.COLUMNSTORE}
- [{C.CAGG_CAP}s](/build/continuous-aggregates) to write blisteringly fast aggregate queries on your data
