---
title: TimescaleDB code style guide
description: Source code should follow the
---
...
[{C.PG} coding conventions](https://www.postgresql.org/docs/current/static/source.html). This
includes
...
There is no official SQL or PL/pgSQL style guide for {C.PG} that
...
clarity and convenience they provide. {C.PG} sticks with C89
...
Unfortunately, {C.PG} does not have a consistent style for naming
...
a consistent style that roughly follows the style of the {C.PG}
source code.
...
* Prefer static allocation and/or stack-allocated variables over
  heap/dynamic allocation when possible. Fortunately, {C.PG} has a
...
For a general guide to writing C functions in {C.PG}, you can
review the section on
[C-language functions](https://www.postgresql.org/docs/current/static/xfunc-c.html)
in the {C.PG} documentation.
...
(which is similar to the {C.PG} style but implement in clang-format).
