...
<SinceRelease mode="deprecated" version="2.13.0" product="tsdb">. Use the [new interface][api-create-hypertable].</SinceRelease><br />
<SinceRelease version="1.0.0" product="tsdb" />


Creates a {C.TIMESCALE_DB} {C.HYPERTABLE} from a {C.PG} table (replacing the latter),
partitioned on time and with the option to partition on one or more other
columns. The {C.PG} table cannot be an already partitioned table
(declarative partitioning or inheritance). In case of a non-empty table, it is
possible to migrate the data during {C.HYPERTABLE} creation using the `migrate_data`
...
on the resulting {C.HYPERTABLE}.

For more information about using {C.HYPERTABLE}s, including chunk size partitioning,
see the [hypertable documentation][hypertable-docs].

## Samples

Convert table `conditions` to {C.HYPERTABLE} with just time partitioning on column `time`:

```sql
SELECT create_hypertable('conditions', 'time');
```

Convert table `conditions` to {C.HYPERTABLE}, setting `chunk_time_interval` to 24 hours.
...
Convert table `conditions` to {C.HYPERTABLE}. Do not raise a warning
if `conditions` is already a {C.HYPERTABLE}:
...
|-|-|-|-|-|
|`relation`|REGCLASS| - | ✔ | Identifier of table to convert to {C.HYPERTABLE}.|
...
|`if_not_exists`|BOOLEAN| `false` | ✖ | Whether to print warning if table already converted to {C.HYPERTABLE} or raise exception. Default is FALSE.|
|`partitioning_func`|REGCLASS| - | ✖ | The function to use for calculating a value's partition.|
|`associated_schema_name`|REGCLASS| `_timescaledb_internal` | ✖ | Name of the schema for internal {C.HYPERTABLE} tables. Default is `_timescaledb_internal`.|
|`associated_table_prefix`|TEXT| `_hyper` | ✖ | Prefix for internal {C.HYPERTABLE} chunk names. Default is `_hyper`.|
|`migrate_data`|BOOLEAN| `false` | ✖ | Set to TRUE to migrate any existing data from the `relation` table to chunks in the new {C.HYPERTABLE}. A non-empty table generates an error without this option. Large tables may take significant time to migrate. Defaults to FALSE.|
...
|`replication_factor`|INTEGER| - | ✖ | Replication factor to use with distributed {C.HYPERTABLE}. If not provided, value is determined by the `timescaledb.hypertable_replication_factor_default` GUC. |
|`data_nodes`|ARRAY| - | ✖ | This is the set of data nodes that are used for this table if it is distributed. This has no impact on non-distributed {C.HYPERTABLE}s. If no data nodes are specified, a distributed {C.HYPERTABLE} uses all data nodes known by this instance.|
|`distributed`|BOOLEAN| `NULL` | ✖ | Set to TRUE to create distributed {C.HYPERTABLE}. If not provided, value is determined by the `timescaledb.hypertable_distributed_default` GUC. When creating a distributed {C.HYPERTABLE}, consider using [`create_distributed_hypertable`][create_distributed_hypertable] in place of `create_hypertable`. Default is NULL. |
...
|`schema_name`|TEXT|Schema name of the table converted to {C.HYPERTABLE}.|
|`table_name`|TEXT|Table name of the table converted to {C.HYPERTABLE}.|
|`created`|BOOLEAN|TRUE if the {C.HYPERTABLE} was created, FALSE when `if_not_exists` is true and no {C.HYPERTABLE} was created.|
...
| Error | Description |
|-|-|
| {C.HYPERTABLE} "\{table_name}" not found | The specified table does not exist |
| permission denied for schema "\{schema_name}" | Insufficient permissions to access the schema |
| must be owner of {C.HYPERTABLE} "\{table_name}" | Only the table owner can convert it to a {C.HYPERTABLE} |
| permissions denied: cannot create {C.CHUNK}s in schema "\{schema_name}" | Insufficient permissions on the associated schema for {C.CHUNK}s |
| table "\{table_name}" is already a {C.HYPERTABLE} | The table has already been converted to a {C.HYPERTABLE} |
| table "\{table_name}" is not empty | The table contains data and `migrate_data` is not set to true |
...
When converting a normal SQL table to a {C.HYPERTABLE}, pay attention to how you handle
constraints. A {C.HYPERTABLE} can contain foreign keys to normal SQL table columns,
...
[{C.PG} documentation][pg-documentation]
for the syntax.
...
function is used. The default partitioning function calls {C.PG}'s internal
hash function for the given type, if one exists. Thus, a custom partitioning
function can be used for value types that do not have a native {C.PG} hash
...
[api-create-hypertable]: /reference/timescaledb/hypertables/create_hypertable

[create_distributed_hypertable]: /reference/timescaledb/distributed-hypertables/create_distributed_hypertable

[hypertable-docs]: /manage-data/capabilities/hypertables/understand-hypertables
