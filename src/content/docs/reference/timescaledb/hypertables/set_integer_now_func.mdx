...
<TagIcon iconType="duotone" /> Since [1.5.0][tsdb-1.5.0]
...
[{C.CHUNK}s][chunks] of a certain age. `integer_now_func` determines the age of each {C.CHUNK}.
...
- `STABLE`: `integer_now_func` is evaluated just before query execution starts.
  [{C.CHUNK} pruning][chunk-pruning] is executed at runtime. This generates a correct result, but may increase
  planning time.
...
Set the integer `now` function for a {C.HYPERTABLE} with a time column in [unix time][unix-time].
