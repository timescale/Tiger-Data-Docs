...
set the current time in the integer `time` column in a {C.HYPERTABLE}. Many policies only apply to
[{C.CHUNK}s](/build/data-management/understand-hypertables) of a certain age. `integer_now_func` determines the age of each {C.CHUNK}.
...
- `IMMUTABLE`: Use when you execute the query each time rather than prepare it prior to execution. The value
  ...
  plan, especially if you have a lot of {C.CHUNK}s.

- `STABLE`: `integer_now_func` is evaluated just before query execution starts.
  [{C.CHUNK} pruning](https://www.timescale.com/blog/optimizing-queries-timescaledb-hypertables-with-partitions-postgresql-6366873a995d) is executed at runtime. This generates a correct result, but may increase
  planning time.
...
Set the integer `now` function for a {C.HYPERTABLE} with a time column in [unix time](https://en.wikipedia.org/wiki/Unix_time).
...
|-|-|-|-|-|
|`hypertable`|REGCLASS| - | ✔ | The {C.HYPERTABLE} `integer_now_func` is used in. |
|`integer_now_func`|REGPROC| - | ✔ | A function that returns the current time set in each row in the `time` column in the {C.HYPERTABLE}.|
