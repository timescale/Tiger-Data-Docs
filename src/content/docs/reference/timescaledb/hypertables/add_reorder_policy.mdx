...
24 hours. To change the schedule, call [alter\_job](/reference/timescaledb/jobs-automation/alter_job) and adjust `schedule_interval`.

You can have only one reorder policy on each {C.HYPERTABLE}.

For manual reordering of individual {C.CHUNK}s, see [reorder\_chunk](/reference/timescaledb/hypertables/reorder_chunk).
...
<Callout variant="note">
  ...
  already been reordered, re-run [reorder\_chunk](/reference/timescaledb/hypertables/reorder_chunk) on them. If you have changed a lot of older {C.CHUNK}s, it
