...
<SinceRelease mode="deprecated" version="2.13.0" product="tsdb">. Use [add_dimension()](/reference/timescaledb/hypertables/add_dimension).</SinceRelease><br />
<SinceRelease version="1.0.0" product="tsdb" />

Add an additional partitioning dimension to a {C.TIMESCALE_DB} {C.HYPERTABLE}.
...
<Callout variant="warning">
  ...
  converted to a {C.HYPERTABLE} (via `create_hypertable`), but must similarly
  be run only on an empty {C.HYPERTABLE}.
</Callout>

**Space partitions**: Using space partitions is highly recommended
for distributed {C.HYPERTABLE}s to achieve
efficient scale-out performance. For regular {C.HYPERTABLE}s
...
intervals to manage {C.CHUNK} sizes; the main purpose of space
partitioning is to enable parallelization across multiple
data nodes (in the case of distributed {C.HYPERTABLE}s) or
across multiple disks within the same time interval
(in the case of single-node deployments).

## Samples

First convert table `conditions` to {C.HYPERTABLE} with just time
...
Convert table `conditions` to {C.HYPERTABLE} with time partitioning on `time` and
...
Now in a multi-node example for distributed {C.HYPERTABLE}s with a cluster
...
a distributed {C.HYPERTABLE} with just time partitioning on column `time`,
...
In a distributed {C.HYPERTABLE}, space partitioning enables inserts to be
...
1. Use a RAID setup across multiple physical disks, and expose a
   single logical disk to the {C.HYPERTABLE} (that is, via a single tablespace).

2. For each physical disk, add a separate tablespace to the
   database. {C.TIMESCALE_DB} allows you to actually add multiple tablespaces
   to a *single* {C.HYPERTABLE} (although under the covers, a {C.HYPERTABLE}'s
   {C.CHUNK}s are spread across the tablespaces associated with that {C.HYPERTABLE}).
...
{C.TIMESCALE_DB} does *not* benefit from a very large number of space
...
|-|-|-|-|-|
|`hypertable`|REGCLASS| - | ✔ | {C.HYPERTABLE_CAP} to add the dimension to|
...
|`chunk_time_interval`|INTERVAL| - | ✖ | Interval that each {C.CHUNK} covers. Must be > 0|
|`partitioning_func`|REGCLASS| - | ✖ | The function to use for calculating a value's partition (see `create_hypertable` [instructions](/reference/timescaledb/hypertables/create_hypertable_old))|
...
|`schema_name`|TEXT|Schema name of the {C.HYPERTABLE}|
|`table_name`|TEXT|Table name of the {C.HYPERTABLE}|
