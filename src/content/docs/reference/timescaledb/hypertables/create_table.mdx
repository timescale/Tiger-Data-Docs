...
WITH (
   ...
   tsdb.direct_compress_schedule_interval = '<interval>'
)
```
...
| `tsdb.direct_compress_schedule_interval` |INTERVAL| `1 minute`                                                                                                                                                                                                                                                                                        | ✖                                                           | Set the schedule interval for the [compaction policy](/reference/timescaledb/hypercore/add_compaction_policy) that `tsdb.direct_compress` creates, which defragments the small batches direct compress writes. Requires `tsdb.direct_compress` to also be set in the same statement; setting it without `direct_compress` raises an error. |
...
| `functionality not supported under the current "apache" license` | Feature requires a {C.TIMESCALE_DB} license with additional capabilities. To check your edition, see [Compare {C.TIMESCALE_DB} editions](/get-started/choose-your-path/timescaledb-editions#check-your-installed-edition). |
