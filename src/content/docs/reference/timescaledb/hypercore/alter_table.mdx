...
ALTER TABLE <table_name> SET (timescaledb.enable_columnstore,
   ...
   timescaledb.direct_compress_schedule_interval = '<interval>',
...
| `timescaledb.direct_compress_schedule_interval` | INTERVAL | `1 minute`                                                                                                                                                                                                                          | ✖        | Set the schedule interval for the [compaction policy](/reference/timescaledb/hypercore/add_compaction_policy) that `timescaledb.direct_compress` creates, which defragments the small batches direct compress writes. Requires `timescaledb.direct_compress` to also be set in the same statement; setting it without `direct_compress` raises an error. |
