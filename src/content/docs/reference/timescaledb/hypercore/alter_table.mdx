...
- [add\_columnstore\_policy][add_columnstore_policy]: create a [job][job] that automatically moves {C.CHUNK}s in a
  {C.HYPERTABLE} to the {C.COLUMNSTORE} at a
  specific time interval.
- [convert\_to\_columnstore][convert_to_columnstore]: manually add a specific {C.CHUNK} in a {C.HYPERTABLE} to the
  {C.COLUMNSTORE}.
...
For standard `ALTER TABLE` return behavior, see the [{C.PG} ALTER TABLE documentation][postgres-alter-table].
...
[create-hypertable]: /reference/timescaledb/hypertables/create_hypertable
