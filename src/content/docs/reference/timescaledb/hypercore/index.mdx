...
1. **Enable {C.COLUMNSTORE} on a {HYPERTABLE}**
...
{CHUNK}s in the {C.COLUMNSTORE} have the following limitations:

- `ROW LEVEL SECURITY` is not supported on {CHUNK}s in the {C.COLUMNSTORE}.
...
- [`remove_columnstore_policy()`][remove_columnstore_policy]: remove a {C.COLUMNSTORE} policy from a {HYPERTABLE}

### Configuration

- [`ALTER TABLE (hypercore)`][alter_table_hypercore]: enable the {C.COLUMNSTORE} for a {HYPERTABLE}

### Manual conversion

- [`convert_to_columnstore()`][convert_to_columnstore]: manually add a {CHUNK} to the {C.COLUMNSTORE}
- [`convert_to_rowstore()`][convert_to_rowstore]: move a {CHUNK} from the {C.COLUMNSTORE} to the {ROWSTORE}

### Statistics and information

- [`chunk_columnstore_stats()`][chunk_columnstore_stats]: get statistics about {CHUNK}s in the {C.COLUMNSTORE}
