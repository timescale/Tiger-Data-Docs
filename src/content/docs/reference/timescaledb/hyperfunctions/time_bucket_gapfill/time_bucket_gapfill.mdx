...
keywords: [hyperfunctions, time bucket gapfill, time_bucket_gapfill, timescaledb]
---
...
3. **Fill gaps between the earliest and latest rows in the table**

   In {C.TIMESCALE_DB} 2.28 and later, the gapfill boundaries can come from subquery results instead of constants. For example, use the table's own minimum and maximum timestamps as the gapfill range:
   ...
   SELECT
     ...
     locf(AVG(temperature)) AS locf_temp
   FROM sensor_data
   WHERE time >= (SELECT min(time) FROM sensor_data)
     AND time < (SELECT max(time) FROM sensor_data)
   GROUP BY hour, sensor_id
   ORDER BY sensor_id, hour;
   ```

   The subqueries must be uncorrelated: a subquery that references a column of the outer query cannot be used to infer the gapfill boundaries.

4. **Use a different bucket width for each group**

   In {C.TIMESCALE_DB} 2.28 and later, `bucket_width` can also come from a subquery. Combine this with a `LATERAL` join to gapfill each sensor at its own resolution:

   ```sql
   SELECT s.sensor_id, bucket, locf_temp
   FROM (VALUES (1, '15 minutes'::interval),
                (2, '1 hour'::interval)) s (sensor_id, bucket_width)
   INNER JOIN LATERAL (
     SELECT
       time_bucket_gapfill(s.bucket_width, time) AS bucket,
       locf(AVG(temperature)) AS locf_temp
     FROM sensor_data
     WHERE sensor_id = s.sensor_id
       AND time >= (SELECT min(time) FROM sensor_data)
       AND time < (SELECT max(time) FROM sensor_data)
     GROUP BY bucket
   ) sq ON true
   ORDER BY s.sensor_id, bucket;
   ```

5. **Filter gapfilled results**

   To filter the gapfilled rows, wrap the gapfill query in a common table expression and filter its results. For example, to return only the hours with no readings:
   ...
   WITH gapfilled AS (
     ...
     FROM sensor_data
     WHERE time >= '2025-12-10 09:00:00+00'
       AND time < '2025-12-10 14:00:00+00'
     GROUP BY hour, sensor_id
   )
   SELECT * FROM gapfilled
   ...
   | hour | sensor\_id | reading\_count |
   | --- | --- | --- |
   | 2025-12-10 10:00:00+00 | 1 | |
   | 2025-12-10 12:00:00+00 | 1 | |

   <Callout variant="warning">
     Do not use `HAVING` to filter gapfilled rows. `HAVING` is applied before gapfilling runs, so it filters the aggregation results instead. Do not wrap the gapfill query in a subquery either, because the query planner often optimizes the subquery away.
   </Callout>
...
|--|--|--|--|--|
| `bucket_width` | INTERVAL | INTEGER | - | ✔ | A {C.PG} time interval to specify the length of each bucket. For example, use `1 day` to get daily buckets. Use `INTEGER` only if your time column is integer-based. |
| `ts` | TIMESTAMPTZ | TIMESTAMP | DATE | SMALLINT | INT | BIGINT | - | ✔ | The timestamp on which to base the bucket |
| `timezone` | TEXT | - | - | The timezone to use for bucketing. For example, `Europe/Berlin`. Available in {C.TIMESCALE_DB} 2.9 or later. Does not work for integer-based time. If you have an untyped `start` or `finish` argument and a `timezone` argument, you might run into a problem where you are not passing your arguments for the parameter that you expect. To solve this, either name your arguments or explicitly type cast them. |
| `start` | TIMESTAMPTZ | INTEGER | - | - | The start of the period to gapfill. Values before `start` are passed through, but no gapfilling is performed. Use `INTEGER` only if your time column is integer-based. Best practice is to use the `WHERE` clause. Specifying `start` is legacy. The `WHERE` is more performant, because the query planner can filter out {C.CHUNK}s by constraint exclusion. |
