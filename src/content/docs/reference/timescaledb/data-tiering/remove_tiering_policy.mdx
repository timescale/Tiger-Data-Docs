...
Remove the tiering policy from a {C.HYPERTABLE} or {C.CAGG}. The underlying {C.TIMESCALE_DB} background job is deleted, so no further {C.CHUNK}s are tiered automatically.

{C.CHUNK_CAP}s that the policy had already frozen and queued for upload but had not yet shipped to object storage are unfrozen and removed from the tiering queue. {C.CHUNK_CAP}s that have already been uploaded to object storage remain tiered; use [`untier_chunk`](/reference/timescaledb/data-tiering/untier_chunk) to bring them back.
