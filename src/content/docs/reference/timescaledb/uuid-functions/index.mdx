...
UUIDv7 also increases query speed by reducing the number of {C.CHUNK}s scanned during queries. For example, in a database
...
Using UUIDv7 means that {C.CHUNK}s are excluded at startup, and the query time is reduced to 550 ms:
...
- [`generate_uuidv7()`](/reference/timescaledb/uuid-functions/generate_uuidv7): generate a version 7 UUID based on the current time
- [`to_uuidv7()`](/reference/timescaledb/uuid-functions/to_uuidv7): create a version 7 UUID from a {C.PG} timestamp
- [`to_uuidv7_boundary()`](/reference/timescaledb/uuid-functions/to_uuidv7_boundary): create a version 7 "boundary" UUID from a {C.PG} timestamp
- [`uuid_timestamp()`](/reference/timescaledb/uuid-functions/uuid_timestamp): extract a {C.PG} timestamp from a version 7 UUID
- [`uuid_timestamp_micros()`](/reference/timescaledb/uuid-functions/uuid_timestamp_micros): extract a {C.PG} timestamp with microsecond precision from a
  version 7 UUID
- [`uuid_version()`](/reference/timescaledb/uuid-functions/uuid_version): extract the version of a UUID
