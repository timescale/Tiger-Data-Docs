...
To help when asking for support and reporting bugs, {C.TIMESCALE_DB} includes an SQL dump script. It outputs metadata from
the internal {C.TIMESCALE_DB} tables, along with version information.
...
- [`get_telemetry_report()`](/reference/timescaledb/administration/get_telemetry_report): view the background telemetry string sent to Timescale
- [`timescaledb_post_restore()`](/reference/timescaledb/administration/timescaledb_post_restore): perform required operations after finishing a database
  restore
- [`timescaledb_pre_restore()`](/reference/timescaledb/administration/timescaledb_pre_restore): prepare the database for a restore operation
