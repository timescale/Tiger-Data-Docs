...
Create a policy that automatically refreshes a {C.CAGG}. To view the
...
|-|-|-|-|-|
| `continuous_aggregate` | REGCLASS | - | ✔ | The {C.CAGG} to add the policy for |
| `start_offset` | INTERVAL or integer | - | ✔ | Start of the refresh window as an interval relative to the time when the policy is executed. `NULL` is equivalent to `MIN(timestamp)` of the {C.HYPERTABLE}. |
| `end_offset` | INTERVAL or integer | - | ✔ | End of the refresh window as an interval relative to the time when the policy is executed. `NULL` is equivalent to `MAX(timestamp)` of the {C.HYPERTABLE}. |
...
| `buckets_per_batch` | INTEGER | 10 | - | Number of buckets to be refreshed by a batch. This value is multiplied by the {C.CAGG} bucket width to determine the size of the batch range. Default value is `10`. Values of less than `0` are not allowed. |
...
depending on the type of the time column of the {C.HYPERTABLE}:

- For {C.HYPERTABLE}s with `TIMESTAMP`, `TIMESTAMPTZ`, and `DATE` time columns,
  set the offset as an `INTERVAL` type.
- For {C.HYPERTABLE}s with integer-based timestamps, set the offset as an
  `INTEGER` type.

<Callout variant="info">
  While setting `end_offset` to `NULL` is possible, it is not recommended. To include the data between `end_offset` and
  the current time in queries, enable [real-time aggregation](/build/continuous-aggregates/real-time-aggregates).
</Callout>

You can add [concurrent refresh policies](/build/continuous-aggregates/refresh-policies) on each {C.CAGG}, as long as the `start_offset` and `end_offset` does not overlap with another policy on the same {C.CAGG}.

<Callout variant="info">
  Setting `buckets_per_batch` greater than zero means that the refresh window is split in batches of `bucket width` \*
  `buckets per batch`. For example, a given {C.CAGG} with `bucket width` of `1 day` and `buckets_per_batch` of 10 has a
