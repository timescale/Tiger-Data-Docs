...
<Callout variant="info">
  ...
  the current time in queries, enable [real-time aggregation][real-time-aggregates].
</Callout>

You can add [concurrent refresh policies][refresh-policies] on each {C.CAGG}, as long as the `start_offset` and `end_offset` does not overlap with another policy on the same {C.CAGG}.
...
[refresh-policies]: /use-timescale/latest/continuous-aggregates/refresh-policies/

[utc-bucketing]: https://docs.tigerdata.com/use-timescale/latest/time-buckets/about-time-buckets/
