...
Match the type of `window_start` and `window_end` to the time column of the
{C.HYPERTABLE}. For {C.HYPERTABLE}s with `TIMESTAMP`, `TIMESTAMPTZ`, or `DATE`
time columns, pass either an absolute timestamp or date, or an `INTERVAL`. An
`INTERVAL` is interpreted relative to the current transaction time, so
`window_start => INTERVAL '1 month'` means one month before now. For
{C.HYPERTABLE}s with integer-based time columns, pass an `INTEGER`.
