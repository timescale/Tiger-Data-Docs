...
- **Convert {C.CAGG} {C.CHUNK}s to the {C.COLUMNSTORE} after each refresh**:
  ...
  See [Convert refreshed chunks to the columnstore during refresh](/build/continuous-aggregates/compression-on-continuous-aggregates#convert-refreshed-chunks-to-the-columnstore-during-refresh).

  ```sql
  -- Find the job ID of the refresh policy:
