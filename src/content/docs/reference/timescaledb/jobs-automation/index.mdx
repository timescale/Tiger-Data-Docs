...
{C.JOB_CAP}s allow you to run functions and procedures implemented in a
...
The following APIs and views allow you to manage the {C.JOB}s that you create and
get details around automatic {C.JOB}s used by other {C.TIMESCALE_DB} functions like
{C.CAGG} refresh policies and data retention policies. To view the
...
- [`add_job()`](/reference/timescaledb/jobs-automation/add_job): add a job to run a function or procedure automatically
- [`alter_job()`](/reference/timescaledb/jobs-automation/alter_job): alter a job that is scheduled to run automatically
- [`delete_job()`](/reference/timescaledb/jobs-automation/delete_job): delete a job from the automatic scheduler
- [`run_job()`](/reference/timescaledb/jobs-automation/run_job): manually run a job
