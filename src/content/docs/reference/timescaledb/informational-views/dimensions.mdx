...
Returns information about the dimensions of a {C.HYPERTABLE}. {C.HYPERTABLE}s can be
partitioned on a range of different dimensions. By default, all {C.HYPERTABLE}s are
partitioned on time, but it is also possible to partition on other dimensions in
addition to time.

For {C.HYPERTABLE}s that are partitioned solely on time,
`timescaledb_information.dimensions` returns a single row of metadata. For
{C.HYPERTABLE}s that are partitioned on more than one dimension, the call returns a
...
If the {C.HYPERTABLE} uses time data types, the `time_interval` column is defined.
Alternatively, if the {C.HYPERTABLE} uses integer data types, the `integer_interval`
and `integer_now_func` columns are defined.

## Samples

Get information about the dimensions of {C.HYPERTABLE}s.
...
Get information about dimensions of a {C.HYPERTABLE} that has two time-based dimensions.
...
| `num\_partitions\` | SMALLINT | Number of partitions for the dimension |
