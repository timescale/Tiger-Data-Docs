...
Shows a job associated with the refresh policy for {C.CAGG}s:
...
Find all jobs related to compression policies (before {C.TIMESCALE_DB} v2.20):
...
Find all jobs related to {C.COLUMNSTORE} policies ({C.TIMESCALE_DB} v2.20 and later):
