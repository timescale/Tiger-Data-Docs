...
sidebar:
  label: chunk_compression_settings
...
Shows information about compression settings for each {C.CHUNK} that has compression enabled on it.

## Samples

Show compression settings for all {C.CHUNK}s:
...
Find all {C.CHUNK} compression settings for a specific {C.HYPERTABLE}:
