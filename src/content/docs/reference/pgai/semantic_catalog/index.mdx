...
description: >-
  Text-to-SQL is a natural language processing technology that converts
  questions made in human language into SQL queries
---
...
that you execute in relational databases such as {C.PG}. Text-to-SQL is a tool that enables non-technical users to
...
leverages the {C.PG} planner to **validate the queries written by the LLM**.
...
We do this using {C.PG}’s `EXPLAIN` statement, where we ask the database to return a query plan. That plan gives us:
...
- [I want to create a semantic catalog for my database.](/reference/pgai/semantic_catalog/quickstart-your-data)
- [I want a walkthrough of capabilities on demo data.](/reference/pgai/semantic_catalog/quickstart-demo-data)
- Check out the [CLI reference.](/reference/pgai/semantic_catalog/cli)
