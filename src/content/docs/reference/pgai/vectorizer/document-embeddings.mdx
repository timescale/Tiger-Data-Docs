---
title: Document embeddings in pgai
description: >-
  This is a comprehensive walkthrough of how embedding generation for documents
  work in pgai. If you want to get started quickly check out the runnable
  example.
---
...
This is a comprehensive walkthrough of how embedding generation for documents work in pgai. If you want to get started quickly check out the [runnable example](https://github.com/timescale/pgai/tree/main/examples/embeddings_from_documents).
...
Then you can [query the generated embeddings](#querying-document-embeddings) using the usual pgvector semantic search. 

In this guide, we'll also cover how to [monitor and troubleshoot](#monitoring-and-troubleshooting) your vectorizers.

If you are storing documents in AWS S3, you can use the [S3 documentation](/reference/pgai/vectorizer/s3-documents) to learn more about how to configure S3 for document storage and synchronize your S3 buckets with your document table.
...
If you don't have such a table yet and are storing documents in S3 we have a [guide on how to sync S3 to a document table](/reference/pgai/vectorizer/s3-documents#syncing-s3-to-a-documents-table).
...
A vectorizer is a declarative configuration that defines how documents are processed, chunked, and embedded. pgai's vectorizer system automatically keeps document embeddings in sync with source documents. You can find the reference for vectorizers in the [API Reference documentation](/reference/pgai/vectorizer/api-reference).
...
**Storing documents in AWS S3**: Timescale Cloud and a self-hosted pgai installation support AWS S3 URLs out of the box. Check the [S3 documentation](/reference/pgai/vectorizer/s3-documents) for more information on how to authenticate and configure S3.
...
You can find more information about the parsers in the [parsing reference](/reference/pgai/vectorizer/api-reference#parsing-configuration).
...
For more information about chunking, see the [chunking reference](/reference/pgai/vectorizer/api-reference#chunking-configuration).

#### Embedding

pgai support a wide range of embedding providers. You can find the reference for the embedding providers in the [embedding documentation](/reference/pgai/vectorizer/api-reference#embedding-configuration).
...
Once your vectorizer is created, pgai automatically generates a target table with your embeddings and a view that joins the embeddings with the original document table. We configure the view name in the `ai.destination_table('document_embeddings')` [destination configuration](/reference/pgai/vectorizer/api-reference#destination-configuration). The view contains all the columns from the original document table plus the following additional columns:
...
- Adjust the processing batch size and concurrency explained in the [processing reference](/reference/pgai/vectorizer/api-reference#processing-configuration) in general we recommend a low batch size (e.g. 1) and a high concurrency (e.g. 10) for documents. Since parsing takes some time.
...
- Supported documents depend on the parser that you are using. Check the [parser reference](/reference/pgai/vectorizer/api-reference#parsing-configuration) to see what types of documents are supported by the parser you are using.
