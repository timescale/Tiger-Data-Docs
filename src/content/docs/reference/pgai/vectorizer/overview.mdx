...
description: Vector embeddings have emerged as a powerful tool for transforming text into
---
...
While modern vector databases like {C.PG} excel at storing and querying
...
- Designate any text column for embedding using customizable rules (or, if you are embedding binary documents such as PDFs, you can see our guide for [embedding documents](/reference/pgai/vectorizer/document-embeddings))
...
[Vectorizer quick start](/reference/pgai/vectorizer/quick-start). For a more detailed technical specification, see the
[Vectorizer API reference](/reference/pgai/vectorizer/api-reference).
...
etc.) or self-hosted {C.PG}, you can use the [vectorizer worker](/reference/pgai/vectorizer/worker) to
process your vectorizers.

This guide walks you the steps to configure your vectorizer to embed data stored in text columns. If you are embedding binary documents such as PDFs, see our guide for [embedding documents](/reference/pgai/vectorizer/document-embeddings).
...
Timescale Cloud or on a self-hosted {C.PG} server.

- Timescale Cloud

  1. In [Timescale Console > Project Settings](https://console.cloud.tigerdata.com/dashboard/settings), click `AI Model API Keys`.
...
- Self-hosted {C.PG}
  ...
  Set an environment variable that is the [same as your API key name](/reference/pgai/vectorizer/worker#install-and-configure-vectorizer-worker). 
  For example:
  ```bash
...
consult the [embedding configuration](/reference/pgai/vectorizer/api-reference#embedding-configuration)
...
For more details, check the [loading configuration](/reference/pgai/vectorizer/api-reference#loading-configuration) section 
of the vectorizer API reference or our [guide for embedding documents](/reference/pgai/vectorizer/document-embeddings).
...
every five minutes. If you are self-hosting, you need to [run the vectorizer-worker](/reference/pgai/vectorizer/worker)
...
<Callout variant="tip">
You can use the `ai.ollama_embed` function in our [{C.PG} extension](/reference/pgai) to generate an embedding for a user-provided query right inside the database.
</Callout>
...
Indexing relies on a background job that runs periodically, so this feature will not work if scheduling is disabled (which is the default for self-hosted installations).
</Callout>
...
<Callout variant="note">
When scheduling is disabled, the index is not created automatically. You need to create it manually.
</Callout>
...
<Callout variant="note">
Column destination requires chunking to be set to `ai.chunking_none()` since it can only store one embedding per row.
</Callout>
