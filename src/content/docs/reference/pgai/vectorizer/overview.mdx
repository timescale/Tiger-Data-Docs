...
[Vectorizer quick start](/pgai/vectorizer/quick-start). For a more detailed technical specification, see the
[Vectorizer API reference](/pgai/vectorizer/api-reference).
...
etc.) or self-hosted {C.PG}, you can use the [vectorizer worker](/pgai/vectorizer/worker) to
...
- Self-hosted {C.PG}
  ...
  Set an environment variable that is the [same as your API key name](/pgai/vectorizer/worker.md#install-and-configure-vectorizer-worker). 
  For example:
  ```bash
...
consult the [embedding configuration](/pgai/vectorizer/api-reference.md#embedding-configuration)
...
For more details, check the [loading configuration](/pgai/vectorizer/api-reference.md#loading-configuration) section 
...
every five minutes. If you are self-hosting, you need to [run the vectorizer-worker](/pgai/vectorizer/worker)
...
<Callout variant="tip">
You can use the `ai.ollama_embed` function in our [{C.PG} extension](/projects/extension/README) to generate an embedding for a user-provided query right inside the database.
</Callout>
