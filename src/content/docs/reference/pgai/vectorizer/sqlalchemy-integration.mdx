---
title: SQLAlchemy integration with pgai Vectorizer
description: >-
  When creating vectorizers that use the ai.destinationtable option, the
  vectorizer will create a new table in the database to store the vector
  embeddings. This guide describes how to integrate this ...
---
...
Note if you work with alembics autogenerate functionality for migrations, also check the [Alembic integration guide](/reference/pgai/vectorizer/alembic-integration).
...
We recommend adding this to a migration script and run it via alembic (see our [alembic integration docs](/reference/pgai/vectorizer/alembic-integration) for more details).
