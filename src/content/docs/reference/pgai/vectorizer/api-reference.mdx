...
description: This page provides an API reference for Vectorizer functions. For an overview
---
...
of Vectorizer and how it works, see the [Vectorizer Guide](/reference/pgai/vectorizer/overview).

A vectorizer provides you with a powerful and automated way to generate and 
manage LLM embeddings for your {C.PG} data. Here's a summary of what you 
...
semantic search and analysis capabilities into existing {C.PG} databases.  
...
<Callout variant="note">
Mistral limits the maximum input per batch to 16384 tokens.
</Callout>
...
![Azure AI Foundry console example](https://raw.githubusercontent.com/timescale/pgai/main/docs/images/azure_openai.png)
...
<Callout variant="tip">
We recommend using the `wait_for_model` parameter when using vectorizer with serverless inference to force the call to block until the model has been loaded. If you do not use `wait_for_model`, it's likely that vectorization will never succeed.
</Callout>
...
<Callout variant="note">
`VERTEX_CREDENTIALS` should contain the path to a file containing the API key; the vectorizer worker must have access to this file in order to load the credentials.
</Callout>
...
- Set the name of the [environment variable that holds the value of your OpenAI API key](https://help.openai.com/en/articles/5112595-best-practices-for-api-key-safety#h_a1ab3ba7b2).  
...
1. Set the value of your OpenAI API key.
   ...
   For example, [in an environment variable](https://help.openai.com/en/articles/5112595-best-practices-for-api-key-safety#h_a1ab3ba7b2) or in a [Docker configuration](/reference/pgai/vectorizer/worker#install-and-configure-vectorizer-worker).
...
analysis capabilities within a {C.PG} database.
