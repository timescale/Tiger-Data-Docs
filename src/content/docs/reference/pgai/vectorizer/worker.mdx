---
title: Running on Timescale Cloud
description: >-
  When you install pgai on Timescale Cloud, the vectorizer worker is
  automatically activated and run on a schedule
---
...
running. You can do this by setting [scheduling =>ai.scheduling_none()](/reference/pgai/vectorizer/api-reference#scheduling-configuration)
...
# Running on self-hosted {C.PG} or other platforms

When you use pgai vectorizers on a self-hosted {C.PG} installation or another cloud provider other than Timescale Cloud, you have to run the vectorizer worker yourself. The vectorizer worker will connect to your database and process the vectorizers you have defined. You can run the vectorizer:
...
**Prerequisites**: [Python (>= 3.10)](https://www.python.org/downloads/) and [pip](https://pip.pypa.io/en/stable/installation/#supported-methods)
...
1. **Run the vectorizer worker**
    ...
    After you [define a vectorizer in your database](/reference/pgai/vectorizer/overview#define-a-vectorizer), you run
...
1. **Run the vectorizer worker**
    ...
    After you [define a vectorizer in your database](/reference/pgai/vectorizer/overview#define-a-vectorizer), you run a vectorizer worker to generate and update your embeddings.
...
For example, if you are using a local {C.PG} database, you can set the database connection string as follows:
