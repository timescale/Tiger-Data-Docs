---
title: Migrating from the extension to the python library
description: >-
  Previous versions of pgai vectorizer used an extension to provide the
  vectorizer
---
...
to be used on more {C.PG} cloud providers (AWS RDS, Supabase, etc.) and
...
3. **Manage the vectorizer with the python library:** You can then manage the vectorizer from the python library or cli by using `pgai install -d DB_URL` as described in the new python-library-based [workflow](/reference/pgai/vectorizer/api-reference#install-or-upgrade-the-database-objects-necessary-for-vectorizer).
...
-  The `ai.create_vectorizer` call now requires a top-level `loading` argument. This allows us more flexibility in how we load data into the vectorizer. For example, we can now load data from file using the [`loading => loading_uri()`](/reference/pgai/vectorizer/api-reference#ailoading_uri) function.
- The destination where embeddings are stored is now configured via the `destination` top-level argument. This was done to allow us to support more types of schema design for storing embeddings. For example, we can now store embeddings in a column of a table via the [`destination => ai.destination_column()`](/reference/pgai/vectorizer/api-reference#aidestination_column) function in addition to the previous behavior of using a separate table via the [`destination => ai.destination_table()`](/reference/pgai/vectorizer/api-reference#aidestination_table) function.
...
* `ai.create_vectorizer` now requires a [`loading =>`](/reference/pgai/vectorizer/api-reference#loading-configuration) argument. Previous behavior is provided via the [`loading => loading_column()`](/reference/pgai/vectorizer/api-reference#ailoading_column) function.
* `ai.create_vectorizer` no longer takes `destination`, `target_table`, `target_schema`, `view_schema`, `view_name` as arguments configure these options via the new [`destination => ai.destination_table()`](/reference/pgai/vectorizer/api-reference#destination-configuration) function instead.
* [ai.chunking_character_text_splitter](/reference/pgai/vectorizer/api-reference#aichunking_character_text_splitter) and [ai.chunking_recursive_character_text_splitter](/reference/pgai/vectorizer/api-reference#aichunking_recursive_character_text_splitter) no longer take a `chunk_column` argument, that column name is now provided via [`loading => loading_column()`](/reference/pgai/vectorizer/api-reference#ailoading_column) function instead.
