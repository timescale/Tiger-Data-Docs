---
title: Overview
description: This document describes how to create and run vectorizers from Python.
---
...
It accepts all the options listed in the [SQL API](/reference/pgai/vectorizer/api-reference) and exposes the `to_sql`
...
Then, you can run this statement using the {C.PG} library of your choice. For example, using the [`psycopg`](https://www.psycopg.org/psycopg3/docs/) library:
...
You can then run the vectorizer worker using the the CLI tool or the `Worker` class discussed in the [vectorizer worker documentation](/reference/pgai/vectorizer/worker).  
...
- [SQLAlchemy integration](/reference/pgai/vectorizer/sqlalchemy-integration)
- [Alembic integration](/reference/pgai/vectorizer/alembic-integration)
