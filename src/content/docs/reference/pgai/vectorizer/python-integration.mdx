...
It accepts all the options listed in the [SQL API](/pgai/vectorizer/api-reference) and exposes the `to_sql`
...
You can then run the vectorizer worker using the the CLI tool or the `Worker` class discussed in the [vectorizer worker documentation](/pgai/vectorizer/worker).  
...
- [SQLAlchemy integration](/pgai/vectorizer/sqlalchemy-integration)
- [Alembic integration](/pgai/vectorizer/alembic-integration)
