...
description: >-
  The ai.loaddataset function allows you to load datasets from Hugging Face's
  datasets library directly into your PostgreSQL database.
---
...
The `ai.load_dataset` function allows you to load datasets from Hugging Face's datasets library directly into your {C.PG} database.
...
| schema_name   | text    | 'public'    | ✖        | The {C.PG} schema where the table will be created                                             |
...
| field_types   | jsonb   | -           | ✖        | Custom {C.PG} data types for columns as a JSONB dictionary from name to type.                 |
...
- The function automatically maps Hugging Face dataset types to appropriate {C.PG} data types unless overridden by `field_types`.
