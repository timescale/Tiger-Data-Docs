...
description: A number of pgai functions call out to third-party APIs which require an API
---
...
2. If you are self-hosting, you can [configure an API key through an environment variable available to the {C.PG} process](#configure-an-api-key-through-an-environment-variable-available-to-the-postgresql-process-self-hosted)
...
1. Navigate to the "AI Model API Keys" tab under "Project settings"
   ...
   ![Timescale Cloud project settings](https://raw.githubusercontent.com/timescale/pgai/main/docs/extension/images/timescale_project_settings.png)

1. Add a new AI Model API key, providing the name and API key

   ![Timescale Cloud new AI model API key](https://raw.githubusercontent.com/timescale/pgai/main/docs/extension/images/timescale_new_ai_model_api_key.png)
...
## Configure an API key through an environment variable available to the {C.PG} process (self-hosted)

If you're running {C.PG} yourself, or have the ability to configure the
runtime of {C.PG}, you set an environment variable for the {C.PG}
process.
...
Passing the `api_key` parameter to a pgai function as text results in the value being printed into the {C.PG} logs. This could expose your API key. Instead, we recommend passing the `api_key` parameter as a bind variable.
</Callout>
