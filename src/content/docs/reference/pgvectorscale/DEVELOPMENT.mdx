---
title: Setup your pgvectorscale developer environment
description: >-
  You build pgvectorscale from source, then integrate the extension into each
  database in your PostgreSQL environment.
---
...
You build pgvectorscale from source, then integrate the extension into each database in your {C.PG} environment.

## pgvectorscale prerequisites

To create a pgvectorscale developer environment, you need the following on your local machine:

* [{C.PG} v16](https://docs.timescale.com/self-hosted/latest/install/installation-linux/#install-and-configure-timescaledb-on-postgresql)
...
1. Install [Cargo-pgrx](https://lib.rs/crates/cargo-pgrx):
...
If the {C.PG} directory where the extension needs to be installed requires
elevated permissions (for example, when using MacOS {C.PG} that installs in the
