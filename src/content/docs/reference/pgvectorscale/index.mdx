...
title: pgvectorscale
description: >-
  pgvectorscale builds on pgvector with higher performance embedding search and
  cost-efficient storage for AI applications.
---
import * as C from "@constants";
import { Callout } from "@stainless-api/docs/components";

import { Image } from "astro:assets";
import imgBenchmarkComparisonPgvectorscalePinecone from "../../../../assets/images/reference/pgvectorscale/benchmark-comparison-pgvectorscale-pinecone.png";
...
pgvectorscale complements [pgvector](https://github.com/pgvector/pgvector/blob/master/README.md), the open-source vector data extension for {C.PG}, and introduces the following key innovations for pgvector data:
...
each, {C.PG} with `pgvector` and `pgvectorscale` achieves **28x lower p95
...
<Image src={imgBenchmarkComparisonPgvectorscalePinecone} alt="Benchmark comparison of pgvectorscale versus Pinecone" />
...
In contrast to pgvector, which is written in C, pgvectorscale is developed in [Rust](https://www.rust-lang.org/) using the [PGRX framework](https://github.com/pgcentralfoundation/pgrx),
offering the {C.PG} community a new avenue for contributing to vector support.

**Application developers or DBAs** can use pgvectorscale with their {C.PG} databases.
   * [Install pgvectorscale](#installation)
   * [Get started using pgvectorscale](#get-started-with-pgvectorscale)

If you **want to contribute** to this extension, see how to [build pgvectorscale from source in a developer environment](/reference/pgvectorscale/development) and our [testing guide](/reference/pgvectorscale/testing).
...
The fastest ways to run {C.PG} with pgvectorscale are:
...
You can install pgvectorscale from source and install it in an existing {C.PG} server

<Callout variant="warning">
Building pgvectorscale on macOS X86 (Intel) machines is currently not
supported due to an [open issue](https://github.com/timescale/pgvectorscale/issues/155). As alternatives, you can:
...
1. Compile and install the extension
    ...
    You can also take a look at our [documentation for extension developers](/reference/pgvectorscale/development) for more complete instructions.
...
The instructions below are for Timescale's standard compute instance. For production vector workloads, we're offering **private beta access to vector-optimized databases** with pgvector and pgvectorscale on Timescale. [Sign up here for priority access](https://timescale.typeform.com/to/H7lQ10eQ).
</Callout>

To enable pgvectorscale:

1. Create a new [Timescale Service](https://console.cloud.tigerdata.com/signup?utm_campaign=vectorlaunch).
...
<Callout variant="note" title="Label values">
  Label values must be within the {C.PG} `smallint` range (-32768 to 32767). Using `smallint[]` for labels ensures that {C.PG}'s type system will automatically enforce these bounds.
...
You can also use any {C.PG} WHERE clause with vector search, but these conditions will be applied as post-filtering:
...
[`iterative scan with relaxed ordering`](https://github.com/pgvector/pgvector?tab=readme-ov-file#iterative-index-scans) with
pgvector's ivfflat or hnsw indexes.

If you need strict ordering you can use a [materialized CTE](https://www.postgresql.org/docs/current/queries-with.html#QUERIES-WITH-CTE-MATERIALIZATION):
...
Timescale is a {C.PG} cloud company. To learn more visit the [timescale.com](https://www.timescale.com).

[Timescale Cloud](https://console.cloud.tigerdata.com/signup?utm_campaign=vectorlaunch) is a high-performance, developer focused, cloud platform that provides {C.PG} services for the most demanding AI, time-series, analytics, and event workloads. Timescale Cloud is ideal for production applications and provides high availability, streaming backups, upgrades over time, roles and permissions, and great security.
