...
pgvectorscale complements [pgvector][pgvector], the open-source vector data extension for {C.PG}, and introduces the following key innovations for pgvector data:
...
Label values must be within the {C.PG} `smallint` range (-32768 to 32767). Using `smallint[]` for labels ensures that {C.PG}'s type system will automatically enforce these bounds. pgvectorscale includes an implementation of the `&&` overlap operator for `smallint[]` arrays, which is used for efficient label-based filtering.
</Callout>
