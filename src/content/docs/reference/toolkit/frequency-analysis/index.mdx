...
keywords: [frequency analysis, frequency analysis overview, timescaledb toolkit]
---
...
- **freq\_agg**: Get the most common elements and their relative frequency using the SpaceSaving algorithm
- **count\_min\_sketch**: Estimate the absolute number of times a specific value appears using the count-min sketch data
  structure
...
SELECT toolkit_experimental.approx_count('user123', cms) AS estimated_count
FROM sketch;
```
...
- [`freq_agg()`](/reference/toolkit/frequency-analysis/freq_agg): track the most common values using a minimum frequency cutoff

### Count-min sketch

- [`count_min_sketch()`](/reference/toolkit/frequency-analysis/count_min_sketch): estimate absolute counts using the count-min sketch data structure
