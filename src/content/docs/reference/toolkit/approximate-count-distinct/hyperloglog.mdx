...
keywords: [approximate count distinct, hyperloglog, timescaledb toolkit]
---
...
using [`rollup()`](/reference/toolkit/approximate-count-distinct/rollup) before an accessor is applied.

If you're not sure what value to set for `buckets`, try using the alternate
aggregate function, [`approx_count_distinct()`](/reference/toolkit/approximate-count-distinct/approx_count_distinct).
