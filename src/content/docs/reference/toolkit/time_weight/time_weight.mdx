...
keywords: [time weight, time_weight, timescaledb toolkit]
---
...
using [`rollup()`](/reference/toolkit/time_weight/rollup) before an accessor is applied.
...
|--------|------|-------------|
| time\_weight | TimeWeightSummary | A `TimeWeightSummary` object that can be passed to other functions within the time-weighting API |
