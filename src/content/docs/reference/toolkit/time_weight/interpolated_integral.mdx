...
keywords: [interpolated_integral, time weight, timescaledb toolkit]
---
...
Similar to [`integral`](/reference/toolkit/time_weight/integral), but allows an accurate calculation across interval bounds when data has been bucketed
...
Equal to [`interpolated_average`](/reference/toolkit/time_weight/interpolated_average) multiplied by the elapsed time.
...
| prev | TimeWeightSummary | NULL | | The TimeWeightSummary from the prior interval, used to interpolate the value at `start`. If NULL, the first timestamp in `tws` is used for the starting value. The prior interval can be determined from the {C.PG} [`lag()`](https://www.postgresql.org/docs/current/functions-window.html#FUNCTIONS-WINDOW-TABLE) function |
| next | TimeWeightSummary | NULL | | The TimeWeightSummary from the next interval, used to interpolate the value at `start` + `interval`. If NULL, the last timestamp in `tws` is used for the starting value. The next interval can be determined from the {C.PG} [`lead()`](https://www.postgresql.org/docs/current/functions-window.html#FUNCTIONS-WINDOW-TABLE) function |
