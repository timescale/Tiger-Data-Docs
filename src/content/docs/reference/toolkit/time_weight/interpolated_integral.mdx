...
<TagIcon iconType="duotone" /> Since [1.15.0][toolkit-1.15.0]
...
| prev | TimeWeightSummary | NULL | | The TimeWeightSummary from the prior interval, used to interpolate the value at `start`. If NULL, the first timestamp in `tws` is used for the starting value. The prior interval can be determined from the {C.PG} [`lag()`][pg-window-functions] function |
| next | TimeWeightSummary | NULL | | The TimeWeightSummary from the next interval, used to interpolate the value at `start` + `interval`. If NULL, the last timestamp in `tws` is used for the starting value. The next interval can be determined from the {C.PG} [`lead()`][pg-window-functions] function |
