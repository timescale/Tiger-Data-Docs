...
keywords: [time weight, time-weighted calculations overview, timescaledb toolkit]
---
...
Time-weighted average calculations are not strictly parallelizable, as defined by {C.PG}. These calculations require
inputs to be strictly ordered, but in general, {C.PG} parallelizes by assigning rows randomly to workers.
...
The time-weighted average functions are not strictly parallelizable in the {C.PG} sense. {C.PG} requires that
...
- [`time_weight()`](/reference/toolkit/time_weight/time_weight): aggregate data into an intermediate time-weighted aggregate form for further
  calculation

### Accessors

- [`average()`](/reference/toolkit/time_weight/average): calculate the time-weighted average of values in a TimeWeightSummary
- [`first_time()`](/reference/toolkit/time_weight/first_time): get the timestamp of the first point in the TimeWeightSummary
- [`first_val()`](/reference/toolkit/time_weight/first_val): get the value of the first point in the TimeWeightSummary
- [`integral()`](/reference/toolkit/time_weight/integral): calculate the integral from a TimeWeightSummary
- [`interpolated_average()`](/reference/toolkit/time_weight/interpolated_average): calculate the time-weighted average, interpolating at boundaries
- [`interpolated_integral()`](/reference/toolkit/time_weight/interpolated_integral): calculate the integral, interpolating at boundaries
- [`last_time()`](/reference/toolkit/time_weight/last_time): get the timestamp of the last point in the TimeWeightSummary
- [`last_val()`](/reference/toolkit/time_weight/last_val): get the value of the last point in the TimeWeightSummary

### Rollup

- [`rollup()`](/reference/toolkit/time_weight/rollup): combine multiple TimeWeightSummaries
