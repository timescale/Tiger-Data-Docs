...
keywords: [counters and gauges, gauge agg, interpolated_rate, timescaledb toolkit]
---
...
|--------|------|-------------|
| interpolated\_rate | DOUBLE PRECISION | The per-second rate of change of the gauge between the specified bounds. If exact values are missing in the raw data for the first and last points, these values are interpolated linearly from the neighboring gauge aggregates. |
