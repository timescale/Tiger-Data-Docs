...
keywords: [percentile approximation, percentile approximation overview, timescaledb toolkit]
---

import * as C from "@constants";

These functions are more CPU- and memory-efficient than exact calculations using {C.PG}'s `percentile_cont` and
...
### Using percentile\_agg (recommended for most use cases)
...
- [`uddsketch()`](/reference/toolkit/percentile-approximation/uddsketch): estimate percentiles using the UddSketch algorithm with guaranteed relative error

### t-digest (for extreme quantiles)

- [`tdigest()`](/reference/toolkit/percentile-approximation/tdigest): estimate percentiles using the t-digest algorithm, optimized for extreme quantiles
