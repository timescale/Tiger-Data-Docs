...
calculates the variance for all the values of the `y` variable, independent of values of the `x` variable.
...
| method | TEXT | `sample` | - | The method used for calculating the variance. The two options are `population` and `sample`, which can be abbreviated to `pop` or `samp` |
