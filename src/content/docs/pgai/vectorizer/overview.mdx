...
etc.) or self-hosted Postgres, you can use the [vectorizer worker](/pgai/vectorizer/worker) to
...
**Tip**: You can use the `ai.ollama_embed` function in our [PostgreSQL extension](/projects/extension/README) to generate an embedding for a user-provided query right inside the database.
