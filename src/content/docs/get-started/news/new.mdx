...
<div class="changelog-page-toc">
  ...
  The Logs page has been fully overhauled based on customer interviews to make debugging workflows faster and more intuitive.

  **What's new**

  - **Search:** Text search logs directly in the UI to quickly find specific phrases or error messages
  - **Filters:** Apply severity filters to show only the log levels you care about
  - **Histogram view:** Visualize log volume over time to spot spikes that correlate with database or application changes
  - **Better time filters:** Improved date and time selectors for pinpointing logs from a specific window
  - **CSV download:** Export logs as a CSV to analyze in your preferred tool or feed into an LLM for debugging help
  - **Scroll:** Smoother, glitch-free scroll experience when browsing large log volumes
  - **Copy:** Copy individual log lines directly from the UI to paste into your favorite LLM or support ticket
