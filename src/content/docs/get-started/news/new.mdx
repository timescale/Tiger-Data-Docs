...
## Storage breakdown by category on the service overview and in the explorer

<ChangelogEntry
  title="Storage breakdown by category on the service overview and in the explorer"
  date="2026-09-17"
  tags={["improvement"]}
>
<h2 slot="heading" class="changelog-entry__title">Storage breakdown by category on the service overview and in the explorer</h2>

Tiger Console now shows you where the disk space of a service goes, on the `Overview` tab and in the `Explorer`, so that the numbers in both places add up.

- **Storage categories on the overview**: the `Storage` card breaks disk usage down into `Rowstore`, `Columnstore`, `Tiered`, `WAL`, `Temp`, and `Other`. Click `What's included` to see every category with its size and what it contains. See [Storage used](/deploy/tiger-cloud/tiger-cloud-aws/service-management/service-overview#storage-used).
- **Rowstore and columnstore sizes in the explorer**: the `Columnstore` table lists the `Rowstore` and `Columnstore` size of every hypertable and continuous aggregate next to its total size, and the former `Columnstore` percentage column is now called `Compression`. In `General information`, hover over `On-disk size` to see how it splits between the rowstore and the columnstore. See [Service explorer](/deploy/tiger-cloud/tiger-cloud-aws/service-management/service-explorer#columnstore).
