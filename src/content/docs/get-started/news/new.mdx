...
## Updated service creation flow on {C.CLOUD_LONG}

<ChangelogEntry
  title="Updated service creation flow on {C.CLOUD_LONG}"
  date="2026-07-01"
  tags={["improvement"]}
>
<h2 slot="heading" class="changelog-entry__title">Updated service creation flow on {C.CLOUD_LONG}</h2>
...
It is now the default in {C.CLOUD_LONG}, so check it out.
</ChangelogEntry>

## {C.TIMESCALE_DB} v2.27.2 now on {C.CLOUD_LONG}
...
>
<h2 slot="heading" class="changelog-entry__title">{C.TIMESCALE_DB} v2.27.2 now on {C.CLOUD_LONG}</h2>

{C.TIMESCALE_DB} v2.27.2 is a stability and security patch that supersedes 2.27.1 on {C.CLOUD_LONG}. Together, 2.27.1 and 2.27.2 close three access-control gaps in the jobs and policy subsystems, fix a correctness bug in grouped aggregation on the {C.COLUMNSTORE}, refine the {C.COLUMNSTORE} query planner, and ship upgrade-compatibility scripts for composite bloom filter metadata. {C.TIMESCALE_DB} 2.27.1 was released on May 19, 2026 and 2.27.2 on June 2, 2026; both are delivered to {C.CLOUD_LONG} as 2.27.2 starting June 22, 2026.

### Highlighted fixes in {C.TIMESCALE_DB} v2.27.2
...
**Correctness and planner refinements on the {C.COLUMNSTORE}**
...
For complete details, refer to the [{C.TIMESCALE_DB} 2.27.1 release notes](https://github.com/timescale/timescaledb/releases/tag/2.27.1) and the [{C.TIMESCALE_DB} 2.27.2 release notes](https://github.com/timescale/timescaledb/releases/tag/2.27.2).
</ChangelogEntry>
...
## {C.TIMESCALE_DB} v2.27 now on {C.CLOUD_LONG}
...
<ChangelogEntry
  ...
  tags={["new-feature"]}
>
<h2 slot="heading" class="changelog-entry__title">{C.TIMESCALE_DB} v2.27 now on {C.CLOUD_LONG}</h2>

{C.TIMESCALE_DB} 2.27.0 is a performance-focused release that extends bloom filter pruning into the write path so `UPDATE`, `DELETE`, and `UPSERT` can now skip decompressing batches that can't match, broadens vectorized execution on the {C.COLUMNSTORE} to cover more analytical query patterns, and introduces an opt-in query rewriter that transparently routes matching queries to existing continuous aggregates. {C.TIMESCALE_DB} 2.27.0 was released on May 12, 2026, and is available on GitHub and on {C.CLOUD_LONG} as of May 19, 2026.

### Highlighted features in {C.TIMESCALE_DB} v2.27

- **Bloom filter pruning now applies to writes**

    This release extends bloom filter pruning — previously only used by `SELECT` — to `UPDATE`, `DELETE`, and `UPSERT` on compressed {C.CHUNK}s.
...
- **More queries on the vectorized {C.COLUMNSTORE} path**
    ...
    - **Vectorized filters via the standard {C.PG} function path:** The {C.COLUMNSTORE} engine now supports vectorized evaluation of filters inline through the standard {C.PG} function path. This broadens the class of queries that can take the fast columnar path — including continuous aggregate refreshes — with benchmarks showing speedups from **30% up to 2x** on affected workloads.
...
- **Smarter {C.CHUNK} exclusion**
    ...
    - **{C.CHUNK_CAP} exclusion for `IN` / `ANY` on open time dimensions:** Queries that filter the time dimension with `IN` or `= ANY(...)` clauses now benefit from {C.CHUNK} pruning — the planner derives a bounding min/max range from the values so irrelevant {C.CHUNK}s can be excluded earlier.
    - **Faster `ORDER BY … LIMIT` on more compressed data:** Batch Sorted Merge — which returns ordered results from compressed {C.CHUNK}s without fully decompressing and sorting them — now applies to {C.CHUNK}s with no `segmentby`, and to {C.CHUNK}s where the query pins every `segmentby` column to a constant.

- **Quality of life and operability**

    - **Index creation progress reporting:** Hypertable index builds now populate {C.PG}'s built-in `pg_stat_progress_create_index` view, so long-running builds can be monitored the same way as on plain {C.PG}.
    ...
    - **`ENABLE` / `DISABLE TRIGGER` on hypertables:** Standard {C.PG} trigger-state commands now work directly on hypertables and propagate to every underlying {C.CHUNK} (including `ENABLE ALWAYS`, `ENABLE REPLICA`, and `DISABLE TRIGGER USER`/`ALL`).
...
### Upcoming {C.PG} 15 EOL

As announced in the [v2.23.0 changelog](https://github.com/timescale/timescaledb/releases/tag/2.23.0) on October 29, 2025, the upcoming {C.TIMESCALE_DB} release in June 2026 will be the last version supporting {C.PG} 15. If you are still on {C.PG} 15, plan an upgrade to {C.PG} 16 or higher to maintain access to new features, bug fixes, and performance improvements.

For complete details, refer to the [{C.TIMESCALE_DB} 2.27.0 release notes](https://github.com/timescale/timescaledb/releases/tag/2.27.0).
</ChangelogEntry>
