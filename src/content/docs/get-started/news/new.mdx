---
title: Changelog
description: Get the latest updates to Tiger Cloud with links to detailed documentation
excerpt: Stay informed about new features, improvements, and releases
...
import imgServiceCreationFlowTigerCloud from "../../../../assets/images/get-started/news/service-creation-flow-tiger-cloud.png";
import imgChunkTimelineTigerCloud from "../../../../assets/images/get-started/news/chunk-timeline-tiger-cloud.png";
...
import imgAutomatedChunkTuningLight from "../../../../assets/images/learn/performance-optimization/automated-chunk-tuning.png";
import imgAutomatedChunkTuningDark from "../../../../assets/images/learn/performance-optimization/automated-chunk-tuning-dark.png";
...
import imgTigerConsoleMembersAuthEnforcement from "../../../../assets/images/deploy/tiger-cloud/tiger-console-members-authentication-enforcement.png";
...
import imgCreateHypertable1 from "../../../../assets/images/get-started/news/create-hypertable-1.png";
import imgCreateHypertable2 from "../../../../assets/images/get-started/news/create-hypertable-2.png";
...
import imgCmdKCommandPaletteTigerConsole from "../../../../assets/images/get-started/news/cmd-k-command-palette-tiger-console.png";
import imgTigerlakeGenericRestCatalogSetup from "../../../../assets/images/get-started/news/tigerlake-generic-rest-catalog-setup.png";
import imgBackupsExtendedRetention from "../../../../assets/images/get-started/news/backups-extended-retention.png";

<ChangelogFilter />º

<div class="changelog-page-toc">

## Tiger CLI 0.25: clearer command names, service rename and delete from agents, and a feedback command

<ChangelogEntry
  title="Tiger CLI 0.25: clearer command names, service rename and delete from agents, and a feedback command"
  date="2026-09-21"
  tags={["new-feature", "cli-mcp"]}
>
<h2 slot="heading" class="changelog-entry__title">Tiger CLI 0.25: clearer command names, service rename and delete from agents, and a feedback command</h2>

Tiger CLI 0.25 renames several commands for clarity, closes the gap between the CLI and Tiger MCP on service management, and cleans up the CLI's output.

- **Clearer command names**: `tiger db connect` is now `tiger db psql` (it requires a local `psql`, unlike `tiger db query`), `tiger db connection-string` is `tiger db uri`, `tiger db test-connection` is `tiger db ping`, and `tiger config show` is `tiger config list`. Every old name keeps working as an alias, so existing scripts are unaffected.
- **`tiger service rename`**: rename a service without touching its ID, endpoints, or stored credentials. A paired `service_rename` MCP tool does the same from your agent.
- **`service_delete` MCP tool**: agents can now delete services. Deleting a service tagged PROD prompts you through the MCP client to type the service ID back before it proceeds; if the client can't prompt, the deletion is refused. The CLI's `tiger service delete` also drops its `--no-wait` and `--wait-timeout` flags, since the API removes the service as soon as it accepts the deletion.
- **`environment` parameter on `service_create` and `service_fork` MCP tools**: agents can tag new services and forks as DEV or PROD, matching the CLI's `--environment` flag, with `read_only=prod` refusing PROD requests.
- **`tiger feedback`**: send feedback or a bug report to the Tiger Data team straight from the terminal.
- **Devin support**: `tiger mcp install devin` configures Tiger MCP for the Devin client.
- **Cleaner output**: CLI status messages drop the emoji and filler for a plain, professional tone.

To upgrade, run `tiger upgrade` or use your package manager. For all commands and flags, see the [Tiger CLI reference](/reference/tiger-cloud/tiger-cli).

</ChangelogEntry>

## Extended backup retention and point-in-time recovery

<ChangelogEntry
  title="Extended backup retention and point-in-time recovery"
  date="2026-09-16"
  tags={["new-feature"]}
>
<h2 slot="heading" class="changelog-entry__title">Extended backup retention and point-in-time recovery</h2>

Tiger Cloud now offers extended backup retention on the Enterprise plan, enabling longer compliance and audit windows.

All services on the Enterprise plan include 14 days of backups and point-in-time recovery by default. You can extend the retention period up to 180 days to meet longer compliance requirements.

Extended retention includes full backups and WAL files, which enable point-in-time recovery down to the second for any point within your retention window. The recovery window for [point-in-time recovery forks](/deploy/tiger-cloud/tiger-cloud-aws/high-availability/backup-restore#recover-to-a-point-in-time) matches your retention period: if you set retention to 90 days, backups will accumulate to cover that period, and you can fork your service to any point in the previous 90 days.

When you extend the retention period, backups and WAL accumulate until they cover it in full. When you shorten it, backups outside the new period are deleted and are no longer available for recovery.

Extended backup retention beyond the included 14 days is charged at $0.03 per GB per month based on the additional storage consumed.

<Image src={imgBackupsExtendedRetention} alt="Backup and restore screen in Tiger Console, showing how backup retention window can be extended" />

</ChangelogEntry>

## Automatic replica management during major version upgrades

<ChangelogEntry
  title="Automatic replica management during major version upgrades"
  date="2026-09-14"
  tags={["improvement"]}
>
<h2 slot="heading" class="changelog-entry__title">Automatic replica management during major version upgrades</h2>

Tiger Cloud now automatically handles replicas during major Postgres version upgrades.

When you upgrade your service, we automatically spin down any HA replicas or read replicas, perform the upgrade, and bring them back up with the new version. This eliminates the need to manually remove and recreate replicas, and keeps your replica connection strings unchanged throughout the upgrade process.

</ChangelogEntry>

## Revoke sessions from Tiger Console

<ChangelogEntry
  title="Revoke sessions from Tiger Console"
  date="2026-09-09"
  tags={["new-feature", "security"]}
>
<h2 slot="heading" class="changelog-entry__title">Revoke sessions from Tiger Console</h2>

You can now see and revoke your account's active sessions directly in Tiger Console. This is particularly useful for revoking Tiger CLI and Tiger MCP access: every `tiger auth login` appears as a session, so you can sign out a CLI or an AI agent on another machine without touching that machine.

To review your sessions, click your name in the top right corner of Tiger Console and select `Account settings`. The `Active sessions` list shows every signed-in device, browser and CLI alike, with its location, sign-in time, and last activity. Revoke any session you don't recognize or no longer need, or click `Revoke all other sessions` to sign out everything except your current browser.

See [Active sessions](/deploy/tiger-cloud/tiger-cloud-aws/security/overview#active-sessions) for details.

</ChangelogEntry>

## Tiger CLI 0.24: db query command, device code login, and GitHub Copilot support

<ChangelogEntry
  title="Tiger CLI 0.24: db query command, device code login, and GitHub Copilot support"
  date="2026-09-09"
  tags={["new-feature", "cli-mcp"]}
>
<h2 slot="heading" class="changelog-entry__title">Tiger CLI 0.24: db query command, device code login, and GitHub Copilot support</h2>

Tiger CLI 0.24 makes it easier to run SQL from the terminal and to log in from machines without a browser.

- **`tiger db query`**: run SQL against a service directly from the terminal, with no local `psql` required. Pass the query with `--command`, point `--file` at a `.sql` file, or pipe it through stdin, and get results as a `psql`-style table, JSON, or YAML. The matching MCP tool is renamed from `db_execute_query` to `db_query`, gains the same `file` input, and now returns values exactly as PostgreSQL's text format renders them. See the [`tiger db query` reference](/reference/tiger-cloud/tiger-cli#tiger-db-query).
- **Device code login**: `tiger auth login --headless` prints a short code and a URL, and you authorize from a browser on any machine. This makes logging in from SSH sessions, containers, and remote development environments work; the browser login only completes when the browser and the CLI are on the same machine. If the browser cannot be opened, `tiger auth login` falls back to the device code flow automatically.
- **GitHub Copilot support**: `tiger mcp install copilot` configures Tiger MCP for the GitHub Copilot CLI, and the Copilot desktop app picks up the same configuration.
- **`--version-check` flag**: the global `--skip-update-check` flag is renamed to `--version-check` to match the `version_check` configuration key; disabling the startup update check is now `--version-check=false`. The old flag still works as a hidden alias.

To upgrade, run `tiger upgrade` or use your package manager. For all commands and flags, see the [Tiger CLI reference](/reference/tiger-cloud/tiger-cli).

</ChangelogEntry>

## Tiger CLI 0.23: protect production services from writes, list your projects

<ChangelogEntry
  title="Tiger CLI 0.23: protect production services from writes, list your projects"
  date="2026-09-01"
  tags={["new-feature", "cli-mcp"]}
>
<h2 slot="heading" class="changelog-entry__title">Tiger CLI 0.23: protect production services from writes, list your projects</h2>

Tiger CLI 0.23 makes read-only mode production-aware and adds a command to list your projects.

- **Read-only modes**: the `read_only` configuration option now takes three modes instead of a boolean: `all` protects every service, `prod` protects only services tagged PROD, and `off` (the default) protects none. With `read_only=prod`, an AI agent can create and modify development services freely while production stays protected: mutating commands and MCP tools are refused for PROD-tagged services, and database sessions against them open in Tiger Cloud's immutable read-only mode. The old boolean values keep working (`true` means `all`, `false` means `off`), and your first interactive `tiger auth login` now asks which mode you want. See [Restrict Tiger MCP to read-only](/get-started/quickstart/mcp-cli#restrict-tiger-mcp-to-read-only).
- **`tiger project list`**: list the projects you have access to, with the active one marked. See the [`tiger project list` reference](/reference/tiger-cloud/tiger-cli#tiger-project-list).

To upgrade, run `tiger upgrade` or use your package manager.

</ChangelogEntry>

## Billing page now segments costs by service and adds a timeline view

<ChangelogEntry
  title="Billing page now segments costs by service and adds a timeline view"
  date="2026-08-12"
  tags={["improvement"]}
>
<h2 slot="heading" class="changelog-entry__title">Billing page now segments costs by service and adds a timeline view</h2>

The billing page in Tiger Console now gives you clearer visibility into what drives your costs.

- **Cost segmentation by service:** Usage and charges are broken down per service, so you can see exactly which services contribute to your bill.
- **Timeline view:** A new timeline shows how your usage and costs trend over the last 7 days, making it easier to spot changes and plan ahead.

</ChangelogEntry>
  
## TimescaleDB v2.29 now on Tiger Cloud

<ChangelogEntry
  title="TimescaleDB v2.29 now on Tiger Cloud"
  date="2026-08-10"
  tags={["new-feature", "performance"]}
>
<h2 slot="heading" class="changelog-entry__title">TimescaleDB v2.29 now on Tiger Cloud</h2>
...
- **Common subexpression elimination in columnar aggregation:** When several aggregates reference the same expression — for example `SUM(price * quantity)` and `AVG(price * quantity)` — the executor now computes it once and reuses the result across aggregates.
- **`first`/`last` in `ColumnarIndexScan`:** `first(value, time)` and `last(value, time)` can now read from sparse `min`/`max` metadata in the `ColumnarIndexScan` path, so common earliest/latest-value queries skip decompressing full batches.
- **Faster last-point and point queries:** The planner can now choose a row-by-row decompression path for startup-cost-sensitive queries such as `ORDER BY time DESC LIMIT 1` and selective point lookups, making last-point queries several times faster.
- **`MIN`/`MAX` correctness:** Corrects a case where `MIN` and `MAX` in the columnar aggregation pipeline could disagree with plain PostgreSQL results.
...
- **Forced refreshes are now incremental by default:** Building on the batched manual refresh introduced in 2.28, forced runs of `refresh_continuous_aggregate()` now execute incrementally by default, and you gain finer control over batch size, per-execution limits, and iteration order through the JSONB options argument.

    ```sql
    CALL refresh_continuous_aggregate(
      'my_cagg', '2023-01-01', '2024-01-01',
      options => '{"buckets_per_batch": 20, "max_batches_per_execution": 5, "refresh_newest_first": true}'::jsonb
    );
    ```
...
- **`VACUUM` propagation:** Running `VACUUM` on a chunk now also propagates to its compressed relation.
- **Merge job configuration:** `alter_job()` adds a `config_merge` parameter that merges JSONB into an existing job configuration instead of replacing it — for example, `SELECT alter_job(42, config_merge => '{"max_retries":3}'::jsonb)`.
- **In-place sparse index maintenance:** `_timescaledb_functions.rebuild_sparse_index()` rebuilds sparse-index metadata on an already-compressed chunk, with a fix for stale entries left after a rebuild.
- **New maintenance helpers:** Adds `decompress_batch()` and a `samplerate` argument on `estimate_uncompressed_size()`.
- **Transactional extension update:** `ALTER EXTENSION timescaledb UPDATE` can now run inside a transaction.

### Backward-incompatible changes

- **PostgreSQL 15 support removed:** TimescaleDB 2.29 drops support for PostgreSQL 15, completing the previously announced EOL. Move to PostgreSQL 16 or 17 before upgrading.
- **`job_errors` view removed:** Removed as part of the `bgw_job_stat_history` migration. Query `bgw_job_stat_history` for job error history instead.

### Stability and security patch (2.29.1)

2.29.1 addresses stability issues; adds missing permission checks for internal chunk functions; and adds an ordered-scan path for hypertable max-time lookups. It also addresses security vulnerabilities (advisory; PRs #10360, #10379, #10386); upgrade at your next opportunity.

For complete details, refer to the [TimescaleDB 2.29.0 release notes](https://github.com/timescale/timescaledb/releases/tag/2.29.0) and [2.29.1 release notes](https://github.com/timescale/timescaledb/releases/tag/2.29.1).

</ChangelogEntry>

## Generic Iceberg REST catalog support in TigerLake

<ChangelogEntry
  title="Generic Iceberg REST catalog support in TigerLake"
  date="2026-08-05"
  tags={["new-feature", "integration"]}
>
<h2 slot="heading" class="changelog-entry__title">Generic Iceberg REST catalog support in TigerLake</h2>

TigerLake can now write to any Iceberg REST-compatible catalog, including Snowflake Open Catalog, Apache Polaris, and Snowflake Horizon Catalog. This is for customers who already run an external Iceberg catalog and want TigerLake to write into it directly, rather than migrating to a new destination.

Amazon S3 Tables remains the recommended managed Iceberg destination and includes automatic compaction and table maintenance.

<Image src={imgTigerlakeGenericRestCatalogSetup} alt="Connector setup in Tiger Console showing the Iceberg REST catalog configuration form with catalog URI, warehouse, client ID and secret, scope, and vended credentials fields" />

- OAuth 2.0 client-credentials authentication
- Configurable catalog URI, warehouse, and scope
- Vended storage credential requests
- Custom catalog headers
- Secure client-secret handling
- Catalog connectivity and permission validation before you sync

REST catalogs use the same TigerLake snapshot and continuous CDC pipeline as S3 Tables. See the [TigerLake destination connector documentation](/integrate/connectors/destination/tigerlake) for setup details.

</ChangelogEntry>

## More reliable TigerLake replication slots with the Slot Keep Controller

<ChangelogEntry
  title="More reliable TigerLake replication slots with the Slot Keep Controller"
  date="2026-08-05"
  tags={["improvement"]}
>
<h2 slot="heading" class="changelog-entry__title">More reliable TigerLake replication slots with the Slot Keep Controller</h2>

All TigerLake-enabled services now run with the Slot Keep Controller (SKC), which manages replication-slot retention automatically. New TigerLake connectors are enrolled at creation, so no setup is required.

Previously, a fixed, instance-wide WAL retention limit could invalidate a replication slot without warning once WAL lag grew too large, forcing TigerLake's CDC pipeline to fully re-bootstrap. SKC replaces that fixed limit with retention that scales with each service's available disk space, and prioritizes dropping TigerData-owned slots (which recreate automatically) over customer slots if a service ever comes under disk pressure.

Every SKC action is recorded in an audit log, so any slot change can be traced after the fact.

</ChangelogEntry>

## Cmd+K command palette in Tiger Console

<ChangelogEntry
  title="Cmd+K command palette in Tiger Console"
  date="2026-07-27"
  tags={["new-feature"]}
>
<h2 slot="heading" class="changelog-entry__title">Cmd+K command palette in Tiger Console</h2>

Press `Cmd`+`K` (`Ctrl`+`K` on Windows and Linux) anywhere in Tiger Console to open a command palette:

<Image src={imgCmdKCommandPaletteTigerConsole} alt="Cmd+K command palette open in Tiger Console showing account, actions, and project pages" />

- **Go anywhere**: jump to any page in the Console without clicking through menus.
- **Switch instantly**: type a service or project name to jump straight to it.
- **Run actions**: create a service, manage existing ones, and more, directly from the palette.

Fully keyboard-navigable, end to end. No setup is needed.

</ChangelogEntry>

## Live migration has been removed

<ChangelogEntry
  title="Live migration has been removed"
  date="2026-07-24"
  tags={["deprecation"]}
>
<h2 slot="heading" class="changelog-entry__title">Live migration has been removed</h2>

The live-migration Docker image has been removed from Tiger Cloud and is no longer supported. It is replaced by [Livesync replication](/migrate/livesync-replication), which provides the same near-zero-downtime experience with better throughput and broader source-database support.

Livesync replication is based on standard PostgreSQL logical replication.

### What this means for you

- **New migrations:** Use [Livesync replication](/migrate/livesync-replication). See [Choose a migration approach](/migrate/choose-your-approach) to pick the right path for your source database, size, and downtime budget.
- **Existing migrations:** If you have an in-progress live migration, finish it and cut over, then use Livesync replication for any future migrations.
- **Documentation:** The dedicated live-migration page has been removed. Any links to it now redirect to Livesync replication.

### Why the change

Livesync replication uses the same PostgreSQL logical replication protocol under the hood, but as a first-class managed integration and self-hosted image rather than a standalone Docker image built on `pgcopydb`. It matches every live-migration use case with fewer constraints, including no strict TimescaleDB version matching and support for multi-tenant consolidation and splitting.
...
<ChangelogEntry
  title="Enforce MFA or SSO for all project members"
  date="2026-07-23"
  tags={["new-feature", "security"]}
>
<h2 slot="heading" class="changelog-entry__title">Enforce MFA or SSO for all project members</h2>

### Now available

Scale and Enterprise plan customers can now [enforce multi-factor authentication (MFA) or single sign-on (SSO)](/deploy/tiger-cloud/tiger-cloud-aws/security/members#enforce-mfa-or-sso-for-all-project-members) for every member of a project. This is table stakes for security-conscious teams running mission-critical applications, ensuring everyone who accesses a project meets your organization's authentication standard.

- **Project-wide enforcement:** Turn on enforcement of either MFA or SSO for all members of a project.
- **Access blocking:** When users log in or switch to a project, they are blocked if they don't use the required authentication method.
- **Clear enforcement visibility:** The project selector dropdown shows which enforcement method is required for each project.
- **Compliance at a glance:** The members page delineates who currently complies with the enforcement method, so project owners can contact members who need to update their authentication method.

<Image src={imgTigerConsoleMembersAuthEnforcement} alt="Members page in Console, showing the authentication enforcement banner and members grouped by policy compliance" />
...
<ChangelogEntry
  title="TimescaleDB v2.28 now on Tiger Cloud"
  date="2026-07-02"
  tags={["new-feature", "performance", "deprecation"]}
>
<h2 slot="heading" class="changelog-entry__title">TimescaleDB v2.28 now on Tiger Cloud</h2>
...
**Developer experience improvements on continuous aggregates**
...
- **`ALTER MATERIALIZED VIEW ... ADD COLUMN`:** Add new aggregate columns directly to an existing continuous aggregate, in place, when the new column is a `GENERATED ALWAYS AS (<aggregate>) STORED` expression. New refreshes populate the column going forward, and you can backfill historical rows at your own pace.
...
- **`ANALYZE` and `VACUUM` work directly on continuous aggregates:** Run either maintenance command against a continuous aggregate by its own name, just like any other table, and TimescaleDB transparently applies it to the underlying materialization hypertable and all associated chunks, including columnstore chunks.
- **Incremental manual refresh:** `refresh_continuous_aggregate()` now supports batched, incremental refresh through a new JSONB options argument, bringing manual refresh in line with policy-driven refresh and reducing memory pressure and lock duration on large refreshes.

    ```sql
    CALL refresh_continuous_aggregate(
      'metrics_hourly',
      now() - INTERVAL '30 days', now(),
      options => '{"buckets_per_batch": 24}'::jsonb
    );
    ```

- **`timescaledb.skip_cagg_invalidation` GUC for bulk loads:** A new session- or transaction-scoped GUC suppresses continuous aggregate invalidation tracking during DML and DDL on hypertables, for trusted bulk-load workflows where the caller refreshes affected CAggs afterward. This eliminates per-row invalidation overhead during large ingests.
- **Lighter locking during invalidation processing:** Refresh operations now use a targeted row-level lock on the `continuous_agg` catalog row instead of a broader `ShareUpdateExclusiveLock`, so unrelated work on the same CAgg can proceed concurrently with refresh.
...
```sql
SELECT device_id, last(value, time)
FROM metrics
GROUP BY device_id;
```

**Broader vectorized execution**

- **Vectorized `CASE` in the columnar executor:** `CASE ... WHEN ... THEN ... ELSE ... END` expressions inside vectorized aggregation over compressed hypertables now run natively on the vectorized path, a meaningful speedup for conditional aggregation.
- **Higher parallelism for `SELECT` on compressed hypertables:** Parallelism is now sized to the uncompressed data size, so large analytical scans over compressed hypertables get proportionally more workers and finish faster.

**Sparse-index maintenance**

- **Retrofit sparse indexes on existing chunks:** A new internal utility `_timescaledb_functions.rebuild_sparse_index()` updates sparse-index metadata on already-compressed chunks without a full decompress and recompress cycle. It diffs current metadata against the active compression settings and applies only what changed — useful after changing `segmentby`, `orderby`, or index settings on a hypertable that already has compressed data.
- **Automatic cleanup when dropping columns:** When a column is dropped from a hypertable on the columnstore, TimescaleDB now automatically removes all associated sparse-index metadata columns (minmax and bloom) from compressed chunks, keeping compressed storage clean.
...
```sql
SELECT hypertable_name, chunk_name, compressed_batch_rows_avg
FROM timescaledb_information.stat_chunk_activity;
```

**Gap-fill planning improvements**

- **Better row-count estimates for `time_bucket_gapfill()`:** The planner now accounts for rows synthesized by gap-filling rather than relying solely on the subplan's row count. When `start`, `finish`, and `period` are known at plan time, estimates are materially closer to actual emitted rows, leading to better join and aggregation plans downstream.
- **Subquery execution-parameter arguments to `time_bucket_gapfill()`:** Gap-fill arguments can now resolve from subquery execution parameters, expanding support to more dynamic-argument query shapes.

### Correctness and upgrade hardening

The 2.28.1 and 2.28.2 patches folded into this rollout focus on compressed-data correctness and a clean upgrade path.

- **Compressed-data correctness (2.28.1):** Fixes a data-loss bug where a `DELETE` with a subquery resolving to constant false could remove every row; fixes a server crash during DML when the planner chose a Bitmap Heap Scan; enforces `CHECK`, `NOT NULL`, and view `WITH CHECK OPTION` constraints on direct-compress inserts; corrects `ALTER TABLE ... RENAME COLUMN` on compressed chunks; and resolves several `first`/`last` aggregate edge cases.
- **Upgrade path (2.28.1 and 2.28.2):** The 2.28.0 upgrade blocker that prevented older instances (created before 2.15.0 without a prior PostgreSQL version upgrade) from upgrading is resolved. 2.28.2 stabilizes the migration further by correctly migrating internal catalog tables, automatically dropping incompatible bloom filters during upgrade instead of halting for manual intervention, and fixing sparse-index column ordering for `first`/`last`.
...
- **`_timescaledb_catalog.chunk_constraint` removed:** The internal `chunk_constraint` catalog table has been removed. Customers who query this catalog table directly will need to migrate. There are no SQL-surface behavior changes for standard TimescaleDB users.

For complete details, refer to the [TimescaleDB 2.28.0 release notes](https://github.com/timescale/timescaledb/releases/tag/2.28.0), [2.28.1 release notes](https://github.com/timescale/timescaledb/releases/tag/2.28.1), and [2.28.2 release notes](https://github.com/timescale/timescaledb/releases/tag/2.28.2).
</ChangelogEntry>

## Chunk timeline visualization

<ChangelogEntry
  title="Chunk timeline visualization"
  date="2026-07-01"
  tags={["improvement"]}
>
<h2 slot="heading" class="changelog-entry__title">Chunk timeline visualization</h2>

You can now see your chunks visualized in the Explorer for each hypertable. The chart shows a timeline view of the hypertable's chunks, their relative sizes, and their composition (rowstore/columnstore). By default, a max of 100 of the latest chunks are shown. Click and drag on the chart to zoom in on a specific time range. It also shows the status of the latest runs for the associated columnstore job and lets you jump directly to the job deep dive page for that hypertable.

<Image src={imgChunkTimelineTigerCloud} alt="Chunk timeline panel in Tiger Cloud showing compression stats and rowstore versus columnstore chunk sizes over time" />

</ChangelogEntry>

## Updated service creation flow on Tiger Cloud

<ChangelogEntry
  title="Updated service creation flow on Tiger Cloud"
  date="2026-07-01"
  tags={["improvement"]}
>
<h2 slot="heading" class="changelog-entry__title">Updated service creation flow on Tiger Cloud</h2>

We have updated the service creation flow in the Console to make it clearer and easier to use:
- Fewer steps: We have reduced the number of steps down to three.
- Persistent price display: You can see the price as you change various configurations, to better explore your options.
- Updated GUI: The overall design is easier to navigate and parse.

It is now the default in Tiger Cloud, so check it out.
...
<ChangelogEntry 
  title="Automated chunk tuning is now GA"
  date="2026-06-26"
  tags={["ga"]}
>

<h2 slot="heading" class="changelog-entry__title">Automated chunk tuning is now GA</h2>

Automated chunk tuning is now GA for all services. Enable the feature for each hypertable through the Explorer without having to request access.

When enabled, Tiger Cloud continuously evaluates your hypertable and adjusts the interval on your behalf when it detects it is no longer optimal. Note that updated chunk intervals apply to new chunks only.

<ThemeImage light={imgAutomatedChunkTuningLight} dark={imgAutomatedChunkTuningDark} alt="Chunk auto-tuning in Tiger Console" />

See the [docs](/build/performance-optimization/improve-hypertable-performance#automated-tuning) for details.
...
<ChangelogEntry
  title="New Azure regions for Tiger Cloud services"
  date="2026-06-24"
  tags={["new-feature"]}
>
<h2 slot="heading" class="changelog-entry__title">New Azure regions for Tiger Cloud services</h2>
...
See all available regions in [Supported regions](/learn/tiger-cloud/regions).
...
<ChangelogEntry
  title="Updates to the Goals flow in Tiger Console"
  date="2026-06-09"
  tags={["improvement"]}
>
<h2 slot="heading" class="changelog-entry__title">Updates to the Goals flow in Tiger Console</h2>

We have updated the Goals flow Actions in Tiger Cloud to make them more effective and easier to use.

- **Add columns from the UI:** The Action for creating a hypertable now lets you define columns directly in the Console instead of writing SQL.
- **Set up columnstore at creation time:** You can configure a columnstore for your hypertable as part of the same creation flow, without a separate step.

<Image src={imgCreateHypertable1} alt="Create a hypertable wizard" />

<Image src={imgCreateHypertable2} alt="Completed Create a hypertable Action" />
...
<ChangelogEntry
  title="TimescaleDB v2.27 now on Tiger Cloud"
  date="2026-05-19"
  tags={["new-feature"]}
>
<h2 slot="heading" class="changelog-entry__title">TimescaleDB v2.27 now on Tiger Cloud</h2>
...
- **Continuous aggregates**
    ...
    - **[Compress as part of the continuous aggregate refresh policy](/build/continuous-aggregates/compression-on-continuous-aggregates#convert-refreshed-chunks-to-the-columnstore-during-refresh):** The CAgg refresh policy can now compress chunks of the materialization hypertable that fall within the refresh window as part of the same job, consolidating what used to require a separate columnstore policy.
        ...
        SELECT alter_job(1001, config => jsonb_set(
          (SELECT config FROM _timescaledb_catalog.bgw_job WHERE id = 1001), 
...
- 2.27 uses a new naming convention for composite bloom filter metadata. Queries continue to work, but composite bloom filters created in 2.26 won't be used until a lightweight, catalog-only [migration script](https://github.com/timescale/timescaledb-extras/blob/main/utils/2.27.x-fix-composite-bloom-columns.sql) renames the legacy columns (no data recompression required).

### Upcoming PostgreSQL 15 EOL

As announced in the [v2.23.0 changelog](https://github.com/timescale/timescaledb/releases/tag/2.23.0) on October 29, 2025, the upcoming TimescaleDB release in June 2026 will be the last version supporting PostgreSQL 15. If you are still on PostgreSQL 15, plan an upgrade to PostgreSQL 16 or higher to maintain access to new features, bug fixes, and performance improvements.

For complete details, refer to the [TimescaleDB 2.27.0 release notes](https://github.com/timescale/timescaledb/releases/tag/2.27.0).
</ChangelogEntry>

## New I/O Boost options and extended storage capacity

<ChangelogEntry
  title="New I/O Boost options and extended storage capacity"
  date="2026-05-11"
  tags={["new-feature", "performance"]}
>

<h2 slot="heading" class="changelog-entry__title">New I/O Boost options and extended storage capacity</h2>
...
For full details, see [Manage storage and tiering](/build/data-management/storage/manage-storage).

<Image src={imgHighPerformanceStorageTigerConsole} alt="High-performance storage options with I/O Boost in Tiger Console" />
...
<ChangelogEntry
  title="Cross-region restore is now available on Enterprise"
  date="2026-05-06"
  tags={["new-feature"]}
>
<h2 slot="heading" class="changelog-entry__title">Cross-region restore is now available on Enterprise</h2>
...
To learn more see [cross-region backup and restore](/deploy/tiger-cloud/tiger-cloud-aws/high-availability/backup-restore#enable-cross-region-backup).

<Image src={imgCreateCrossRegionBackupInTigerConsole} alt="Enabling cross-region backup in Tiger Console" />
...
<ChangelogEntry
  title="Logs page revamped with search, filters, histogram view, and more"
  date="2026-04-29"
  tags={["new-feature", "improvement"]}
>
<h2 slot="heading" class="changelog-entry__title">Logs page revamped with search, filters, histogram view, and more</h2>
...
<Image src={imgTigerConsoleServiceLogs} alt="Redesigned Logs page in Tiger Console with search, severity filters, and histogram view" />

<Image src={imgTigerCloudDownloadLogs} alt="Download logs as CSV from the Tiger Console Logs page" />
...
## Dark mode now available on Tiger Cloud
...
<ChangelogEntry
  ...
  tags={["new-feature"]}
>
<h2 slot="heading" class="changelog-entry__title">Dark mode now available on Tiger Cloud</h2>
...
## TimescaleDB v2.26.3 now on Tiger Cloud
...
<ChangelogEntry
  ...
  tags={["bug-fix"]}
>
<h2 slot="heading" class="changelog-entry__title">TimescaleDB v2.26.3 now on Tiger Cloud</h2>
...
For complete details, refer to the [TimescaleDB 2.26.3 release notes](https://github.com/timescale/timescaledb/releases/tag/2.26.3).
</ChangelogEntry>

## TimescaleDB v2.26.2 now on Tiger Cloud
...
<ChangelogEntry
  ...
  tags={["bug-fix"]}
>
<h2 slot="heading" class="changelog-entry__title">TimescaleDB v2.26.2 now on Tiger Cloud</h2>
...
For complete details, refer to the [TimescaleDB 2.26.2 release notes](https://github.com/timescale/timescaledb/releases/tag/2.26.2).
</ChangelogEntry>
...
## TimescaleDB v2.26.1 now on Tiger Cloud
...
<ChangelogEntry
  ...
  tags={["bug-fix"]}
>
<h2 slot="heading" class="changelog-entry__title">TimescaleDB v2.26.1 now on Tiger Cloud</h2>
...
For complete details, refer to the [TimescaleDB 2.26.1 release notes](https://github.com/timescale/timescaledb/releases/tag/2.26.1).
</ChangelogEntry>
...
<ChangelogEntry
  ...
  tags={["new-feature"]}
>
<h2 slot="heading" class="changelog-entry__title">TimescaleDB v2.26 now on Tiger Cloud</h2>
...
To learn more, see [Integrate Azure Monitor with Tiger Cloud](/integrate/observability-alerting/azure-monitor).
</ChangelogEntry>
...
<ThemeImage light={imgAutomatedChunkTuningLight} dark={imgAutomatedChunkTuningDark} alt="Chunk auto-tuning in Tiger Cloud" />
...
<ChangelogEntry 
  title="PostgreSQL 18 support"
  date="2026-01-13"
  tags={["new-feature"]}
>
<h2 slot="heading" class="changelog-entry__title">PostgreSQL 18 support</h2>
...
<ChangelogEntry 
  title="Enhancements to the PostgreSQL source connector"
  date="2025-08-28"
  tags={["improvement"]}
>
<h2 slot="heading" class="changelog-entry__title">Enhancements to the PostgreSQL source connector</h2>
### What's new
...
<ChangelogEntry 
  title="Cross-region backups, PostgreSQL options, and onboarding"
  date="2025-08-14"
  tags={["new-feature", "security"]}
>
<h2 slot="heading" class="changelog-entry__title">Cross-region backups, PostgreSQL options, and onboarding</h2>
### Cross-region backups
...
<ChangelogEntry 
  title="Enhanced service monitoring, TimescaleDB v2.20, and livesync for PostgreSQL"
  date="2025-05-30"
  tags={["performance", "new-feature", "deprecation"]}
>
<h2 slot="heading" class="changelog-entry__title">Enhanced service monitoring, TimescaleDB v2.20, and livesync for PostgreSQL</h2>
...
**Breaking change:** on continuous aggregates, `timescaledb.chunk_time_interval` is renamed to `timescaledb.chunk_interval`. The old name is rejected, not aliased, so statements using it fail after the upgrade.
...
<ChangelogEntry 
  title="In-Console Livesync for PostgreSQL"
  date="2025-03-21"
  tags={["new-feature"]}
>
<h2 slot="heading" class="changelog-entry__title">In-Console Livesync for PostgreSQL</h2>
### Set up in Console
...
<ChangelogEntry 
  title="New region in India, PostgreSQL 17 upgrades, and TimescaleDB on AWS Marketplace"
  date="2025-01-10"
  tags={["region", "new-feature"]}
>
<h2 slot="heading" class="changelog-entry__title">New region in India, PostgreSQL 17 upgrades, and TimescaleDB on AWS Marketplace</h2>
...
<ChangelogEntry 
  title="PostgreSQL 17, feature requests, and PostgreSQL Livesync"
  date="2024-12-20"
  tags={["new-feature", "beta"]}
>
<h2 slot="heading" class="changelog-entry__title">PostgreSQL 17, feature requests, and PostgreSQL Livesync</h2>
### PostgreSQL 17
