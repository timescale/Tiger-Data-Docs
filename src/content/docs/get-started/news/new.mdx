...
## Cross-region restore is now available on Enterpise
...
Cross-region backup protects data by copying backups to a separate, geographically distant region. Users subscribed to the Enterprise plan can now restore from a cross-region backup using the Tiger Cloud console, ensuring business continuity in the event of a regional outage.

To learn more see [cross-region backup and restore](https://www.tigerdata.com/docs/deploy/tiger-cloud/tiger-cloud-aws/high-availability/backup-restore#enable-cross-region-backup).
