...
3. click `Download the config`
4. expect label `Ready`
5. run command: `psql "<your-service-url>"`
6. run SQL: `SELECT CURRENT_DATE;`
7. exit psql
8. click `SQL Editor`
9. run SQL: `SELECT CURRENT_DATE;`
10. expect rows: `SELECT 1 FROM pg_extension WHERE extname = 'timescaledb'`

Not scripted:
- Connect to your service, Data view tab: the Data view is PopSQL in a cross-origin iframe the
  walker cannot reach into. The same documented test query is driven through the SQL Editor tab and
  the psql tab, so only the route goes untested.
