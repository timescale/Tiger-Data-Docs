...
6. expect label `Ready`

Not scripted:
- Connect to your service: the steps after the service is created would run against the STANDING
  service, not the one just created. The tool adopts a created service as the target only between
  procedures, and this page has one, so a `SELECT` here reports a pass for a database the page never
  told anyone to open. That is a false pass, not thin coverage, so the whole procedure is left out
  until the tool can retarget mid-procedure. Its Data view tab is separately undrivable: PopSQL in a
  cross-origin iframe.
