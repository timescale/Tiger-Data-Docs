...
<Steps>
  ...
  <Step title="Import some time-series data into hypertables">
  ...
  <Step title="Have a quick look at your data">
...
<Steps>
  ...
  <Step title="Create a policy to refresh the view every hour">
   ```sql
...
<Steps>
  ...
  <Step title="Enable data tiering">
  ...
  <Step title="Set the time interval when data is tiered">
...
<Steps>
  ...
  <Step>In [Tiger Console][cloud-login], select the service to enable replication for.</Step>
  <Step>Click `Operations`, then select `High availability`.</Step>
  <Step>
