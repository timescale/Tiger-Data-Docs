...
keywords: [replicas, replication, streaming]
---
...
{C.TIMESCALE_DB} running. If you installed TimescaleDB using a Docker container, use
a [{C.PG} entry point script](https://hub.docker.com/_/postgres/) to run the
configuration. For more advanced examples, see the
[{C.TIMESCALE_DB} Helm Charts repository](https://github.com/timescale/helm-charts/tree/main/charts/timescaledb-single).

To configure replication on {C.SELF_LONG}, you need to perform these
procedures:

1.  [Configure the primary database](#configure-the-primary-database)
1.  [Configure replication parameters](#configure-replication-parameters)
1.  [Create replication slots](#create-replication-slots)
1.  [Configure host-based authentication parameters](#configure-host-based-authentication-parameters)
1.  [Create a base backup on the replica](#create-a-base-backup-on-the-replica)
1.  [Configure replication and recovery settings](#configure-replication-and-recovery-settings)
1.  [Verify that the replica is working](#verify-that-the-replica-is-working)

## Configure the primary database

To configure the primary database, you need a {C.PG} user with a role that
allows it to initialize streaming replication. This is the user each replica
uses to stream from the primary database.

<NumberedList>
<NumberedItem title="Set the password encryption level">
...
<NumberedItem title="Create a new user called repuser">
...
password-based authentication available in {C.PG}. It is only available in {C.PG} 10 and later.
</Callout>
</NumberedItem>
</NumberedList>
...
<NumberedList>
<NumberedItem title="Set the synchronous_commit parameter to off">
</NumberedItem>
<NumberedItem title="Set the max_wal_senders parameter">
Set it to the total number of concurrent
connections from replicas or backup clients. As a minimum, this should equal
the number of replicas you intend to have.
</NumberedItem>
<NumberedItem title="Set the wal_level parameter">
Set it to the amount of information written to the
{C.PG} write-ahead log (WAL). For replication to work, there needs to be
enough data in the WAL to support archiving and replication. The default
value is usually appropriate.
</NumberedItem>
<NumberedItem title="Set the max_replication_slots parameter">
Set it to the total number of replication
slots the primary database can support.
</NumberedItem>
<NumberedItem title="Set the listen_addresses parameter">
Set it to the address of the primary database.
...
<NumberedItem title={`Restart ${C.PG} to pick up the changes`}>
...
When you have configured `postgresql.conf` and restarted {C.PG}, you can
create a [replication slot](https://www.postgresql.org/docs/current/static/warm-standby.html#STREAMING-REPLICATION-SLOTS) for each replica. Replication
...
[archiving](https://www.postgresql.org/docs/current/continuous-archiving.html) for this purpose, but replication slots
provide the strongest protection for streaming replication.

<NumberedList>
<NumberedItem title="Create the first replication slot">
At the `psql` prompt, create the first replication slot. The name of the slot
...
<NumberedItem title="Repeat for each required replication slot">
</NumberedItem>
</NumberedList>
...
{C.PG} user `repuser` with a valid password. `REPLICATION_HOST_IP` can
...
[`pg_hba` documentation](https://www.postgresql.org/docs/current/auth-pg-hba-conf.html).

<NumberedList>
<NumberedItem title="Open the pg_hba.conf configuration file">
...
transactions in {C.PG} recovery mode. To do this, the replica needs to be in
a state where it can replay the log. You can do this by restoring the replica
from a base backup of the primary instance.

<NumberedList>
<NumberedItem title={`Stop ${C.PG} services`}>
</NumberedItem>
<NumberedItem title="Delete the existing data if applicable">
If the replica database already contains data, delete it before you run the
backup, by removing the {C.PG} data directory:
...
<NumberedItem title="Restore from the base backup">
Use the IP address of the primary database
...
automated setup, you might need to use a [pgpass file](https://www.postgresql.org/docs/current/libpq-pgpass.html).
</NumberedItem>
<NumberedItem title="Create a standby.signal file">
When the backup is complete, create a
[standby.signal](https://www.postgresql.org/docs/current/runtime-config-wal.html#RUNTIME-CONFIG-WAL-ARCHIVE-RECOVERY) file in your data directory. When
{C.PG} finds a `standby.signal` file in its data directory, it starts in
...
<NumberedList>
<NumberedItem title="Add primary server details to the replica">
In the replica's `postgresql.conf` file, add details for communicating with the
...
<NumberedItem title="Mirror the configuration of the primary database">
If you are
...
on the replica. In {C.PG} 10 and later, this setting is `on` by default.
</NumberedItem>
<NumberedItem title={`Restart ${C.PG} to pick up the changes`}>
</NumberedItem>
</NumberedList>
...
might not provide enough of a durability guarantee. The {C.PG}
[`synchronous_commit`](https://www.postgresql.org/docs/current/runtime-config-wal.html#GUC-SYNCHRONOUS-COMMIT) feature has several
...
*   `off`: The server returns `success` when the WAL transaction has been sent
    ...
    `synchronous_commit` off is a well-known {C.PG} optimization for
...
The {C.PG} [pg_stat_replication](https://www.postgresql.org/docs/current/monitoring-stats.html#MONITORING-PG-STAT-REPLICATION-VIEW) view
...
{C.PG} provides some failover functionality, where the replica is promoted
to primary in the event of a failure. This is provided using the
[pg_ctl](https://www.postgresql.org/docs/current/static/app-pg-ctl.html) command or the `trigger_file`. However, {C.PG} does
not provide support for automatic failover. For more information, see the
[{C.PG} failover documentation](https://www.postgresql.org/docs/current/static/warm-standby-failover.html). If you require a
configurable high availability solution with automatic failover functionality,
check out [Patroni](https://github.com/zalando/patroni).