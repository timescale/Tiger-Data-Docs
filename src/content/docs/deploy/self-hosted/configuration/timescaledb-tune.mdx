...
description: Use timescaledb-tune to automatically configure your TimescaleDB instance based on your system's resources
...
To help make configuring {C.TIMESCALE_DB} a little easier, you can use the [`timescaledb-tune`](https://github.com/timescale/timescaledb-tune)
tool. This tool handles setting the most common parameters to good values based
on your system. It accounts for memory, CPU, and {C.PG} version.
`timescaledb-tune` is packaged with the {C.TIMESCALE_DB} binary releases as a
dependency, so if you installed {C.TIMESCALE_DB} from a binary release (including
