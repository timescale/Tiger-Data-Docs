...
description: Choose how to migrate your existing PostgreSQL database to your self-hosted TimescaleDB installation
...
You can migrate your existing {C.PG} database to {C.SELF_LONG}.

There are several methods for migrating your data:

*   If the database you want to migrate is smaller than 100&nbsp;GB,
    [migrate your entire database at once](/deploy/self-hosted/migration/entire-database):
    ...
    policies are automatically available in the new {C.SELF_LONG} instance.
*   For databases larger than 100GB,
    [migrate your schema and data separately](/deploy/self-hosted/migration/schema-then-data): With this
...
*   If you need to move data from {C.PG} tables into hypertables within an
    existing {C.SELF_LONG} instance,
    [migrate within the same database](/deploy/self-hosted/migration/same-db): This method assumes that
    you have {C.TIMESCALE_DB} set up in the same database instance as your existing table.
*   If you have data in an InfluxDB database,
    [migrate using Outflux](/deploy/self-hosted/migration/migrate-influxdb):
    Outflux pipes exported data directly to your {C.SELF_LONG} instance, and manages schema
...
your {C.SELF_LONG} instance contains the data that is ingested while the migration
