...
description: Save on data storage by moving older chunks to cheaper, slower storage, while still using faster, more expensive storage for frequently accessed data
products: [self_hosted]
keywords: [storage, tablespaces, move, chunks]
---
...
If you are running {C.TIMESCALE_DB} on your own hardware, you can save storage
...
Using tablespaces is one way to manage data storage costs with {C.TIMESCALE_DB}. You
can also use [compression](/learn/columnar-storage/understand-hypercore) and
[data retention](/learn/data-management/data-retention/about-data-retention) to reduce
...
[`move_chunk`](/reference/timescaledb/hypertables/move_chunk) API call to move individual chunks from the
...
[`reorder_chunk` command](/reference/timescaledb/hypertables/reorder_chunk).
...
<NumberedList>
<NumberedItem title="Create a new tablespace">
In this example, the tablespace is called
...
<NumberedItem title="List chunks that you want to move">
In this example, chunks that contain data
...
<NumberedItem title="Move a chunk and its index to the new tablespace">
You can also reorder the
...
<NumberedItem title="Verify that the chunk now resides in the correct tablespace">
Query `pg_tables` to list all of the chunks on the tablespace:
...
In {C.TIMESCALE_DB} v2.0 and later, you can use `move_chunk` with the job scheduler
framework. For more information, see the [jobs section](/reference/timescaledb/jobs-automation).