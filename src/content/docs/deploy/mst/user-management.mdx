...
description: Add and manage users in your Managed Service for TimescaleDB project
products: [mst]
keywords: [users, admin, authentication, multi-factor, roles]
tableOfContents:
  minHeadingLevel: 2
  maxHeadingLevel: 2
---
...
import imgCreateServiceUser from "../../../../assets/images/deploy/mst/create-service-user.png";

You can add new users, and manage existing users, in {C.MST_CONSOLE_LONG}. New users can be added to an entire project, or a single
{C.MST_SERVICE_SHORT}.
...
<NumberedItem title="Sign in to the MST Console">

[Sign in](https://portal.managed.timescale.com/login) to your {C.MST_CONSOLE_LONG}.

</NumberedItem>

<NumberedItem title="Navigate to `Members`">
...
<NumberedItem title="Add a new member">

In the `Project members` page, type the email address of the member you want
to add, and select a role for the member.

</NumberedItem>

<NumberedItem title="Send the invitation">

Click `Send invitation`. The new user is sent an email inviting them to the project, and the invite
...
<NumberedItem title="Manage members">

When they accept the invitation, the user details show in the `Members`
...
The `tsdbadmin` user for {C.MST_LONG}s can:
...
[{C.PG} role attributes documentation](https://www.postgresql.org/docs/current/role-attributes.html).
...
[Sign in](https://portal.managed.timescale.com/login) to {C.MST_CONSOLE_LONG}. By
...
<NumberedItem title="Add a service user">

Select `Users`, then click `Add service user`:

<Image src={imgCreateServiceUser} alt="Adding a new MST service user" />

</NumberedItem>

<NumberedItem title="Configure the user">

In the `Username` field, type a name for your user. If you want to allow
the user to be replicated, toggle `Allow replication`. Click
`Add service user` to save the user.

</NumberedItem>

<NumberedItem title="View the new user">

The new user shows in the `Username` list.
...
You can use multi-factor authentication (MFA) to log in to {C.MST_CONSOLE_LONG}. This requires an authentication code, provided by the
Google Authenticator app on your mobile device.

You can see which authentication method is in use by each member of your {C.MST_LONG} project. From the dashboard, navigate to the `Members`
...
[the Google Authenticator documentation](https://support.google.com/accounts/answer/1066447).
...
[Sign in](https://portal.managed.timescale.com/login) to {C.MST_CONSOLE_LONG}.

</NumberedItem>

<NumberedItem title="Open `User profile`">

Click the `User information` icon in the top-right of the dashboard to go to
the `User profile` section.

</NumberedItem>

<NumberedItem title="Enable two-factor authentication">

In the `Authentication` tab, toggle `Two-factor authentication` to
`Enabled`, and enter your password.

</NumberedItem>

<NumberedItem title="Scan the QR code">

On your mobile device, open the Google Authenticator app, tap `+` and select
`Scan a QR code`. Scan the QR code provided by {C.MST_LONG}.

</NumberedItem>

<NumberedItem title="Confirm the code">

In your {C.MST_SHORT} dashboard, enter the confirmation
...
authentication, you cannot sign in to your {C.MST_LONG}
...
Every time a registered user logs in, {C.MST_LONG} creates a
...
You can see the list of all current authentication tokens in the {C.MST_LONG} dashboard. Sign in to your account, and click the
...
use them with monitoring applications that make automatic API calls to {C.MST_LONG}.
