---
title: Backups
description: Back up your services running in the Managed Service for TimescaleDB with alternative tools
products: [mst]
keywords: [backups, restore, pg_dump]
tableOfContents:
  minHeadingLevel: 2
  maxHeadingLevel: 2
---
...
{C.MST_SERVICE_LONG}s are automatically backed up, with full
backups daily, and write-ahead log (WAL) continuously recorded. All backups are
[encrypted](https://aiven.io/docs/platform/concepts/cloud-security#data-encryption).

{C.MST_LONG} uses [`pghoard`](https://github.com/aiven/pghoard), a {C.PG} backup
...
The size of logical backups can be different from the size of the {C.MST_LONG} backup that appears on the web console. In some cases,
the difference is significant. Backup sizes that appear in the {C.MST_CONSOLE_SHORT} are for daily backups, before encryption and
...
| Data | Contains deleted and updated rows which have not been cleaned up by {C.PG} VACUUM process, and all databases, including templates | Does not contain any data already deleted, and depending on the options given, the output might be compressed |

## Restore a service

{C.MST_LONG} provides a point-in-time recovery (PITR). To
restore your service from a backup, click the `Restore` button in the `Backups`
tab for your service. The backups are taken automatically by {C.MST_LONG} and retained for a few days depending on your plan type.
