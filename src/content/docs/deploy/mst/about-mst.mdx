---
title: About MST
description: Set up and manage your services in Managed Service for TimescaleDB
products: [mst]
keywords: [services, mst, managed service, projects, databases, high availability, configuration plans]
tableOfContents:
  minHeadingLevel: 2
  maxHeadingLevel: 2
---
...
import CloudMSTComparison from "../../../../partials/_cloud-mst-comparison.mdx";

import { Image } from "astro:assets";
import imgCreateProject from "../../../../assets/images/deploy/mst/create-project.png";
import imgServices from "../../../../assets/images/deploy/mst/services.png";
import imgCreateDatabase from "../../../../assets/images/deploy/mst/create-database.png";
import imgServicePlans from "../../../../assets/images/deploy/mst/service-plans.png";

<MSTIntro />

<CloudMSTComparison />

Your {C.MST_LONG} account has three main components:
projects, {C.MST_SERVICE_SHORT}s, and databases.

## Projects

When you [sign up for {C.MST_LONG}](https://www.tigerdata.com/mst-signup), an empty project is
...
groups of {C.MST_SERVICE_SHORT}s. Each project can also have its own billing settings.

To create a new project: In [{C.MST_CONSOLE_SHORT}](https://portal.managed.timescale.com/login), click `Projects` > `Create project`.

<Image src={imgCreateProject} alt="Creating a project in MST portal" />

## {C.MST_SERVICE_LONG}s

Each project contains one or more {C.MST_SERVICE_SHORT}s. You can have multiple {C.MST_SERVICE_SHORT}s under
each project, and each {C.MST_SERVICE_SHORT} corresponds to a cloud service provider tier. You
can access all your {C.MST_SERVICE_SHORT}s from the `Services` tab within your projects.

<Image src={imgServices} alt="MST services list" />

For more information about getting your first {C.MST_SERVICE_SHORT} up and running, see the
[{C.MST_LONG} installation section](/deploy/mst/create-mst-service).

<Callout variant="warning">

When you have created, and named, a new {C.MST_SERVICE_LONG},
...
[migrating your data](/deploy/mst/migrate-to-mst).

</Callout>

For information about billing on {C.MST_LONG}, see the
[billing section](/deploy/mst/billing).
...
<Image src={imgCreateDatabase} alt="Creating a database in MST portal" />

## Service level agreement

{C.MST_LONG} is provided through a partnership with Aiven.
This provides you with a service commitment to deliver 99.99% availability. For
more information, see the
[Aiven Service Level Agreement policy](https://aiven.io/sla).
...
no data loss, and the possible downtime is minimal. If an acting {C.TIMESCALE_DB}
...
<Image src={imgServicePlans} alt="Selecting an MST service configuration plan" />
...
A failing node is automatically detected by the {C.MST_SHORT} monitoring infrastructure.
...
{C.MST_LONG} availability features differ based on the service
plan:
...
In Pro plans, if a {C.PG} standby fails, the master node keeps running
...
If the {C.PG} master fails, the combined information from the {C.MST_SHORT} monitoring
...
information from the {C.MST_SHORT} system infrastructure, reports the failover. If the
...
For backups and restoration, {C.MST_LONG} uses the
open source backup daemon `PGHoard` that {C.MST_SHORT} maintains. It makes real-time
copies of write-ahead log (WAL) files to an object store in a compressed and
encrypted format.

## Connection limits

{C.MST_LONG} limits the maximum number of connections to each
...
[connection pooling section](/deploy/mst/connection-pools).
...
{C.MST_LONG} uses the default keep alive settings for TCP
...
[{C.PG} documentation](https://www.postgresql.org/docs/9.5/libpq-connect.html#LIBPQ-KEEPALIVES).

## Long running queries

{C.MST_LONG} does not cancel database queries. If you
