---
title: Failover
description: Learn how failover to a read-only replica ensures business continuity for your services running in Managed Service for TimescaleDB
products: [mst]
keywords: [failover, high availability, replica, maintenance, upgrades]
tableOfContents:
  minHeadingLevel: 2
  maxHeadingLevel: 2
---
...
One standby read-only replica server is configured, for each {C.MST_SERVICE_LONG} on a Pro plan. You can query a read-only replica server, but cannot
...
There is a 300 second timeout before {C.MST_LONG}
