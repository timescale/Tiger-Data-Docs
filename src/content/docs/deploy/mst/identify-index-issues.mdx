...
description: Identify and resolve issues with Postgres indexes in Managed Service for TimescaleDB
products: [mst]
keywords: [index, reindex, postgresql]
tableOfContents:
  minHeadingLevel: 2
  maxHeadingLevel: 2
---
...
{C.PG} indexes can be corrupted for a variety of reasons, including
...
For more information on the `REINDEX` command, see the [{C.PG} documentation](https://www.postgresql.org/docs/current/sql-reindex.html).
