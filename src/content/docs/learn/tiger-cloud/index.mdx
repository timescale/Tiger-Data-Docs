...
{C.CLOUD_LONG} runs {C.TIMESCALE_DB} on {C.PG} as a managed service. You use {C.HYPERTABLE}s, {C.HYPERCORE_CAP},
{C.CAGG}s, and {C.HYPERFUNC} as usual, while backups, upgrades, scaling, and infrastructure are handled for you,
along with tiered storage, high-availability and read replicas, and observability in {C.CONSOLE}.
