...
description: Group data by time interval using the time_bucket function to perform aggregate calculations over arbitrary time intervals
products: [cloud, mst, self_hosted]
---

import * as C from "@constants";

The `time_bucket` function helps you group data in a [{C.HYPERTABLE}](/learn/hypertables/understand-hypertables) so you can
...
This section shows examples of `time_bucket` use. To learn how {C.TIME_BUCKET}s
work, see the [about {C.TIME_BUCKET}s section](/learn/data-management/time-buckets/about-time-buckets).

## Group data by time buckets and calculate a summary value

Group data into {C.TIME_BUCKET}s and calculate a summary value for a column. For
...
{C.TIME_BUCKET_CAP}s are usually used together with `GROUP BY` to aggregate data. But
...
For example, to see the 1-week {C.TIME_BUCKET} into which January 5, 2021 would
fall, run:
...
The function returns `2021-01-04 00:00:00`. The start time of the {C.TIME_BUCKET} is
the Monday of that week, at midnight.

## Learn more

- [Understand {C.TIME_BUCKET}s](/learn/data-management/time-buckets/about-time-buckets): How
  {C.TIME_BUCKET}s work, origins, and timezones.
...
- [Troubleshoot {C.TIME_BUCKET}s](/build/tips-and-tricks/troubleshoot-time-buckets): Common
