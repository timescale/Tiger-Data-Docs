...
keywords: [finance, analytics, websockets, data pipeline, real-time data ingestion, Twelve Data, stocks, cryptocurrency, Python]
---

import * as C from "@constants";
import { NumberedList, NumberedItem } from "@components/NumberedList";
import FinancialIndustryIntro from "../../../../partials/_financial-industry-data-analysis.mdx";
import CandlestickIntro from "../../../../partials/_candlestick_intro.mdx";
import CreateHypertableCrypto from "../../../../partials/_create-hypertable-twelvedata-crypto.mdx";
import GrafanaConnect from "../../../../partials/_grafana-connect.mdx";
import GraphOhlcv from "../../../../partials/_graphing-ohlcv-data.mdx";
import IntegrationPrereqs from "../../../../partials/_prereqs-cloud-and-self.mdx";
import { Prerequisites } from "@components/Prerequisites";

import { Image } from "astro:assets";
import imgCandlestickFig from "../../../../assets/images/learn/tutorials/candlestick_fig.png";

<FinancialIndustryIntro />

This tutorial shows you how to ingest real-time time-series data into
{C.TIMESCALE_DB} using a websocket connection. The tutorial sets up a data pipeline
to ingest real-time data from our data partner, [Twelve Data](https://twelvedata.com).
...
up that ingests real-time financial data into your {C.CLOUD_LONG}.

This tutorial uses Python and the API
[wrapper library](https://github.com/twelvedata/twelvedata-python) provided by Twelve Data.

This tutorial covers:

1.  **Set up your dataset**: connect to the Twelve Data websocket server, create {C.HYPERTABLE}s, and ingest real-time cryptocurrency data.
1.  **Query your data**: create {C.CAGG}s to aggregate OHLCV data, query the aggregated data, and visualize the data in Grafana.
...
* Python 3 installed.
* A [Twelve Data](https://twelvedata.com/pricing) account. The free tier is perfect for this tutorial.
* Your Twelve Data [API key](https://twelvedata.com/account/api-keys).
</Prerequisites>
...
<Image src={imgCandlestickFig} alt="Candlestick chart showing intraday stock price movements" />

{C.TIMESCALE_DB} is well suited to storing and analyzing financial candlestick data,
and many {C.COMPANY} community members use it for exactly this purpose.

## Ingest data into a {C.SERVICE_LONG}

This tutorial uses a dataset that contains second-by-second cryptocurrency trade data,
in a {C.HYPERTABLE} named `crypto_ticks`. It also includes a separate table of
cryptocurrency symbols and names, in a regular {C.PG} table named `crypto_assets`.
...
<NumberedItem title="Create and activate a Python virtual environment">
...
<NumberedItem title="Install the Twelve Data Python wrapper library with websocket support">

This [library](https://github.com/twelvedata/twelvedata-python) allows you to make requests to the
...
<NumberedItem title="Install Psycopg2">

[Psycopg2](https://www.psycopg.org/docs/) enables you to connect to
{C.TIMESCALE_DB} from your Python script:
...
<NumberedItem title="Create a new Python file called `websocket_test.py`">

Connect to the Twelve Data servers using the `<YOUR_API_KEY>`:
...
<NumberedItem title="Run the Python script">
...
When you ingest data into a transactional database like {C.TIMESCALE_DB}, it is more
...
ingest capacity and speed of your {C.SERVICE_LONG}.
...
Using batching is a fairly common pattern when ingesting data into {C.TIMESCALE_DB}
from Kafka, Kinesis, or websocket connections.

To ingest the data into your {C.SERVICE_LONG}, you need to implement the
`on_event` function.
...
financial data into your {C.SERVICE_LONG}.
...
<NumberedItem title="Update the Python script">

Update the script to print out the current batch size, so you can
follow when data gets ingested from memory into your database. Use
the `<HOST>`, `<PASSWORD>`, and `<PORT>` details for the {C.SERVICE_LONG}
...
To look at OHLCV values, the most effective way is to create a {C.CAGG}. You can create a {C.CAGG} to aggregate data
...
<NumberedItem title={`Connect to the ${C.SERVICE_LONG}`}>

Connect to the service that contains the Twelve Data cryptocurrency dataset.

</NumberedItem>

<NumberedItem title={`Create the ${C.CAGG} to aggregate data every day`}>
...
When you create the {C.CAGG}, it refreshes by default.

</NumberedItem>

<NumberedItem title={`Set a refresh policy to update the ${C.CAGG} every day`}>

This refreshes if there is new data available in the {C.HYPERTABLE} for the last two days:
...
When you have your {C.CAGG} set up, you can query it to get the
OHLCV values.
...
<NumberedItem title="Select all Bitcoin OHLCV data for the past 14 days">
