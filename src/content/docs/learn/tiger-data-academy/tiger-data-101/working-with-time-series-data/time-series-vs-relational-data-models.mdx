...
```sql
-- A time-series table (hypertable in {C.TIMESCALE_DB})
...
The same database can hold both! {C.TIMESCALE_DB} is a {C.PG} extension, so you can store relational data (users, orders, metadata) **and** time-series data (metrics, events, sensor readings) in the same instance, with each optimized appropriately.
</Callout>

---

<div style="display: flex; justify-content: flex-end; margin-top: 40px;">
  <a href="/learn/tiger-data-academy/tiger-data-101/working-with-time-series-data/common-time-series-patterns" style="text-decoration: none; display: block; width: 100%;">
    <div style="background: linear-gradient(135deg, #4f6bed 0%, #667eea 100%); border-radius: 8px; padding: 20px; color: white; box-shadow: 0 4px 12px rgba(79, 107, 237, 0.15); cursor: pointer; transition: all 0.2s ease; text-align: right;">
      ...
      <div style="color: #e0e7ff; font-weight: 500; font-size: 15px; margin-bottom: 8px; margin-top: 0;">Common Time-Series Patterns</div>
