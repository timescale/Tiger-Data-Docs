...
description: Understand the compression algorithms used when converting data from the rowstore to the columnstore
products: [cloud, mst, self_hosted]
---

import * as C from "@constants";

Depending on the data type that is compressed when your data is converted from the {C.ROWSTORE} to the
{C.COLUMNSTORE}, {C.TIMESCALE_DB} uses the following compression algorithms:

- **Integers, timestamps, boolean and other integer-like types**: a combination of the following compression
  methods is used: [delta encoding](#delta-encoding), [delta-of-delta](#delta-of-delta-encoding), [simple-8b](#simple-8b), and
  [run-length encoding](#run-length-encoding).
- **Columns that do not have a high amount of repeated values**: [XOR-based](#xor-based-compression) compression with
  some [dictionary compression](#dictionary-compression).
- **All other types**: [dictionary compression](#dictionary-compression).

This page gives an in-depth explanation of the compression methods used in {C.HYPERCORE}.

## Integer compression

For integers, timestamps, and other integer-like types {C.TIMESCALE_DB} uses a
...
run-length and Simple-8b techniques. {C.TIMESCALE_DB} implements a variant of
Simple-8b RLE. This variant uses different sizes to standard Simple-8b, in order
to handle 64-bit values, and RLE.

## Floating point compression

For columns that do not have a high amount of repeated values, {C.TIMESCALE_DB} uses
...
For values that are not integers or floating point, {C.TIMESCALE_DB} uses dictionary
compression.
...
nearly 3 times. In {C.TIMESCALE_DB}, the list of indices is compressed even further
...
original data. {C.TIMESCALE_DB} automatically detects this case, and falls back to
not using a dictionary in that scenario.

#### JSONB compression

For JSONB columns, {C.TIMESCALE_DB} uses a two-layer compression approach:

1. **{C.TIMESCALE_DB} dictionary compression**: {C.TIMESCALE_DB} attempts to apply
...
2. **{C.PG} TOAST compression**: if dictionary compression does not perform, {C.TIMESCALE_DB} skips compression and allows {C.PG} to handle the data using
   Oversized-Attribute Storage Technique (TOAST) compression. By default,
   {C.PG} uses `pglz` compression. To configure {C.PG} to use more
...
- [Improve query and upsert performance](/build/performance-optimization/secondary-indexes):
  Tune `segmentby` and `orderby` for best compression and query speed.
...
- [Troubleshoot {C.HYPERCORE}](/build/tips-and-tricks/troubleshoot-hypercore): Common issues
  and solutions.
- [Understand {C.HYPERCORE}](/learn/columnar-storage/understand-hypercore): How the hybrid
  row-columnar engine works.
