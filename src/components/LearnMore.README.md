...
1. [TL;DR — 30-second copy-paste](#tldr--30-second-copy-paste)
...
5. [Variations and recipes](#variations-and-recipes) ← **most authors land here**
   ...
   - [Recipe 6: External vs. internal links](#recipe-6-external-vs-internal-links)
...
**Every field is optional.** If `tutorials`, `relatedPosts`, and `cta` are all absent (or empty), the card hides itself completely — no empty card, no broken layout.
...
| `tutorials` | `{ label, href }[]` | — | Internal tutorial-style links. Render with a right-arrow icon. |
| `tutorialsHeading` | string | `"Tutorials"` | Heading above the tutorials list. |
| `relatedPosts` | `{ label, href }[]` | — | External references (blog posts, articles, third-party docs). Render with an external-link icon. |
| `relatedPostsHeading` | string | `"Related blog posts"` | Heading above the related posts list. |
| `cta` | `{ label, href }` | — | A single outlined button at the bottom of the card. Solid fill on hover/focus. |
...
The full Monty — next tutorials, related blog posts, and a sign-up CTA. Best for end-of-tutorial pages where you want to keep readers in the funnel.
...
learnMore:
  relatedPosts:
    ...
    - label: Hypertables vs. partitioned tables
...
### Recipe 6: External vs. internal links
...
- The right rail and this card both disappear on viewports under 1024px — that's by design. If you need related-content links to surface on mobile, put them in the page body itself (for example, using `RelatedContentCards`).
