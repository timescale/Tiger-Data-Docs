...
1. **Configure your self-hosted {C.PG} deployment**
   ...
   1. Open `pg_hba.conf` and enable host replication.
      ...
      permissions, see [Configuring {C.PG} to allow replication with the Debezium connector host][debezium-replication-permissions].
...
1. **Enable a replication spot for Debezium**
   ...
   1. Create a hypertable for Debezium to listen to:
      ...
      Debezium also works with [{C.CAGG}s][caggs].
