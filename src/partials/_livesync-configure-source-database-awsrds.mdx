...
1. **Tune the Write Ahead Log (WAL) on the RDS/Aurora {C.PG} source database**
   ...
   1. Click `Configuration`, scroll down and note the `DB instance parameter group`, then click `Parameter Groups`
      ...
      src="../assets/images/migrate-awsrds-parameter-groups.webp"
...
1. **Create a user for the {C.PG_CONNECTOR} and assign permissions**
