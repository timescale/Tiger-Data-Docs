...
[Materialized views][pg-materialized views] are a standard {C.PG} function.
...
[Continuous aggregates][caggs] are a {C.TIMESCALE_DB}-only feature. They work in
...
[Real-time aggregates][real-time-aggregates] are a {C.TIMESCALE_DB}-only feature. They are
