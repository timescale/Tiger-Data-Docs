...
<Step title={`In ${C.CONSOLE}, choose the ${C.SERVICE_SHORT}`} />

<Step title="Click `Operations` > `Exporters`" />

<Step title="Select the exporter, then click `Attach exporter`" />

<Step title={`If you are attaching a first \`Logs\` data type exporter, restart the ${C.SERVICE_SHORT}`} />
...
<Step title="Next to the exporter you want to edit, click the menu > `Edit`" />
