...
To create a `_timescaledb_internal.dimension_info` instance, you call [add_dimension](/reference/timescaledb/hypertables/add_dimension) 
...
By default, {C.TIMESCALE_DB} calls {C.PG}'s internal hash function for the given type.
You use a custom partitioning function for value types that do not have a native {C.PG} hash function.
...
Every distinct item in hash partitioning is hashed to one of *N* buckets. By default, {C.TIMESCALE_DB} uses flexible range 
...
- **Multiple tablespaces**: for each physical disk, add a separate tablespace to the database. {C.TIMESCALE_DB} allows you to
...
{C.TIMESCALE_DB} does *not* benefit from a very large number of hash
