...
- **{C.CLOUD_LONG} on Azure**: The same core database and management platform. Private connectivity through Azure Private Link, cross-region backup, and metric export to Azure Monitor (as well as Prometheus and Datadog). Some AWS-specific integrations, such as the S3 and Iceberg connectors, {C.VPC} peering, cross-region restore, and CloudWatch, are not yet available.
