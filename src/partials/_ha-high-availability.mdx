...
<NumberedItem title={`Connect to your primary node as \`tsdbadmin\` or another user that is part of the \`tsdbowner\` group`}>
...
<NumberedItem title="At the `psql` prompt, connect to the `postgres` database">
...
<NumberedItem title="Check if your node is currently in recovery">
...
<NumberedItem title="Check which node is currently your primary">

```sql
select * from pg_stat_replication;
```

Note the `application_name`. This is your {C.SERVICE_SHORT} ID followed by the
node. The important part is the `-an-0` or `-an-1`.

</NumberedItem>

<NumberedItem title="Schedule a switchover">
...
<NumberedItem title="Wait for the switchover to occur, then check which node is your primary">
...
<NumberedItem title="Check the `application_name`">

If your primary was `-an-1` before, it should
