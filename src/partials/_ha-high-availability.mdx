...
import imgTigerCloudHaArchitectureDiagram from "../assets/images/deploy/tiger-cloud/tiger-cloud-ha-architecture-diagram.svg?url";
import imgSyncAsyncReplicationDraft from "../assets/images/deploy/tiger-cloud/sync_async_replication_draft.png";
...
<img src={imgTigerCloudHaArchitectureDiagram} alt="HA replicas in" />
...
<Image src={imgSyncAsyncReplicationDraft} alt="Sync and async replication" />
...
a {C.SERVICE_SHORT} restart. Restarts are run during the [maintenance window](/deploy/tiger-cloud/tiger-cloud-aws/upgrades). To avoid any downtime, each data
...
failure scenarios and during maintenance, [rapid recovery](/deploy/tiger-cloud/tiger-cloud-aws/high-availability/overview#rapid-recovery) is enabled by default for all {C.SERVICE_SHORT}s.
...
The `High` and `Highest` HA strategies are available with the [{C.SCALE} and the {C.ENTERPRISE}](/deploy/tiger-cloud/tiger-cloud-aws/pricing-and-account-management) {C.PRICING_PLAN}s.
