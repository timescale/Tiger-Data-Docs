...
<NumberedList>
  <NumberedItem title="Extract the Kafka binaries to a local folder">
    ```bash
  ...
  <NumberedItem title="Configure and run Apache Kafka">
    ```bash
  ...
  <NumberedItem title="Create Kafka topics">
    In another terminal window, navigate to `<KAFKA_HOME>`, then call `kafka-topics.sh` and create the following topics:
    - `accounts`: publishes JSON messages that are consumed by the timescale-sink connector and inserted into your {C.SERVICE_LONG}.
  ...
  <NumberedItem title="Test that your topics are working correctly">
