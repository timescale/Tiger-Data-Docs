...
## Cannot convert chunk to columnstore

You might see this error message when trying to convert a chunk to the {C.COLUMNSTORE} if
the permissions for the compressed {C.HYPERTABLE} are corrupt.

```sql
tsdb=> CALL convert_to_columnstore('_timescaledb_internal._hyper_65_587239_chunk');
