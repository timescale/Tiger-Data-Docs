...
The {C.PG} `pg_dump` command does not allow you to specify which version of
...
older version of {C.TIMESCALE_DB}, and when you restore it uses the current version,
...
sure the new {C.PG} instance has the same extension version as the original
database before you perform the restore. After the data is restored, you can
upgrade the version of {C.TIMESCALE_DB}.
