...
*   Create a [Python virtual environment](https://docs.python.org/3/library/venv.html). (Optional) ## Connect to TimescaleDB
...
<NumberedItem title={`Locate your ${C.TIMESCALE_DB} credentials and use them to compose a connection`}>
...
<NumberedItem title="Compose your connection string variable as a">

[libpq connection string](https://www.postgresql.org/docs/current/libpq-connect.html#LIBPQ-CONNSTRING), using this format:

```python
CONNECTION = "postgres://username:password@host:port/dbname"
```

If you're using a hosted version of {C.TIMESCALE_DB}, or generally require an SSL
...
<NumberedItem title="Use the `psycopg2` connect function to create a new">

database session and create a new [cursor object](https://www.psycopg.org/docs/connection.html?highlight=cursor#connection.cursor) to
...
<NumberedItem title="Compose a string which contains the SQL statement to create a relational">
...
<NumberedItem title="Open a cursor, execute the query you created in the previous step, and">
...
<NumberedItem title="Create a string variable that contains the `CREATE TABLE` SQL statement for">
...
<NumberedItem title="Formulate a `SELECT` statement that converts the `sensor_data` table to a">
...
the [`create_hypertable` docs](/api/hypertable/create_hypertable):
...
<NumberedItem title="Open a cursor with the connection, execute the statements from the previous">
...
<NumberedItem title="This example inserts a list of tuples, or relational data, called `sensors`,">
...
<NumberedItem title="(Optional) Alternatively, you can pass variables to the `cursor.execute`">
...
<NumberedItem title="Generate some random sensor data using the `generate_series` function">

provided by {C.PG}. This example inserts a total of 480 rows of data (4
...
<NumberedItem title="Define the column names of the table you want to insert data into. This">
...
<NumberedItem title="Create an instance of the `pgcopy` CopyManager, `mgr`, and pass the">
...
<NumberedItem title={`(Optional) The full sample code to insert data into ${C.TIMESCALE_DB} using`}>
...
<NumberedItem title="Define the SQL query you'd like to run on the database. This example is a">
...
<NumberedItem title="Open a cursor from the existing database connection, `conn`, and then execute">
...
<NumberedItem title="To access all resulting rows returned by your query, use one of `pyscopg2`'s">
...
<NumberedItem title="(Optional) If you want a list of dictionaries instead, you can define the">

cursor using [`DictCursor`](https://www.psycopg.org/docs/extras.html#dictionary-like-cursor):
