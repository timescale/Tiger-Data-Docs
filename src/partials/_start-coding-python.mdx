...
<Step title="Import the psycogpg2 library:">
...
<Step title="Use the `psycopg2` connect function to create a new">
...
<Step title="Compose a string which contains the SQL statement to create a relational">
...
<Step title="Open a cursor, execute the query you created in the previous step, and">
...
<Step title="Create a string variable that contains the `CREATE TABLE` SQL statement for">
...
<Step title="Formulate a `SELECT` statement that converts the `sensor_data` table to a">
...
<Step title="Open a cursor with the connection, execute the statements from the previous">
...
<Step title="This example inserts a list of tuples, or relational data, called `sensors`,">
...
<Step title="(Optional) Alternatively, you can pass variables to the `cursor.execute`">
...
<Step title="Generate some random sensor data using the `generate_series` function">
...
<Step title="Define the column names of the table you want to insert data into. This">
...
<Step title="Create an instance of the `pgcopy` CopyManager, `mgr`, and pass the">
...
<Step title="Commit to persist changes:">
...
<Step title={`(Optional) The full sample code to insert data into ${C.TIMESCALE_DB} using`}>
...
<Step title="(Optional) You can also check if the insertion worked:">
...
<Step title="Define the SQL query you'd like to run on the database. This example is a">
...
<Step title="Open a cursor from the existing database connection, `conn`, and then execute">
...
<Step title="To access all resulting rows returned by your query, use one of `pyscopg2`'s">
...
<Step title="(Optional) If you want a list of dictionaries instead, you can define the">
...
<Step title="Write the query using prepared statements:">
