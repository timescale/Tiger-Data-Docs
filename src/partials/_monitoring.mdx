...
`Queries` displays the top 50 queries matching your search. This includes executions, total rows, total time, median time, P95 time, related hypertables, tables in the {C.COLUMNSTORE}, and user name.
...
| `Hypertables`     | If the query ran on a {C.HYPERTABLE}.                                                         |
| `Columnar tables` | If the query drew results from a {C.CHUNK} in the {C.COLUMNSTORE}.                                |
