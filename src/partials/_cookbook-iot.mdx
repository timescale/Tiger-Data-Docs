...
[Designing Your Database Schema: Wide vs. Narrow {C.PG} Tables](https://www.tigerdata.com/learn/designing-your-database-schema-wide-vs-narrow-postgres-tables).
...
require that your data is resampled and synchronized to a common time basis. Fortunately, {C.TIMESCALE_DB} provides
you with [hyperfunctions](/use-timescale/hyperfunctions/) and other tools to help you work with this data. 
...
structure defined in [Narrow table format example](/tutorials/cookbook/#narrow-table-format-example) as a reference
...
An alternative to [SELECT DISTINCT ON](/tutorials/cookbook/#select-distinct-on) is to use a `JOIN LATERAL`. By selecting your entire 
