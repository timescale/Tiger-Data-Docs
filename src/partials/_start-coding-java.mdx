...
*   The [Java Development Kit (JDK)](https://openjdk.java.net) installed.
*   The [{C.PG} JDBC driver](https://jdbc.postgresql.org) installed.
</Prerequisites>
...
In this section, you create a connection to your {C.TIMESCALE_DB} database using an application in
a single file. You can use any of your favorite build tools, including `gradle`
or `maven`.

<NumberedList>

<NumberedItem title="Create the Main.java file">
...
<NumberedItem title="Run the application">
...
<NumberedItem title={`Import the ${C.PG} JDBC driver`}>

If you are using a dependency manager,
   include the [{C.PG} JDBC Driver](https://mvnrepository.com/artifact/org.postgresql/postgresql) as a dependency.

</NumberedItem>

<NumberedItem title="Download the JDBC Driver JAR">

Download the [JAR artifact of the JDBC Driver](https://jdbc.postgresql.org/download/) and
   save it with the `Main.java` file.

</NumberedItem>

<NumberedItem title="Import the JDBC Driver into the application">

Import the `JDBC Driver` into the Java application and display a list of
...
<NumberedItem title="Run the examples">
...
Run all the examples:
   ...
   are ready to connect to {C.TIMESCALE_DB} from Java.
...
<NumberedItem title="Run the code">
...
<NumberedItem title="Compose the `CREATE TABLE` statement">

Compose a string which contains the SQL statement to create a relational
...
<NumberedItem title="Execute the query and verify">

Create a statement, execute the query you created in the previous step, and
...
<NumberedItem title="Write the `CREATE TABLE` statement for the hypertable">

Create a `CREATE TABLE` SQL statement for
...
Create a statement, execute the query you created in the previous step:
	...
	The `by_range` and `by_hash` dimension builder is an addition to {C.TIMESCALE_DB} 2.13.

	</Callout>

</NumberedItem>

<NumberedItem title="Execute and commit the schema changes">

Execute the two statements you created, and commit your changes to the
    database:
...
<NumberedItem title="Insert rows with prepared statements">
...
<NumberedItem title="Insert batches of rows">
...
Define the SQL query you'd like to run on the database. This example
...
<NumberedItem title="Execute the query and read results">

Execute the query with the prepared statement and read out the result set for
...
application from an existing {C.TIMESCALE_DB} instance, be sure to check out these
advanced {C.TIMESCALE_DB} tutorials:

*   [{C.CAGG_CAP}s](/build/continuous-aggregates/create-a-continuous-aggregate)
