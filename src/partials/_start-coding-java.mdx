...
In this section, you create a connection to your {C.SERVICE_SHORT} using an application in
...
1.  Import the PostgreSQL JDBC driver. If you are using a dependency manager,
   include the [PostgreSQL JDBC Driver](https://mvnrepository.com/artifact/org.postgresql/postgresql) as a dependency.

1.  Download the [JAR artifact of the JDBC Driver](https://jdbc.postgresql.org/download/) and
...
1.  Locate your {C.TIMESCALE_DB} credentials and use them to compose a connection
   string for JDBC.
...
## Execute queries on {C.TIMESCALE_DB}
