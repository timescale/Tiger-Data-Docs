...
when the migration of roles to your {C.SERVICE_LONG} is complete, you
...
{C.SERVICE_LONG}s do not support roles with superuser access. If your SQL
...
- `CREATE ROLE "rds`, `ALTER ROLE “rds`, `TO "rds`, `GRANT "rds`: Any creation
  ...
  in a {C.SERVICE_LONG}. Similarly, any grants to or from "rds" prefixed roles
  are ignored as well.
...
- `GRANTED BY role_specification`: The GRANTED BY clause can also have permissions that
  ...
  {C.TIMESCALE_DB} documentation, the GRANTOR in the GRANTED BY clause must be the
