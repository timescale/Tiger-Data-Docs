...
<Step title="Download the schema from the source database">
...
<Step title={`Apply the schema on the target ${C.SERVICE_SHORT}`}>
...
<Step title="Convert tables to hypertables">
...
<Step title={`Convert ${C.PG} partitions to hypertables`}>
...
<Step title="Create a publication that specifies the table to synchronize">
...
<Step title={`Publish the ${C.PG} declarative partitioned table`}>
...
<Step title="Stop syncing a table in the `PUBLICATION`, use `DROP TABLE`">
...
<Step title={`Start the ${C.PG_CONNECTOR}`}>
...
<Step title="View the progress of tables being synchronized">
...
<Step title="Add or remove tables from the publication">
...
<Step title="Update table statistics">
...
<Step title={`Stop the ${C.PG_CONNECTOR}`}>
...
<Step title={`(Optional) Reset sequence nextval on the target ${C.SERVICE_LONG}`}>
