...
<NumberedList>
  <NumberedItem title="Run Zookeeper in Docker">
  ...
  <NumberedItem title="Run Kafka in Docker">
    In another Terminal window, run the following command:
  ...
  <NumberedItem title="Run Kafka Connect in Docker">
    In another Terminal window, run the following command:
  ...
  <NumberedItem title={`Register the Debezium ${C.PG} source connector`}>
    Update the `<properties>` for the `<debezium-user>` you created in your {C.SELF_LONG} instance in the following command.
  ...
  <NumberedItem title="Verify `timescaledb-source-connector` is included in the connector list">
  ...
  <NumberedItem title="Verify `timescaledb-connector` is running">
    ...
    1. Watch the events in the accounts topic on your {C.SELF_LONG} instance.
