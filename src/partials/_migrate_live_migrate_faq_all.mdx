...
1. See what is happening on the target {C.SERVICE_LONG}:
   ```shell
...
1. Run `grants.psql` on your target {C.SERVICE_LONG}. 
   ```shell
...
1. Go to {C.CONSOLE} -> `Monitoring` -> `Insights` tab and find the query which takes significant time
...
1. Upgrade to Higher Memory Spec Instances: To mitigate memory constraints, consider using a {C.TIMESCALE_DB} instance with higher specifications, such as an instance with 8 CPUs and 32 GB RAM (or more). Higher memory capacity can handle larger workloads and reduce the likelihood of OOM errors.
