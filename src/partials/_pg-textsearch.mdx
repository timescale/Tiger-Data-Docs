...
```sql
-- Cap shared memory used by memtables (default 2GB; 0 disables the limit)
SET pg_textsearch.memory_limit = '2GB';

-- Spill to disk after this many terms in a single transaction (default 100000)
...
<Callout variant="note">
`memory_limit` derives three internal thresholds: a per-index soft limit (`memory_limit / 8`) spills that index's memtable to disk, a global soft limit (`memory_limit / 2`) evicts the largest memtable across all indexes, and the hard limit (`memory_limit`) rejects the insert with an error. You only need to set `memory_limit`; the ratios are tuned for typical workloads. The older `pg_textsearch.memtable_spill_threshold` GUC is **deprecated** in favor of `memory_limit`.
</Callout>
