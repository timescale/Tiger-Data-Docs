...
- The [PGX driver for Go](https://github.com/jackc/pgx) installed.
</Prerequisites>

## Connect to {C.TIMESCALE_DB}

In this section, you create a connection to {C.TIMESCALE_DB} using the PGX driver.
PGX is a toolkit designed to help Go developers work directly with {C.PG}.
You can use it to help your Go application interact directly with {C.TIMESCALE_DB}.
...
<NumberedItem title="(Optional) Verify your database connection">
...
<NumberedItem title="Create a connection pool">
...
<NumberedItem title="Execute the `CREATE TABLE` statement">
...
<NumberedItem title="Formulate the `SELECT` statement to convert the table into a hypertable">

You must specify the table name to convert to a hypertable, and its time column name as the second argument. For more information, see the [`create_hypertable` docs](/reference/timescaledb/hypertables/create_hypertable):
...
<NumberedItem title="Execute the statements to create the hypertable">
...
<NumberedItem title="Insert data using prepared statements">
...
<NumberedItem title="(Optional) Full sample code for data generation and insert">
...
<NumberedItem title="Access the returned rows">
...
<NumberedItem title="(Optional) Full sample code for querying">
...
your Go application, check out these resources:
