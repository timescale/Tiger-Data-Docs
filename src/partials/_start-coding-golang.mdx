...
<Step title={`Locate your ${C.TIMESCALE_DB} credentials and use them to compose a connection`}>
...
<Step title="Compose your connection string variable as a">
...
<Step title="(Optional) You can check that you're connected to your database with this">
...
<Step title="To create a connection pool that can be used for concurrent connections to">
...
<Step title="Compose a string that contains the SQL statement to create a relational">
...
<Step title="Execute the `CREATE TABLE` statement with the `Exec()` function on the">
...
<Step title="Create a variable for the `CREATE TABLE SQL` statement for your hypertable.">
...
<Step title="Formulate the `SELECT` statement to convert the table into a hypertable. You">
...
<Step title="Execute the `CREATE TABLE` statement and `SELECT` statement which converts">
...
<Step title="Open a connection pool to the database, then use the prepared statements to">
...
<Step title={`This example uses ${C.PG} to generate some sample time-series to insert`}>
...
<Step title="Formulate an SQL insert statement for the `sensor_data` hypertable:">
...
<Step title="Execute the SQL statement for each sample in the results slice:">
...
<Step title="(Optional) This example `main.go` generates sample data and inserts it into">
...
<Step title="This example batch inserts data into the database:">
...
<Step title="Define the SQL query you'd like to run on the database. This example uses a">
...
<Step title="Use the `.Query()` function to execute the query string. Make sure you">
...
<Step title="Access the rows returned by `.Query()`. Create a struct with fields">
...
<Step title="(Optional) This example program runs a query, and accesses the results of">
