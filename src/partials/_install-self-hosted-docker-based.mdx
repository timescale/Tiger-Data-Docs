...
{C.TIMESCALE_DB} experience. It uses [Ubuntu][ubuntu], includes
...
on {C.PG} 17. To find other Docker tags you can use, see the [Dockerhub repository][dockerhub].
...
The light-weight [{C.TIMESCALE_DB}](https://hub.docker.com/r/timescale/timescaledb) Docker image uses [Alpine][alpine] and does not contain [{C.TOOLKIT_LONG}](https://github.com/timescale/timescaledb-toolkit) or support for PostGIS and Patroni.
...
The link provided in these instructions is for the latest version of {C.TIMESCALE_DB}
on {C.PG} 16. To find other Docker tags you can use, see the [Dockerhub repository][dockerhub].
...
[install-from-source]: /self-hosted/install/installation-source/
