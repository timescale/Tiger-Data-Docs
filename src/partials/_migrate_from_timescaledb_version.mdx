...
It is very important that the version of the {C.TIMESCALE_DB} extension is the same
...
You can determine the version of {C.TIMESCALE_DB} in the target database with the
following command:
...
To update the {C.TIMESCALE_DB} extension in your source database, first ensure that
