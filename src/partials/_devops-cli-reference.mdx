...
```txt
┌───────────────────┬──────────────────────────────────────────┐
│ Tiger CLI Version │ 0.22.0                                   │
│ Build Time        │ 2026-08-26T21:38:28Z                     │
...
```txt
Upgrading tiger 0.21.2 → v0.22.0
Downloading https://cli.tigerdata.com/releases/v0.22.0/tiger-cli_Darwin_arm64.tar.gz
Verifying checksum
Installing new binary to <install-path>/tiger
tiger upgraded successfully to v0.22.0
```
...
Install and configure {C.MCP_LONG} for an AI agent. Supported clients: `claude-code`, `codex`, `cursor`, `gemini`, `vscode`, `windsurf`, `antigravity`, `kiro-cli`. If no client is given, you are prompted to choose. Alias: `add`.
...
Logging is deliberately sparse: past startup it is mostly silent until something goes wrong.
...
Completion covers command and flag names, and suggests the values for flags that accept a fixed set:

| Flag | Completed values |
| --- | --- |
| `--output`, `-o` | `json`, `yaml`, `table`, and also `bare` on `tiger version` |
| `--environment` | `DEV`, `PROD` |
| `--addons` | `time-series`, `ai`, `none` |
| `--cpu`, `--memory` | the allowed allocations, including `shared` |
| `--password-storage` | `keyring`, `pgpass`, `none` |

`tiger config set <key> <value>` completes the value once you have typed the key.
