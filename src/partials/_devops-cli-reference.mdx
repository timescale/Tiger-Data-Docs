...
Create an authenticated connection to your {C.ACCOUNT_LONG}. By default, this opens a browser on the same machine to authorize. On a machine without a browser, pass `--headless` to authorize with a device code instead. For non-interactive login, pass credentials as flags.
...
The project picker only appears if you have multiple {C.PROJECT_SHORT}s and you did not pass `--project-id`. After you select one (or automatically, if you have only one):

```txt
Logged in to project <project-id>
...
The first time you log in interactively, {C.CLI_LONG} also asks `Which services should be protected from writes?` and stores your choice in the [`read_only` configuration option](#configuration-options). The question is asked once; to change the mode later, use `tiger config set read_only <mode>`.
...
| `--project-id` | {C.PROJECT_SHORT} to log in to. Skips the interactive picker. |
| `--public-key` | Public key for non-interactive login. |
| `--secret-key` | Secret key for non-interactive login. |

The browser login redirects back to a `localhost` port on the machine running {C.CLI_SHORT}, so it only works when the browser and {C.CLI_SHORT} are on the same machine; opening the auth URL from another machine does not complete the login. When {C.CLI_SHORT} runs somewhere without a browser, such as an SSH session, a container, or a remote development environment, pass `--headless` to use the device code flow instead:
...
Open the URL in a browser on any machine, enter the code, and authorize {C.CLI_LONG}; the command then continues exactly like the browser login. You don't have to remember the flag: if the browser cannot be opened, `tiger auth login` falls back to the device code flow automatically.
...
Pass `--project-id` to choose the {C.PROJECT_SHORT} up front instead of picking it from the list. With a browser login, this skips the picker; with client credentials, the command fails if the {C.PROJECT_SHORT} does not match the one your keys belong to. To change {C.PROJECT_SHORT} after logging in, use [`tiger project use`](#tiger-project-use).

Unless you log back in to the same {C.PROJECT_SHORT} as last time, the default {C.SERVICE_SHORT} (`service_id`) is cleared, because it belongs to the {C.PROJECT_SHORT} you set it in. If that value came from `--service-id` or `TIGER_SERVICE_ID` rather than your config file, {C.CLI_LONG} warns you instead of clearing it.
...
```txt
Logged out and removed stored credentials
```

To sign a {C.CLI_SHORT} login out remotely, for example on a machine you no longer control, revoke its session in [{C.CONSOLE}](/deploy/tiger-cloud/tiger-cloud-aws/security/overview#active-sessions) instead.

### `tiger auth status`

Show your current authentication status. What it lists depends on how you logged in: an OAuth login shows the authentication method and the user, and a client-credentials login shows the credential name, public key, {C.PROJECT_SHORT}, and plan type. Alias: `whoami`.
...
List the {C.PROJECT_SHORT}s you have access to. The active {C.PROJECT_SHORT}, the one later commands act on, is marked in the `CURRENT` column. Alias: `ls`.
...
To switch to another {C.PROJECT_SHORT}, use [`tiger project use`](#tiger-project-use).
...
Change the {C.PROJECT_SHORT} that later commands act on. Alias: `switch`.
...
If a default {C.SERVICE_SHORT} was set, it is cleared first, because it belongs to the {C.PROJECT_SHORT} you set it in. The message comes before the line above:
...
The `service_id` config key is always cleared. If `--service-id` or `TIGER_SERVICE_ID` is also set, that value takes precedence over the config key, so it survives the switch and still points at a {C.SERVICE_SHORT} in the {C.PROJECT_SHORT} you just left. {C.CLI_LONG} warns you rather than silently leaving it in place:

```txt
Warning: the default service from --service-id/TIGER_SERVICE_ID belongs to the previous project and is still in effect
```

If you name the {C.PROJECT_SHORT} you are already on, nothing changes:

```txt
Already using project <project-id>
```

If you name a {C.PROJECT_SHORT} you cannot reach, the command exits with code `3`:
...
Switching requires a browser login, because a public/secret key pair belongs to a single {C.PROJECT_SHORT}. With client credentials the command exits with code `4` instead. If your keys are stored:
...
To find the ID of the {C.PROJECT_SHORT} you want, run [`tiger project list`](#tiger-project-list), look it up in [{C.CONSOLE_LONG}](/integrate/find-connection-details#find-your-project-and-service-id), or read it out of any {C.SERVICE_SHORT}'s connection string.
...
```txt
┌───────────────────┬──────────────────────────────────────────┐
│ Tiger CLI Version │ 0.25.0                                   │
│ Build Time        │ 2026-09-21T10:28:41Z                     │
...
```txt
Upgrading Tiger CLI 0.24.0 → v0.25.0
Downloading https://cli.tigerdata.com/releases/v0.25.0/tiger-cli_Darwin_arm64.tar.gz
Verifying checksum
Installing new binary to <install-path>/tiger
Upgraded Tiger CLI to v0.25.0
```
...
Send feedback, a bug report, or a support request to the {C.COMPANY} team. Pass the message as an argument, or omit it to read from stdin (with a prompt in an interactive terminal).

**Usage**: `tiger feedback [message]`
...
The message is sent with the email address of your {C.ACCOUNT_SHORT}, so the team can follow up, and with the {C.CLI_SHORT} version and operating system. The command opens no support case and returns no ticket to track; for anything that needs a tracked response, open a support request in [{C.CONSOLE}](https://console.cloud.tigerdata.com/dashboard/support) instead, and the command prints the link for your {C.PROJECT_SHORT}.

## Configuration

Manage {C.CLI_LONG}'s own settings. `tiger config` has the alias `cfg`, so `tiger cfg list` is the same as `tiger config list`. For the full list of keys, see [Configuration options](#configuration-options).

### `tiger config list`

Show the current configuration. Before {C.CLI_LONG} 0.25, the primary name of this command was `config show`; `show` and `ls` still work as aliases.

**Usage**: `tiger config list [flags]`
...
Reset the configuration to defaults, including your default {C.SERVICE_SHORT} and output preferences. This does not log you out; your authentication credentials are unaffected. Alias: `clear`.
...
Default service set to <service-id>.
...
List the {C.SERVICE_SHORT}s in the current {C.PROJECT_SHORT}. Alias: `ls`.
...
```txt
Forking service '<service-id>' at current state...
Service ID: <new-service-id>
Password saved to system keyring
Default service set to <new-service-id>.
Waiting for fork to be ready (timeout: 30m0s)...
Forked service is ready.
Connect with: tiger db psql
```
...
Service resized to 4 CPU/16 GB.
```
...
Start an inactive {C.SERVICE_SHORT}. Alias: `resume`.
...
Stop an active {C.SERVICE_SHORT}. After stopping, the service no longer accepts connections. Alias: `pause`.
...
```txt
Password saved to system keyring
View it with: tiger service get <service-id> --with-password
...
Rename a {C.SERVICE_SHORT}. Only the display name changes: the {C.SERVICE_SHORT} ID, endpoints, and stored credentials are untouched, so existing connections and connection strings keep working.

**Usage**: `tiger service rename <service-id> <new-name>`

```bash
tiger service rename <service-id> analytics-prod
```

You see:

```txt
Renamed service '<service-id>' to 'analytics-prod'.
```

Both arguments are required; there is no default-{C.SERVICE_SHORT} fallback, because a single argument would be ambiguous between the {C.SERVICE_SHORT} to rename and the name to give it.

### `tiger service delete`

Delete a {C.SERVICE_SHORT}. This is irreversible and prompts you to type the {C.SERVICE_SHORT} ID to confirm before proceeding. The command returns as soon as the API accepts the deletion. Alias: `rm`.
...
Before {C.CLI_LONG} 0.25, this command also took `--no-wait` and `--wait-timeout`; they were removed because the API removes the {C.SERVICE_SHORT} from view as soon as it accepts the deletion, so there is nothing to wait for. The [`service_delete`](/reference/tiger-cloud/tiger-mcp#service_delete) {C.MCP_SHORT} tool is the agent-side equivalent; deleting a {C.SERVICE_SHORT} tagged `PROD` through it prompts you for confirmation through the MCP client.
...
Connect to a {C.SERVICE_SHORT} with `psql`. Pass extra `psql` flags after `--`, for example `tiger db psql <service-id> -- --single-transaction`. Before {C.CLI_LONG} 0.25, the primary name of this command was `db connect`; `connect` still works as an alias.

**Usage**: `tiger db psql <service-id> [flags]`

```bash
tiger db psql <service-id>
```
...
| `--read-only` | Open the session in {C.CLOUD_LONG}'s immutable read-only mode, so writes and DDL are rejected by the server. The [`read_only` config option](#configuration-parameters) (or `TIGER_READ_ONLY`) forces the same behavior for every {C.SERVICE_SHORT} (`all`) or for {C.SERVICE_SHORT}s tagged `PROD` (`prod`). |
| `--no-replica-prompt` | Do not prompt to connect to a read replica. |

### `tiger db query`

Run a SQL query against a {C.SERVICE_SHORT} and print the results. Unlike [`db psql`](#tiger-db-psql), this runs the query directly and does not require a local `psql` installation. Pass a read replica set ID instead of a {C.SERVICE_SHORT} ID to query that replica. Alias: `sql`.
...
This command is the {C.CLI_SHORT} equivalent of the [`db_query`](/reference/tiger-cloud/tiger-mcp#db_query) {C.MCP_SHORT} tool, with one difference: the {C.MCP_SHORT} tool caps how many rows it returns to the agent, while the {C.CLI_SHORT} command returns everything the query produced.

### `tiger db uri`

Print the connection string for a {C.SERVICE_SHORT}. Before {C.CLI_LONG} 0.25, the primary name of this command was `db connection-string`; the old name still works as an alias.

**Usage**: `tiger db uri <service-id> [flags]`

```bash
tiger db uri <service-id>
```
...
Test connectivity to a {C.SERVICE_SHORT}. Before {C.CLI_LONG} 0.25, the primary name of this command was `db test-connection`; `test-connection` and `test` still work as aliases.

**Usage**: `tiger db ping <service-id> [flags]`

```bash
tiger db ping <service-id>
```
...
Install and configure {C.MCP_LONG} for an AI agent. Supported clients: `claude-code`, `codex`, `copilot`, `cursor`, `devin`, `gemini`, `vscode`, `windsurf`, `antigravity`, `kiro-cli`. If no client is given, you are prompted to choose. Alias: `add`.
...
List the available {C.MCP_SHORT} tools, prompts, and resources. Alias: `ls`.
...
{C.CLI_LONG} ships completions for bash, zsh, fish, and PowerShell. Installing with Homebrew sets up the bash, zsh, and fish completions for you; otherwise generate them with `tiger completion <shell>`.

Completion covers command and flag names, most flag values, and most command arguments. That includes {C.SERVICE_SHORT} and project IDs, so you can complete them instead of typing or pasting them.
