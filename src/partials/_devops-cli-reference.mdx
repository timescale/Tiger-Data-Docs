...
This requires a browser login (`tiger auth login` with no keys). A public/secret key pair belongs to a single {C.PROJECT_SHORT}, so if you authenticated with client credentials, the command refuses and tells you to log in with the other {C.PROJECT_SHORT}'s keys instead.

Switching clears the default {C.SERVICE_SHORT} (`service_id`), because it belongs to the {C.PROJECT_SHORT} you set it in.
...
If you name a {C.PROJECT_SHORT} you cannot reach, the reason is printed and the command exits with code `3`:
...
Find your {C.PROJECT_SHORT} ID in [{C.CONSOLE_LONG}](/integrate/find-connection-details#find-your-project-and-service-id), or read it out of any {C.SERVICE_SHORT}'s connection string.
