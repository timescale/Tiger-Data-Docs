...
If a default {C.SERVICE_SHORT} was set, it is cleared first, because it belongs to the {C.PROJECT_SHORT} you set it in:

```txt
Cleared default service (config key service_id): it belonged to the previous project
```

If that value came from `--service-id` or `TIGER_SERVICE_ID` rather than your config file, {C.CLI_LONG} cannot clear it, so it warns instead:
