...
[{C.REST_LONG}](/api/api-reference/) is a comprehensive RESTful API you use to manage {C.CLOUD_LONG} resources
...
1. In {C.CONSOLE} [copy your project ID](/integrations/find-connection-details/#find-your-project-and-service-id) and store it securely using an environment variable:

   ```bash
   export TIGERDATA_PROJECT_ID="your-project-id"
   ```

 1. In {C.CONSOLE} [create your client credentials](/integrations/find-connection-details/#create-client-credentials) and store them securely using environment variables:
...
And that is it, you are ready to use the [{C.REST_LONG}](/api/api-reference/) to manage your
