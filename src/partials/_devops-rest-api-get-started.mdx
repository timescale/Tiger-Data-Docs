...
<Step title={`Test your authenticated connection to ${C.REST_LONG} by listing the ${C.SERVICE_SHORT}s in the current ${C.PROJECT_LONG}`}>
...
<Step title={`Create a ${C.SERVICE_SHORT} using the POST endpoint`}>
...
<Step title="Save `service_id` from the response to a variable:">
...
<Step title={`Check the configuration for the ${C.SERVICE_SHORT}`}>
