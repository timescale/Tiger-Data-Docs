...
<NumberedItem title="At the command prompt, dump the schema post-data from your source database">
...
<NumberedItem title="Restore the dumped schema post-data from the `dump_post_data.dump` file into">

your {C.SERVICE_LONG}, using your connection details. To avoid permissions
...
definitions and recomputing the results on your {C.SERVICE_LONG}. The recomputed
continuous aggregates only aggregate existing data in your {C.SERVICE_LONG}. They
...
separately. Recreate them on your {C.SERVICE_LONG}.
...
<NumberedItem title="Get a list of your existing policies. This query returns a list of all your">
...
<NumberedItem title="Recreate each policy. For more information about recreating policies, see">

the sections on [continuous-aggregate refresh policies](/build/continuous-aggregates/create-a-continuous-aggregate),
[retention policies](/build/data-management/data-retention/create-a-retention-policy), [Hypercore policies](/build/columnar-storage/setup-hypercore), and [reorder policies](/reference/timescaledb/hypertables/add_reorder_policy).
...
Update your table statistics by running [`ANALYZE`](https://www.postgresql.org/docs/10/sql-analyze.html) on your entire
