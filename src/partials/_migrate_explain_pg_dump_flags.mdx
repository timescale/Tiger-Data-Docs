...
- `--no-tablespaces` is required because {C.CLOUD_LONG} does not support
  tablespaces other than the default. This is a known limitation.

- `--no-owner` is required because {C.CLOUD_LONG}'s `tsdbadmin` user is not a
...
- `--no-privileges` is required because the `tsdbadmin` user for your {C.SERVICE_LONG} is not a
