...
{C.PG} builds the index on the fly during ingestion. That means that to build a new entry on the index, 
...
You set `chunk_interval` when you [create a {C.HYPERTABLE}](/reference/timescaledb/hypertables/create_table), or by calling 
[`set_chunk_time_interval`](/reference/timescaledb/hypertables/set_chunk_time_interval) on an  existing hypertable.
