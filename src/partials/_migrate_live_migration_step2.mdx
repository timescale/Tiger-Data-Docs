...
import SetupSourceTarget from "./_migrate_set_up_source_and_target.mdx";

<SetupSourceTarget />

<Callout variant="tip">

Do not use a {C.CLOUD_LONG} connection pooler connection for live migration. There
...
your instance, as shown on [Configure database parameters](/deploy/tiger-cloud/tiger-cloud-aws/configuration/customize-configuration#configure-database-parameters).

</Callout>

It's important to ensure that the `old_snapshot_threshold` value is set to the
default value of `-1` in your source database. This prevents {C.PG} from
