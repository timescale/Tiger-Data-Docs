...
<NumberedItem title="Download the dataset:">

[metrics.csv.gz](https://assets.timescale.com/docs/downloads/metrics.csv.gz)

</NumberedItem>

<NumberedItem title="Decompress the downloaded dataset">

Take a note of the path to the `metrics.csv` file.

</NumberedItem>

<NumberedItem title="Copy the data from the `metrics.csv` file into your hypertable">

Make sure you point to the correct path, if it is not in
...
<NumberedItem title="You can check that the data has been copied successfully with this command:">
