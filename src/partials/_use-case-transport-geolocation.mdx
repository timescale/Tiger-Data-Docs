...
1. In Grafana, create a new dashboard that is connected to your {C.SERVICE_LONG} data source with a Geomap
   visualization.

1. In the `Queries` section, select `Code`, then select the Time series `Format`.

   ![Real-time analytics geolocation](../assets/images/use-case-rta-grafana-timescale-configure-dashboard.webp)
...
1. In the Geomap options, under `Map Layers`, click `+ Add layer` and select `Heatmap`.
  ...
  longer ride, and make more money.
   ...
   ![Real-time analytics geolocation](../assets/images/use-case-rta-grafana-heatmap.webp)
