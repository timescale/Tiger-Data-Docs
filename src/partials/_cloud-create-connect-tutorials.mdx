...
A service in {C.CLOUD_LONG} is a cloud instance which contains your database.
Each service contains a single database, named `tsdb`.
You can connect to a service from your local system using the `psql`
command-line utility. If you've used {C.PG} before, you might already have
`psql` installed. If not, check out the [installing psql](/integrate/query-administration/psql/) section.

<NumberedList>

<NumberedItem title={`In the ${C.CONSOLE}, click \`Create service\`.`}></NumberedItem>

<NumberedItem title="Click `Download the cheatsheet` to download an SQL file that contains the">
...
<NumberedItem title="On your local system, at the command prompt, connect to the service using">
