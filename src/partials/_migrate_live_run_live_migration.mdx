...
<NumberedList>
<NumberedItem title="Pull the live-migration Docker image to your migration machine">
...
<NumberedItem title={`Create a snapshot image of your source database in your ${C.SERVICE_LONG}`}>
...
<NumberedItem title={`Synchronize data between your source database and your ${C.SERVICE_LONG}`}>

This command migrates data from the snapshot to your {C.SERVICE_LONG}, then streams
...
<Callout variant="tip">
If the source {C.PG} version is 17 or later, you need to pass additional
...
contain time-series data in your {C.SERVICE_LONG}. Run `create_hypertable()` to convert these
tables. For more information, see the [hypertable docs](/build/data-management/understand-hypertables).
...
Once the data in your target {C.SERVICE_LONG} has almost caught up with the source database,
...
<NumberedItem title="Start app downtime">

<ol type="a">
<li>

Stop your app writing to the source database, then let the remaining transactions
...
{C.TIMESCALE_DB} metadata, sequences, and run policies. When the migration completes,
you see the following message:
