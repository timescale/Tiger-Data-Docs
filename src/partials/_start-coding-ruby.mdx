...
*   [Rails](https://guides.rubyonrails.org/install_ruby_on_rails.html#installing-rails) installed.
</Prerequisites>

## Connect a Rails app to {C.TIMESCALE_DB}

{C.TIMESCALE_DB} extends {C.PG} with time-series superpowers. You connect to your
{C.TIMESCALE_DB} database from a standard Rails app configured for {C.PG}. 

<NumberedList>

<NumberedItem title={`Create a new Rails app configured for ${C.PG}`}>

Rails creates and bundles your app, then installs the standard {C.PG} Gems.
...
<NumberedItem title={`Install the ${C.TIMESCALE_DB} gem`}>
...
<NumberedItem title={`Connect your app to ${C.TIMESCALE_DB}`}>

1.  In `<my_app_home>/config/database.yml` update the configuration to securely connect to your {C.TIMESCALE_DB} database
...
1.  Set the environment variable for `DATABASE_URL` to your connection string:
    ```bash
...
1.  Verify the connection from your app to your {C.TIMESCALE_DB} database:
   ...
   | postgres_fdw        | 1.1     | public     | foreign-data wrapper for remote {C.PG} servers| 
...
Hypertables are {C.PG} tables designed to simplify and accelerate data analysis. Anything 
you can do with regular {C.PG} tables, you can do with hypertables - but much faster and more conveniently.

In this section, you use the helpers in the {C.TIMESCALE_DB} gem to create and manage a [{C.HYPERTABLE}](/build/how-to/your-first-hypertable).

<NumberedList>

<NumberedItem title="Generate a migration to create the page loads table">
...
<NumberedItem title="Add hypertable options">
...
with the following:
 ...
 The `id` column is not included in the table. This is because {C.TIMESCALE_DB} requires that any `UNIQUE` or `PRIMARY KEY` 
...
<NumberedItem title="Create a `PageLoad` model">
...
The {C.TIMESCALE_DB} gem provides efficient ways to insert data into hypertables. This section 
shows you how to ingest test data into your hypertable.

<NumberedList>

<NumberedItem title="Create a controller to handle page loads">
...
<NumberedItem title="Generate some test data">
...
<NumberedItem title={`Insert the generated data into your ${C.TIMESCALE_DB} database`}>
...
<NumberedItem title={`Validate the test data in your ${C.TIMESCALE_DB} database`}>
...
This section lists the most common tasks you might perform with the {C.TIMESCALE_DB} gem.

### Query scopes

The {C.TIMESCALE_DB} gem provides several convenient scopes for querying your time-series data.
...
### {C.TIMESCALE_DB} features

The {C.TIMESCALE_DB} gem provides utility methods to access hypertable and chunk information. Every model that uses 
...
- Create or drop a continuous aggregate:
  ...
  works in this [blog post](https://www.tigerdata.com/blog/building-a-better-ruby-orm-for-time-series-and-analytics).
...
* Learn more about the [{C.TIMESCALE_DB} gem](https://github.com/timescale/timescaledb-ruby).
