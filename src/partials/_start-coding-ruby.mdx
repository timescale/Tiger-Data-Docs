...
Every {C.SERVICE_LONG} is a 100% {C.PG} database hosted in {C.CLOUD_LONG} with
{C.COMPANY} extensions such as {C.TIMESCALE_DB}. You connect to your {C.SERVICE_LONG}
from a standard Rails app configured for {C.PG}. 
...
<NumberedItem title={`Connect your app to your ${C.SERVICE_LONG}`}>

1.  In `<my_app_home>/config/database.yml` update the configuration to read securely connect to your {C.SERVICE_LONG}
...
1.  Set the environment variable for `DATABASE_URL` to the value of `Service URL` from
    your [connection details](/integrations/find-connection-details/)
    ```bash
    export DATABASE_URL="value of Service URL"
    ```

1. Create the database:
   - **{C.CLOUD_LONG}**: nothing to do. The database is part of your {C.SERVICE_LONG}.  
   - **{C.SELF_LONG_CAP}**, create the database for the project:
...
1.  Verify the connection from your app to your {C.SERVICE_LONG}:

    ```bash
    echo "\dx" | rails dbconsole
    ```

    The result shows the list of extensions in your {C.SERVICE_LONG}
...
In this section, you use the helpers in the {C.TIMESCALE_DB} gem to create and manage a [hypertable](/use-timescale/hypertables/).
...
<NumberedItem title={`Insert the generated data into your ${C.SERVICE_LONG}`}>
...
<NumberedItem title={`Validate the test data in your ${C.SERVICE_LONG}`}>
