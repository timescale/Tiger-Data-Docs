...
Replace `<POOLER_HOST>` and `<POOLER_PORT>` with the values shown in the `Connection pooling` tab.
The host is the same as your {C.SERVICE_SHORT}, the port is different:

```bash
PGPASSWORD=<PASSWORD> psql "postgres://<MY_APP>@<POOLER_HOST>:<POOLER_PORT>/tsdb?sslmode=require"
```
...
6. run SQL: `CREATE ROLE my_app LOGIN PASSWORD '$POOLER_PASSWORD';`
7. run SQL: `ALTER ROLE my_app SET statement_timeout TO '2s';`
8. run command: `PGPASSWORD=$POOLER_PASSWORD psql "postgres://my_app@<POOLER_HOST>:<POOLER_PORT>/tsdb?sslmode=require"`
9. run SQL: `SELECT current_user;`
10. expect rows: `SELECT 1 WHERE current_user = 'my_app'`
11. expect rows: `SELECT 1 WHERE current_setting('statement_timeout') = '2s'`
12. exit psql
13. run command: `psql "postgres://tsdbadmin@<POOLER_HOST>:<POOLER_PORT>/pgbouncer" -c "SHOW STATS;"`
14. click `Remove connection pooler`
15. confirm `Remove`
16. expect no label `Remove connection pooler`
