...
```bash
PGPASSWORD=<NEW_PASSWORD> psql "postgres://my_app@<POOLER_HOST>:<POOLER_PORT>/tsdb?sslmode=require"
```
...
9. expect rows: `SELECT 1 WHERE current_user = 'my_app'`
10. expect rows: `SELECT 1 WHERE current_setting('statement_timeout') = '2s'`
11. exit psql
12. run command: `psql "postgres://tsdbadmin@<POOLER_HOST>:<POOLER_PORT>/pgbouncer" -c "SHOW STATS;"`
13. click `Remove connection pooler`
14. confirm `Remove`
15. expect no label `Remove connection pooler`
